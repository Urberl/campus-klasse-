...
function updateTeacherTeamNav(){
  ...
  // Lernressourcen sind für alle freigeschalteten Campus-Mitglieder sichtbar.
  // Falls die vorhandene Navigation keinen direkten Eintrag enthält, ergänzen wir ihn.
  let resourceLink=sidebar.querySelector('a[href="#ressourcen"]');
  if(!resourceLink){
    resourceLink=document.createElement("a");
    resourceLink.href="#ressourcen";
    resourceLink.dataset.page="ressourcen";
    resourceLink.className="nav-link";
    resourceLink.innerHTML='<span>📚</span> Lernressourcen';
    const lernwerkstattLink=sidebar.querySelector('a[href="#lernwerkstatt"]');
    if(lernwerkstattLink) lernwerkstattLink.insertAdjacentElement("afterend",resourceLink);
    else sidebar.querySelector("nav")?.appendChild(resourceLink);
  }
  resourceLink.hidden=!isApproved();
  resourceLink.setAttribute("aria-hidden",String(!isApproved()));
