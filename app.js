...
async function renderJournal(){
  let data=[];
  try{
    // Kein orderBy() mehr: dadurch wird kein zusätzlicher Firestore-Index benötigt.
    // Die Sortierung erfolgt anschließend lokal.
    ...
    return `${pageHead(
      ...
      `<button class="primary" onclick="openJournalForm()">＋ Eintrag</button>`
    )}
    <div class="notice">
      <strong>Das Lernjournal konnte gerade nicht geladen werden.</strong>
  ...
  <div class="list">${data.map(j=>`<article class="card">
    ...
    <p>${esc(j.text)}</p>
  </article>`).join("")||`<div class="empty"><strong>Noch kein Eintrag</strong><p>Starte mit einer kurzen Reflexion.</p></div>`}</div>${footer()}`;
}
