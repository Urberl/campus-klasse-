...
async function renderKalender(){
  ...
  return `${pageHead("ORGANISATION","Campus-Kalender","Das Schuljahr 26/27 auf einen Blick. Monate mit Terminen sind sofort erkennbar.",
    `<button class="primary" onclick="openCalendarForm()">＋ Termin eintragen</button>`)}
  <style>
...
function openCalendarForm(){
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">CAMPUS-KALENDER</div><h2>Termin ergänzen</h2><div class="form">
    <label>Titel<input id="calTitle" placeholder="z. B. Pädagogischer Halbtag" required></label>
    <label>Datum<input id="calDate" type="date" required></label>
    <label>Ort <span style="color:var(--muted)">(optional)</span><input id="calLocation" placeholder="z. B. F203"></label>
    <label>Beschreibung <span style="color:var(--muted)">(optional)</span><textarea id="calDescription" rows="3" placeholder="Weitere Informationen …"></textarea></label>
    <div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="addCalendar()">Speichern</button></div>
  </div>`);
}
async function addCalendar(){
  const title=$("calTitle")?.value.trim()||"";
  const date=$("calDate")?.value||"";
  const location=$("calLocation")?.value.trim()||"";
  const description=$("calDescription")?.value.trim()||"";
  if(!title){toast("Bitte einen Titel eingeben.");$("calTitle")?.focus();return}
  if(!date){toast("Bitte ein Datum auswählen.");$("calDate")?.focus();return}
  try{
    await addDoc(collection(db,"events"),{
      title,date,start:date,location,description,
      createdBy:currentUser.uid,createdAt:serverTimestamp()
    });
    closeModal();
    await render();
    ...
    console.error("Kalendertermin speichern:",e);
    toast("Termin konnte nicht gespeichert werden.");
  }
}
