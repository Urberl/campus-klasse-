...
const JOURNAL_LIST_CSS=`<style id="journal-list-inline-style">
.journal-overview-card{margin-top:12px}.journal-overview-header{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:14px}
.journal-table{border:1px solid var(--border,#dfe5e2);border-radius:12px;overflow:hidden;background:#fff}
.journal-table-head,.journal-table-row{display:grid;grid-template-columns:150px 1fr 100px;align-items:center;gap:12px}
.journal-table-head{padding:10px 14px;font-size:11px;font-weight:800;letter-spacing:.08em;color:#68736f;background:#f4f7f5}
.journal-table-row{min-height:54px;padding:7px 10px 7px 14px;border-top:1px solid #e8ecea}
.journal-entry-link{border:0;background:transparent;text-align:left;display:grid;grid-template-columns:150px 1fr;gap:12px;align-items:center;padding:7px 4px;cursor:pointer;color:inherit;width:100%;font:inherit}
.journal-entry-link:hover .journal-title{text-decoration:underline}.journal-date{font-size:13px;color:#68736f;white-space:nowrap}.journal-title{font-weight:700}
.journal-pdf-button{justify-self:end;border:1px solid #cfd8d4;border-radius:8px;background:#fff;padding:8px 12px;cursor:pointer;font:inherit}
@media(max-width:700px){.journal-overview-header{align-items:flex-start;flex-direction:column}.journal-table-head,.journal-table-row{grid-template-columns:92px 1fr 72px;gap:7px}.journal-entry-link{grid-template-columns:1fr;gap:2px}.journal-date{font-size:12px}.journal-title{font-size:14px}.journal-pdf-button{padding:7px 8px;font-size:12px}}
</style>`;
...
async function renderJournal(){
  ...
  try{
    ...
    <div class="notice"><strong>Das Lernjournal konnte gerade nicht geladen werden.</strong><p>Bitte versuche es erneut.</p></div>${footer()}`;
  }
  const teacherButton=isTeacher()?`<button class="secondary" onclick="openTeacherJournalOverview()">👩‍🏫 Schüler-Lernjournale</button>`:"";
  const rows=data.map(j=>`<div class="journal-table-row">
    <button class="journal-entry-link" onclick="openJournalEntry('${j.id}')"><span class="journal-date">${esc(fmtDate(j.createdAt))}</span><span class="journal-title">${esc(j.title||"Reflexion")}</span></button>
    <button class="journal-pdf-button" onclick="printSingleJournalPDF('${j.id}')">📄 PDF</button>
  </div>`).join("");
  return `${JOURNAL_LIST_CSS}${pageHead("REFLEXION","Lernjournal","Dein Lernweg, Reflexionen und nächste Schritte.",`${teacherButton}<button class="primary" onclick="openJournalForm()">＋ Eintrag</button>`)}
  <div class="card journal-overview-card"><div class="journal-overview-header"><div><span class="badge">🌱 DEIN LERNWEG</span><h2>Meine Lernjournal-Einträge</h2><p>Klicke auf Datum oder Titel, um den vollständigen Eintrag zu öffnen.</p></div>${data.length?`<button class="secondary" onclick="printMyJournalPDF()">📄 Alle als PDF</button>`:""}</div>
  ${data.length?`<div class="journal-table"><div class="journal-table-head"><span>DATUM</span><span>TITEL</span><span>PDF</span></div>${rows}</div>`:`<div class="empty"><strong>Noch kein Eintrag</strong><p>Starte mit einer kurzen Reflexion.</p><button class="primary" onclick="openJournalForm()">＋ Erste Reflexion</button></div>`}</div>${footer()}`;
}
...
function openJournalPrintWindow(title,students){
  ...
  const sections=students.map(student=>`
    <section class="student-section">
      <h1>${escPDF(student.name)}</h1><div class="meta">CampusKlasse · Persönliches Lernjournal</div>
      ${student.entries.map(j=>`<article class="entry"><div class="date">${escPDF(journalPDFDate(j.createdAt))}</div>
      <h2>${escPDF(j.title||"Reflexion")}</h2>${j.mood?`<div class="mood">${escPDF(j.mood)}</div>`:""}
      <p>${escPDF(j.text||"—").replace(/\n/g,"<br>")}</p></article>`).join("")}
    </section>`).join("");
  win.document.write(`<!doctype html><html lang="de"><head><meta charset="utf-8"><title>${escPDF(title)}</title>
  <style>@page{size:A4;margin:18mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#222;line-height:1.55;margin:0}
  h1{font-size:28px;margin:0 0 4px}h2{font-size:18px;margin:6px 0 10px}.meta{color:#666;font-size:12px;margin-bottom:24px}
  .student-section{page-break-after:always}.student-section:last-child{page-break-after:auto}.entry{border:1px solid #ddd;border-radius:10px;padding:14px;margin:0 0 14px;break-inside:avoid}
  .date{font-size:11px;color:#777}.mood{font-size:22px;margin:4px 0}.print-note{background:#f3f3f3;padding:10px;border-radius:8px;margin-bottom:20px;font-size:12px}@media print{.print-note{display:none}}</style></head>
  <body><div class="print-note">Im Druckdialog „Als PDF sichern“ bzw. „PDF“ auswählen.</div>${sections}
...
async function printMyJournalPDF(){
  try{
    const entries=await getMyJournalEntries();
    if(!entries.length){toast("Noch keine Lernjournal-Einträge vorhanden.");return}
    openJournalPrintWindow("Lernjournal – "+(profile?.displayName||"Schüler/in"),[
      {name:profile?.displayName||currentUser?.email||"Schüler/in",entries}
    ]);
  }catch(e){console.error(e);toast("Das Lernjournal konnte nicht als PDF geöffnet werden.")}
}
...
async function printSingleJournalPDF(id){
  try{
    ...
    if(!j){toast("Der Eintrag wurde nicht gefunden.");return}
    closeModal();
    openJournalPrintWindow("Lernjournal – "+(j.title||"Reflexion"),[
      {name:profile?.displayName||currentUser?.email||"Schüler/in",entries:[j]}
    ]);
  }catch(e){console.error(e);toast("Der Eintrag konnte nicht als PDF geöffnet werden.")}
}
...
async function addJournal(){
  try{
    ...
    try{await render()}catch(renderError){console.error("Lernjournal neu rendern:",renderError)}
  }catch(e){
