...
async function renderResilienz(){
  ...
  <style>
    ...
    .breath-circle{width:154px;height:154px;border-radius:50%;margin:18px auto;display:flex;align-items:center;justify-content:center;border:4px solid currentColor;transform:scale(.84);transition:transform 4s linear,color .6s ease,background .6s ease}
...
async function getMyResilienzSchaetze(){
  ...
  try{
    ...
    return snap.docs.map(d=>({id:d.id,...d.data()}));
  }catch(e){console.error("Resilienz-Schatzkiste laden:",e);return []}
}
...
async function openResilienzSchatzkiste(){
  const favorites=await getMyResilienzSchaetze();
  const cards=favorites.map(f=>{const d=resilienzSkillData(f.skillId);return `<button class="card res-card" onclick="startResilienzSkill('${f.skillId}')"><div class="res-icon">${d[0]}</div><h3>${d[1]}</h3><p>${d[2]}</p></button>`}).join("");
  modal(`<button class="modal-close" onclick="closeResilienzModal()">×</button><div class="kicker">MEINE RESILIENZ-SCHATZKISTE</div>
    <h2>🧰 Meine Schätze</h2><p>Übungen, die du für dich als hilfreich ausgewählt hast. Nur du kannst das hier sehen – nicht einmal Lehrkräfte.</p>
