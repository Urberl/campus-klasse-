...
async function renderForum(){
 ...
 return `${pageHead("GEMEINSCHAFT","Campus-Forum","Gemeinsam denken, fragen, austauschen und unterstützen.",`<button class="primary" onclick="openPostForm()">＋ Beitrag schreiben</button>`)}
   <div class="card" style="margin-bottom:12px"><div class="chips" style="display:flex;gap:8px;flex-wrap:wrap">
     <button class="chip" style="cursor:pointer" onclick="openForumArea('forum')">💬 Öffentlicher Austausch</button>
     <button class="chip" style="cursor:pointer" onclick="openForumArea('messages')">✉️ Meine Nachrichten</button>
   </div></div>
...
async function getMessageUsers(){
  ...
  try{
    ...
    const snap=await getDocs(q);
    return snap.docs.map(d=>({id:d.id,...d.data()})).filter(u=>u.id!==currentUser.uid && (u.role==="teacher"||u.role==="admin"||u.role==="student"));
  }catch(e){
...
async function renderForumMessages(){
  ...
  <div class="grid grid-3">
    <div class="card" style="grid-column:span 1"><h3>Unterhaltungen</h3>
      ${contacts.map(c=>`<button type="button" class="list-item" style="width:100%;text-align:left;border:0;background:transparent;cursor:pointer" onclick="selectMessageConversation('${c.uid}')"><div><strong>${displayName(c.uid)}</strong><small>${esc(c.last?.subject||c.last?.text||"")}</small></div><span class="pill">${c.rows.length}</span></button>`).join("")||`<div class="empty">Noch keine Nachrichten.</div>`}
    </div>
    <div class="card" style="grid-column:span 2">
      ${selectedUid?`<h3>Nachrichten mit ${displayName(selectedUid)}</h3><div class="list" style="max-height:420px;overflow:auto">${selectedRows.map(m=>`<div class="list-item"><div><strong>${esc(m.senderName||m.authorName||"Campus-Mitglied")}</strong><small>${esc(m.subject||"")} · ${messageTime(m.createdAt)}</small><div style="margin-top:6px">${esc(m.text||"")}</div></div></div>`).join("")||`<div class="empty">Noch keine Nachrichten.</div>`}</div>
      <div class="form-actions" style="margin-top:12px"><button class="primary" onclick="openMessageForm('${selectedUid}')">Antworten</button></div>`:`<div class="empty"><strong>Noch keine Unterhaltung ausgewählt.</strong><p>Starte eine neue Nachricht.</p><button class="primary" onclick="openMessageForm()">＋ Neue Nachricht</button></div>`}
    </div>
  </div>${footer()}`;
}
