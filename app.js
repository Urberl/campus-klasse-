...
async function renderJournal(){
  ...
  try{
    ...
    console.error("Lernjournal konnte nicht geladen werden:",e);
    return `${pageHead(
      "REFLEXION",
      "Lernjournal",
      "Dein persönlicher Lernweg – festhalten, verstehen, weiterentwickeln.",
      isTeacher()?`<button class="secondary" onclick="openTeacherJournalOverview()">👩‍🏫 Schüler-Lernjournale</button>`:""
    )}
    <div class="card">
      <h2>Lernjournal konnte nicht geladen werden.</h2>
      <p>Die Verbindung zur Lernjournal-Datenbank konnte nicht hergestellt werden.</p>
      <div class="notice">
        <strong>Technischer Hinweis</strong>
        <p>${esc(e?.message||"Unbekannter Fehler")}</p>
      </div>
  ...
  const teacherButton=isTeacher()
    ? `<button type="button" class="secondary" onclick="openTeacherJournalOverview()">👩‍🏫 Schüler-Lernjournale</button>`
    : "";

  const rows=data.map(j=>`
    <div class="journal-library-row">
      <div class="journal-library-date">${esc(journalDisplayDate(j))}</div>
      <button type="button" class="journal-library-title" onclick="openJournalEntry('${esc(j.id)}')">${esc(j.title||"Lernjournal")}</button>
  ...
  return `
  <style>
    .journal-two-tiles{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,1fr);gap:16px;align-items:start}
    .journal-tile{min-width:0;height:100%}
    .journal-tile-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:16px}
    .journal-tile-head h2{margin:4px 0 5px}
    .journal-tile-head p{margin:0;color:var(--muted)}
    .journal-form-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
    .journal-form-grid .full{grid-column:1/-1}
    .journal-library{overflow:hidden;border:1px solid var(--line,#ddd);border-radius:12px}
    .journal-library-head,.journal-library-row{display:grid;grid-template-columns:110px minmax(0,1fr) 70px;gap:10px;align-items:center;padding:11px 12px}
    .journal-library-head{background:var(--soft-green);font-size:12px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.04em}
    .journal-library-row{border-top:1px solid var(--line,#ddd);background:#fff}
    .journal-library-row:hover{background:var(--soft,#fafafa)}
    .journal-library-date{font-size:13px;color:var(--muted)}
    .journal-library-title{border:0;background:none;padding:0;text-align:left;font:inherit;font-weight:700;color:inherit;cursor:pointer;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .journal-library-title:hover{text-decoration:underline}
    .journal-pdf-btn{border:1px solid var(--line,#ddd);background:#fff;border-radius:8px;padding:7px 8px;cursor:pointer;white-space:nowrap}
    .journal-pdf-btn:hover{background:var(--soft-green)}
    .journal-empty{padding:24px 16px;text-align:center;color:var(--muted)}
    ...
    @media(max-width:520px){
      .journal-library-head,.journal-library-row{grid-template-columns:82px minmax(0,1fr) 55px;gap:7px;padding:10px 8px}
      .journal-library-head{font-size:10px}
      .journal-library-date{font-size:11px}
      .journal-pdf-btn{padding:6px 4px;font-size:11px}
    }
  </style>
  ...
  <div class="journal-two-tiles">
    ...
    <section class="card journal-tile">
      <div class="journal-tile-head">
        <div>
          <span class="badge">✍️ LERNJOURNAL ERSTELLEN</span>
          <h2>Was habe ich heute gelernt?</h2>
          <p>Nimm dir kurz Zeit, deinen Lernprozess bewusst festzuhalten.</p>
        </div>
      </div>
      ...
      <div class="form journal-form-grid">
        ...
        <label>Titel
          <input id="jTitle" type="text" placeholder="z. B. Heute habe ich verstanden …">
        </label>

        <label class="full">Woran habe ich heute gearbeitet?
          <textarea id="jWorkedOn" rows="3" placeholder="Welche Aufgabe, welches Thema oder welches Projekt stand heute im Mittelpunkt?"></textarea>
        </label>

        <label>Was habe ich verstanden oder gelernt?
          <textarea id="jLearned" rows="4" placeholder="Was ist mir heute klarer geworden? Was kann ich jetzt besser als vorher?"></textarea>
        </label>

        <label>Was war schwierig?
          <textarea id="jDifficult" rows="4" placeholder="Wo bin ich ins Stocken geraten oder was ist noch unklar?"></textarea>
        </label>

        <label>Was hat mir geholfen?
          <textarea id="jHelpful" rows="4" placeholder="Welche Methode, Person, Erklärung, Ressource oder Strategie hat mich weitergebracht?"></textarea>
        </label>

        <label>Mein nächster Lernschritt
          <textarea id="jNextStep" rows="4" placeholder="Was möchte oder muss ich als Nächstes tun?"></textarea>
        </label>
    ...
    <section class="card journal-tile">
      <div class="journal-tile-head">
        <div>
          ...
          <p>Alle bisherigen Einträge an einem Ort.</p>
        </div>
        ${data.length?`<button type="button" class="secondary" onclick="printMyJournals()">📚 Alle als PDF</button>`:""}
      </div>
      ...
      <div class="journal-library">
        ...
        ${rows||`<div class="journal-empty"><strong>Noch kein Lernjournal vorhanden.</strong><br>Erstelle links deinen ersten Eintrag.</div>`}
      </div>
    </section>
        ...
        ? student.entries.map(j=>`
          <article class="entry">
            ...
            <div class="status-line">
              ${j.mood?`<span>${escPDF(j.mood)} Befinden</span>`:""}
              ${j.satisfaction?`<span>${escPDF(j.satisfaction)} Zufriedenheit</span>`:""}
            </div>
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .field p{margin:0}
  .status-line{display:flex;gap:12px;flex-wrap:wrap;color:#666;font-size:12px;margin:8px 0 16px}
  .field{margin:14px 0 0}
  .field h3{font-size:13px;margin:0 0 5px;color:#444}
  .field p{margin:0}
  .empty{color:#777}
...
function openJournalEntry(id){
  getMyJournalEntries().then(entries=>{
    ...
    modal(`
      ...
      ${journalDetailBlock("Woran habe ich heute gearbeitet?",j.workedOn)}
      ${journalDetailBlock("Was habe ich verstanden oder gelernt?",j.learned)}
      ${journalDetailBlock("Was war schwierig?",j.difficult)}
      ${journalDetailBlock("Was hat mir geholfen?",j.helpful)}
      ${journalDetailBlock("Mein nächster Lernschritt",j.nextStep)}
...
function journalDetailBlock(title,text){
  if(!text)return "";
  return `<div class="card" style="margin-top:12px">
    <strong>${esc(title)}</strong>
    <p style="white-space:pre-wrap;margin-bottom:0">${esc(text)}</p>
  </div>`;
}
