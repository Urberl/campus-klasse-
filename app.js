...
function showApp(){
  ...
  Promise.resolve(render()).catch(e=>{
    ...
    if(content) content.innerHTML=`<div class="card"><h2>Willkommen auf dem Campus.</h2><p>Die Anmeldung war erfolgreich. Die Startseite konnte gerade nicht vollständig geladen werden.</p><button class="primary" onclick="location.hash='#start';location.reload()">Campus-Startseite laden</button></div>`;
  });
}
...
async function render(){
  ...
  try{
    ...
    $("content").innerHTML=`<div class="card"><h2>Willkommen auf dem Campus.</h2><p>Die Anmeldung ist erfolgreich. Ein Teil der Campus-Daten konnte gerade nicht geladen werden.</p><div class="form-actions"><button class="primary" onclick="go('start')">← Campus-Startseite</button><button class="secondary" onclick="location.reload()">Neu laden</button></div></div>`;
  }
...
async function init(){
  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      // Die Firebase-Anmeldung ist gültig. Firestore darf den Zugang NICHT blockieren.
      profile={
        uid:user.uid,
        email:user.email||"",
      ...
      try{
        ...
        console.warn("Firestore-Profil nicht verfügbar – App wird trotzdem geöffnet.",e);
      }
