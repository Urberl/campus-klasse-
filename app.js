...
async function renderJournal(){
  ...
  try{
    ...
    console.error("Eigene Lernjournale konnten nicht geladen werden:",e);
    // Fallback ohne orderBy: funktioniert auch dann, wenn der Firebase-Index
    // für journal/uid/createdAt noch nicht angelegt wurde.
    try{
      const snap=await getDocs(
        query(collection(db,"journal"),where("uid","==",currentUser.uid))
      );
      data=snap.docs.map(d=>({id:d.id,...d.data()}));
      data.sort((a,b)=>{
      ...
      console.error("Fallback für Lernjournale fehlgeschlagen:",inner);
    }
  }
  ...
  <style>
    ...
    @media(max-width:520px){
      .journal-library-head,
      .journal-library-row{
        grid-template-columns:82px minmax(0,1fr) 55px;
        gap:7px;
        padding:10px 8px;
      }
...
async function getMyJournalEntries(){
  try{
    ...
    console.warn("Journal-Abfrage ohne Index:",e);
    const snap=await getDocs(
      query(collection(db,"journal"),where("uid","==",currentUser.uid))
    );
