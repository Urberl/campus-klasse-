...
const lernpfadStrategies={
  ...
  pause:{icon:"⏸️",label:"Pause machen"},
  erklaeren:{icon:"🗣️",label:"Anderen erklären"},
...
function lernpfadStrategyLinkHTML(strategy){
  const map={
    ...
    pause:["fokus-timer","⏱️ Zum Fokus-Timer"],
...
async function renderLernpfadRoute(){
  ...
  return `${pageHead("SELBSTSTÄNDIG LERNEN","Persönlicher Lernpfad",
    "Finde selbst heraus, was du gerade brauchst, um den nächsten Schritt im Verstehen zu machen.",
    `<button class="primary" onclick="openLernpfadCheckinForm()">＋ Neuer Lern-Check-in</button>`)}
...
class="list-item"><div><strong>${esc(p.title||p.text)}</strong>${p.title?`<small>${esc(p.text)} · ${fmtDate(p.createdAt)}</small>`:`<small>${fmtDate(p.createdAt)}</small>`}</div><div style="display:flex;align-items:center;gap:8px"><span class="pill">Info</span>${isAdmin()?`<button class="secondary" onclick="deleteNews('${p.id}')">Löschen</button>`:""}</div>
  ...
  <div class="card"><h3>  Nächster Termin</h3><div class="list">${nextCalendar?`<div class="list-item"><div><strong>${esc(nextCalendar.title||nextCalendar.name||"Termin")}</strong><small>${esc(upcomingDateText)}${upcomingTime}</small></div><span class="pill green">Termin</span></div>`:`<div class="empty">Noch keine anstehenden Termine.</div>`}</div></div>
  <div class="card" style="${birthdayInfo?.isToday?"background:var(--soft-pink,#fad4e7)":""}"><h3>🎂 Geburtstage</h3>${
...
Aufgaben</strong>Lege deine erste Aufgabe an.</div>`}</div></div>
  <div class="card" style="margin-top:12px"><h3>📅 Meine Projekt-Fristen</h3><p style="color:var(--muted);font-size:12px;margin-top:-4px">Dein persönlicher Überblick über Projekt-Abgabetermine – erscheint bewusst nicht im allgemeinen Campus-Kalender.</p><div class="list">${projectDeadlines.map(p=>{
...
async function renderLernwerkstatt(){
  const groups=[
    {title:"🧭 Dich selbst einschätzen",items:[
    ...
    {title:"🛠️ Konkret lernen & üben",items:[
    ...
    {title:"🤝 Unterstützung holen",items:[
  ...
  ${groups.map(g=>`<div class="kicker" style="margin:22px 0 10px">${g.title}</div><div class="grid grid-4">${g.items.map(x=>`<a class="card tile" href="#${x[3]}"><span class="emoji">${x[0]}</span>
...
const LERNMETHODEN=[
  ...
  {icon:"🗣️",name:"Feynman-Technik",tag:"Lernen",text:"Erkläre ein Thema in ganz einfachen Worten, als würdest du es einem Kind beibringen. Überall, wo du ins Stocken gerätst, hast du eine Lücke gefunden – genau da nochmal nachlesen."},
...
const META_PHASES={
  ...
  ueberwachung:{label:"Überwachung",icon:"👁️",short:"Während des Lernens: Verständnis prüfen, dranbleiben."},
...
const LIST_CATEGORIES={
  ...
  ressourcen:{label:"Ressourcenbezogene Strategien",icon:"⚙️",desc:"Wie du Zeit, Umgebung und Unterstützung nutzt.",
...
const PROK_GRAWE={
  "😰 Angst zu versagen":["Selbstwerterhöhung & -schutz","Die Aufgabe fühlt sich wie ein Test an, bei dem du „nicht gut genug“ sein könntest."],
  "😴 Langeweile":["Lustgewinn & Unlustvermeidung","Die Aufgabe fühlt sich unangenehm an – etwas anderes verspricht gerade mehr Vergnügen."],
  "🌊 Überforderung":["Orientierung & Kontrolle","Die Aufgabe wirkt zu groß oder unübersichtlich – das Gefühl, sie nicht im Griff zu haben."],
  "❓ Unklar, wie ich anfange":["Orientierung & Kontrolle","Ohne einen klaren ersten Schritt fehlt die Orientierung, wo überhaupt loslegen."],
  "👀 Gefühl, verglichen zu werden":["Bindung & Selbstwerterhöhung","Sorge, vor anderen schlecht dazustehen oder abgelehnt zu werden."],
  "😔 Frust/Ärger über mich selbst":["Selbstwerterhöhung & -schutz","Unzufriedenheit mit dir selbst erhöht meist den Druck, statt beim Anfangen zu helfen."],
  "😩 Erschöpfung, keine Energie":["Lustgewinn & Unlustvermeidung","Kopf oder Körper sind gerade leer – dadurch wirkt die Aufgabe anstrengender, als sie eigentlich ist."],
  "😳 Schamgefühl":["Bindung & Selbstwerterhöhung","Sorge, dass andere schlecht über dich denken könnten, wenn sie sähen, woran du gerade scheiterst."]
};
const PROK_ZIELE={
  "😌 Ruhe":["Die Aufgabe in einen klar begrenzten Zeitabschnitt packen – z. B. nur 15 Minuten, danach bewusst Pause.","Vor dem Start kurz durchatmen oder eine entspannte Körperhaltung einnehmen.","Dir bewusst eine ruhige Grundstimmung holen, bevor du anfängst – z. B. kurz rausgehen oder Musik, die dich runterbringt.","Dir sagen: „Ich muss nicht alles auf einmal schaffen, nur den nächsten Schritt.“"],
  "🙂 Leichtigkeit":["Die Aufgabe in einen winzigen ersten Schritt zerlegen, der nichts mit „fertig werden“ zu tun hat – z. B. nur die Überschrift tippen.","Dir bewusst machen: Gerade zählt nur der erste Schritt, nicht das ganze Ergebnis.","Dir direkt nach dem ersten Schritt eine kleine Belohnung gönnen (kurze Pause, etwas Schönes).","Dir einen angenehmen Lernort oder bequeme Kleidung gönnen, statt es dir schwerer zu machen als nötig."],
  "🔍 Neugier":["Dich fragen: „Was könnte ich hier Neues lernen oder entdecken?“ statt ans Endergebnis zu denken.","Mit dem Teil anfangen, der dich am meisten interessiert – nicht zwingend mit Punkt eins.","Dir eine Frage stellen, auf die du selbst noch keine Antwort weißt, und die Aufgabe als Weg dorthin sehen.","Dir kurz vorstellen, wofür du das Wissen später gebrauchen könntest."],
  "💪 Zuversicht":["Dich an eine Aufgabe erinnern, die dir früher auch schwer vorkam und am Ende doch geklappt hat.","Dir ein realistisches statt perfektes Ziel setzen – „gut genug“ statt „perfekt“.","Dir bewusst machen, welche Fähigkeiten du für diese Aufgabe eigentlich schon mitbringst.","Dir kurzes, konkretes Feedback von jemandem holen, um zu sehen, dass du auf einem guten Weg bist."],
  "⭐ Stolz aufs Anfangen":["Bewusst wahrnehmen, dass du angefangen hast – nicht erst feiern, wenn alles fertig ist.","Jemandem kurz Bescheid geben, dass du jetzt anfängst – das erhöht die Verbindlichkeit ein bisschen.","Nach jedem kleinen Fortschritt kurz innehalten und dir selbst anerkennend „gut gemacht“ sagen.","Fortschritte sichtbar machen (z. B. abhaken, Liste führen), statt sie einfach verstreichen zu lassen."],
  "🤗 Verbundenheit":["Mit jemandem zusammen lernen oder zumindest parallel arbeiten – auch per Videocall.","Dir aufschreiben oder vorstellen, für wen oder wofür du das eigentlich machst.","Jemandem kurz von deinem Vorhaben erzählen, bevor du anfängst.","Nach dem Lernen kurz mit jemandem teilen, was du geschafft hast."],
  "😊 Freude":["Dir während des Lernens etwas gönnen, das dir Spaß macht (Musik, Lieblingsgetränk).","Dir bewusst machen, was dich an dem Thema eigentlich interessiert oder wofür es gut ist.","Die Aufgabe spielerischer angeben – z. B. dir selbst ein kleines Zeitrennen stellen.","Dich nach dem Lernen mit etwas belohnen, auf das du dich schon vorher freust."]
};
...
async function downloadLernstandResultPDF(id){
  try{
    ...
    openToolPrintWindow(
      "Lernstandsmessung – "+(t.title||"Thema"),
      `<div class="item" style="background:#f5f7f8"><strong>${escPDF(scoreLine)}</strong></div>`+body,
      "CampusKlasse · Lernstandsmessung "+t.nr+"/26 · "+(LERNSTAND_AREAS[t.learningArea]?.title||"")
    );
...
async function downloadLernstandTeacherPDF(attemptId){
  try{
    ...
    openToolPrintWindow(
      "Bewertungsbericht – "+(t.title||"Thema"),
      `<div class="item" style="background:#f5f7f8"><strong>${escPDF(a.displayName||"Schüler/in")} · Versuch ${a.attempt}/3</strong><br>${escPDF(scoreLine)}${a.feedback?`<br><em>Rückmeldung:</em> ${escPDF(a.feedback)}`:""}</div>`+body,
      "CampusKlasse · Lernstandsmessung "+t.nr+"/26 · "+(LERNSTAND_AREAS[t.learningArea]?.title||"")
    );
