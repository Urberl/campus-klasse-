...
const JOURNAL_WRITING_STYLE=`<style id="journal-writing-style">
.journal-long-text{min-height:320px!important;resize:vertical;line-height:1.65!important;padding:14px!important;white-space:pre-wrap}
.journal-writing-hint{margin-top:10px;padding:12px 14px;border-radius:10px;background:#f4f7f5;color:#68736f;font-size:12px;line-height:1.5}
.journal-writing-hint strong{display:block;color:#476477;margin-bottom:3px}
.field-label{display:block;margin:14px 0 6px;font-size:12px;font-weight:800;color:#476477}
</style>`;
const JOURNAL_LIST_STYLE=`<style id="journal-list-style">
.journal-create-card{margin-top:16px;padding:24px;border:1px solid #dfe5e2;border-radius:18px;background:#fff}
.journal-create-card h2{margin:6px 0}.journal-create-card p{margin:0 0 16px;color:#68736f}
.journal-library-card{margin-top:18px;padding:24px;border:1px solid #dfe5e2;border-radius:18px;background:#fff}
.journal-library-toolbar{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin-bottom:16px}
.journal-library-toolbar h2{margin:6px 0 3px}.journal-library-toolbar p{margin:0;color:#68736f;font-size:12px}
.journal-list{width:100%;border:1px solid #dfe5e2;border-radius:12px;overflow:hidden;background:#fff}
.journal-list-header,.journal-list-item{display:grid;grid-template-columns:145px minmax(0,1fr) 90px;align-items:center;gap:14px}
.journal-list-header{padding:11px 15px;background:#f3f6f4;color:#68736f;font-size:10px;font-weight:900;letter-spacing:.08em}
.journal-list-item{min-height:58px;padding:8px 10px 8px 15px;border-top:1px solid #e7ebe9}
.journal-list-open{width:100%;display:grid;grid-template-columns:145px minmax(0,1fr);align-items:center;gap:14px;border:0;background:transparent;padding:7px 0;text-align:left;cursor:pointer;font:inherit;color:inherit}
.journal-list-open:hover{background:#f8fbfd}.journal-list-open:hover .journal-list-title{text-decoration:underline}
.journal-list-date{font-size:12px;color:#68736f;white-space:nowrap}.journal-list-title{font-size:13px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.journal-list-pdf{justify-self:end;white-space:nowrap}.journal-empty{padding:30px 18px;text-align:center;color:#68736f}
@media(max-width:700px){.journal-library-toolbar{align-items:flex-start;flex-direction:column}.journal-list-header,.journal-list-item{grid-template-columns:88px minmax(0,1fr) 64px;gap:8px}.journal-list-open{grid-template-columns:1fr;gap:2px}.journal-list-pdf{padding:7px 8px;font-size:12px}}
</style>`;
...
async function renderJournal(){
  let entries=[];
  try{entries=await getMyJournalEntries()}catch(e){console.error("Lernjournal laden:",e)}
  const teacherButton=isTeacher()?`<button class="secondary" onclick="openTeacherJournalOverview()">👩‍🏫 Schüler-Lernjournale</button>`:"";
  const rows=entries.map(entry=>`<div class="journal-list-item">
    <button class="journal-list-open" onclick="openJournalEntry('${entry.id}')"><span class="journal-list-date">${esc(fmtDate(entry.createdAt))}</span><span class="journal-list-title">${esc(entry.title||"Reflexion")}</span></button>
    <button class="secondary journal-list-pdf" onclick="printSingleJournalPDF('${entry.id}')">📄 PDF</button>
  </div>`).join("");
  return `${JOURNAL_WRITING_STYLE}${JOURNAL_LIST_STYLE}${pageHead("REFLEXION","Lernjournal","Dein Lernweg, Reflexionen und nächste Schritte.",teacherButton)}
  <div class="journal-create-card"><div class="kicker">✍️ LERNJOURNAL</div><h2>Neuen Lernjournal-Eintrag erstellen</h2><p>Halte deine Lernerfahrungen ausführlich fest.</p><button class="primary" onclick="openJournalForm()">＋ Lernjournal-Eintrag erstellen</button></div>
  <div class="journal-library-card" id="journalLibraryPanel"><div class="journal-library-toolbar"><div><span class="badge">📚 LERNJOURNAL-BIBLIOTHEK</span><h2>Meine Einträge</h2><p>${entries.length} gespeicherte Einträge</p></div>${entries.length?`<button class="secondary" onclick="printMyJournalPDF()">📄 Alle als PDF</button>`:""}</div>
  ${entries.length?`<div class="journal-list"><div class="journal-list-header"><span>DATUM</span><span>TITEL</span><span>PDF</span></div>${rows}</div>`:`<div class="journal-empty"><strong>Noch keine Lernjournal-Einträge</strong><div>Erstelle deinen ersten Eintrag.</div></div>`}</div>${footer()}`;
}
...
async function openJournalEntry(id){
  try{
    ...
    if(!entry){toast("Der Eintrag wurde nicht gefunden.");return}
    modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">LERNJOURNAL · ${esc(fmtDate(entry.createdAt))}</div><h2>${esc(entry.title||"Reflexion")}</h2>${entry.mood?`<div style="font-size:26px;margin:8px 0">${esc(entry.mood)}</div>`:""}<div class="card" style="margin-top:14px"><div style="line-height:1.7">${esc(entry.text||"—").replace(/\n/g,"<br>")}</div></div><div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button><button class="secondary" onclick="printSingleJournalPDF('${entry.id}')">📄 PDF</button></div>`);
...
function openJournalForm(){
  modal(`
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">✍️ LERNJOURNAL</div>
    <h2>Meine Lernreflexion</h2>
    <p class="muted">Nimm dir Zeit für eine ausführliche Reflexion. Dein Eintrag wird gespeichert und später in deiner Lernjournal-Bibliothek angezeigt.</p>

    <label class="field-label" for="jTitle">Titel</label>
    <input id="jTitle" class="input" placeholder="z. B. Was habe ich heute gelernt?">

    <label class="field-label" for="jMood">Wie geht es mir gerade?</label>
    <select id="jMood" class="input">
      <option value="">Bitte auswählen</option>
      <option value="😊 Sehr gut">😊 Sehr gut</option>
      <option value="🙂 Gut">🙂 Gut</option>
      <option value="😐 Ganz okay">😐 Ganz okay</option>
      <option value="😕 Eher schwierig">😕 Eher schwierig</option>
      <option value="😔 Schwierig">😔 Schwierig</option>
    </select>

    <label class="field-label" for="jText">Meine ausführliche Reflexion</label>
    <textarea id="jText" class="input journal-long-text" rows="14" placeholder="Was habe ich heute gelernt?

Was ist mir besonders gut gelungen?

Was war schwierig oder hat mich beschäftigt?

Was möchte ich noch besser verstehen?

Welche Erkenntnis nehme ich mit?

Was ist mein nächster Schritt?

Du kannst hier ausführlich schreiben – nicht nur Stichpunkte."></textarea>

    <div class="journal-writing-hint">
      <strong>💡 Denk an deinen Lernweg</strong>
      <span>Beschreibe nicht nur, was du gemacht hast, sondern auch, wie du gelernt hast, was du daraus mitnimmst und was du als Nächstes tun möchtest.</span>
    </div>

    <div class="form-actions">
      <button class="secondary" onclick="closeModal()">Abbrechen</button>
      <button class="primary" onclick="addJournal()">💾 Lernjournal speichern</button>
    </div>
  `);
}
