...
<strong>${t[1]}</strong><small>${t[2]}</small>${!t[4]?`<span class="badge" style="margin-top:8px">IN VORBEREITUNG</span>`:""}</a>`).join("")}</div>
  ${footer()}`;
}
...
function reportTargetLabel(col){
  return {posts:"Forum-Beitrag",boardPosts:"Pinnwand-Notiz",teamAds:"Team-gesucht-Gesuch"}[col]||col;
}
function reportTargetRoute(col){
  return {posts:"forum-board",boardPosts:"pinnwand",teamAds:"teamgesucht"}[col]||"start";
}
  ...
  return `<div class="card glossary-entry" data-search="${searchKey}" style="margin-bottom:10px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
      ...
      ${canEdit?`<div style="display:flex;gap:6px;flex-shrink:0">
        <button class="secondary" onclick="editGlossaryForm('${g.id}','${esc(g.term||"")}','${esc(g.definition||"")}')">Bearbeiten</button>
