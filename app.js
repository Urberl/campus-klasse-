...
async function renderLernjournalRoute(){
  try{
    const module=await import("./modules/lernjournal.js");
    return await module.renderLernjournal();
  }catch(error){
    console.error("Lernjournal-Modul konnte nicht geladen werden:",error);
    return moduleError(" Lernjournal","modules/lernjournal.js",error);
  }
}
...
async function renderJournal(){
  ...
  return `${pageHead(
    ...
    `${teacherButton}<button class="primary" onclick="openJournalForm()">＋ Eintrag</button>`
  )}
  ${isTeacher()?`
    <div class="card" style="margin-bottom:12px;background:var(--soft-green)">
      <span class="badge">👩‍🏫 LEHRKRAFTBEREICH</span>
      <h2>Lernjournale der Schüler</h2>
      <p>Als Lehrkraft kannst du die Lernjournale der Schüler ansehen und für einzelne Schüler als PDF ausgeben.</p>
      <button class="secondary" onclick="openTeacherJournalOverview()">Lernjournale öffnen →</button>
    </div>`:""}
  <div class="list">${data.map(j=>`<article class="card">
    <div style="display:flex;justify-content:space-between">
      <div><span class="pill">${fmtDate(j.createdAt)}</span><h3 style="margin-top:9px">${esc(j.title)}</h3></div>
      <span style="font-size:25px">${esc(j.mood||"💭")}</span>
    </div>
    <p>${esc(j.text)}</p>
  </article>`).join("")||`<div class="empty"><strong>Noch kein Eintrag</strong>Starte mit einer kurzen Reflexion.</div>`}</div>${footer()}`;
}
    ...
    <section class="student-section">
      ...
      ${student.entries.length
        ? student.entries.map(j=>`
          <article class="entry">
            <div class="date">${escPDF(journalPDFDate(j.createdAt))}</div>
            <h2>${escPDF(j.title||"Reflexion")}</h2>
            ${j.mood?`<div class="mood">${escPDF(j.mood)}</div>`:""}
            <p>${escPDF(j.text||"—").replace(/\n/g,"<br>")}</p>
          </article>
        `).join("")
...
async function renderPraktikum(){
  let practice=[], impressions=[], questions=[], projects=[], cloudEntries=[];
  try{practice=await getCollection("practice","createdAt",true)}catch(e){console.warn(e)}
  try{impressions=await getCollection("practiceImpressions","createdAt",true)}catch(e){console.warn(e)}
  try{const posts=await getCollection("posts","createdAt",true);questions=posts.filter(p=>p.type==="question").slice(0,6)}catch(e){console.warn(e)}
  try{projects=await getCollection("projects","createdAt",true)}catch(e){console.warn(e)}
  try{cloudEntries=await getCollection("practiceWordCloud","createdAt",true)}catch(e){console.warn(e)}

  const mine=impressions.filter(i=>i.uid===currentUser.uid);
  const counts={};
  cloudEntries.forEach(e=>String(e.words||"").split(/[,\n;]+/).map(x=>x.trim().toLowerCase()).filter(x=>x.length>=3&&x.length<=28).forEach(x=>counts[x]=(counts[x]||0)+1));
  const words=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,40), max=Math.max(1,...words.map(x=>x[1]));

  const practiceCards=practice.map(p=>`
    <div class="card">
      <span class="pill ${p.state==="offen"?"orange":"green"}">${esc(p.state||"geplant")}</span>
      <h3>${esc(p.title||"Praxisauftrag")}</h3><p>${esc(p.text||"")}</p>
      <small>${esc(p.date||"")}</small>
      <div class="form-actions"><button class="primary" onclick="openPracticeImpressionForm('${esc(p.id)}')">＋ Mein Eindruck</button>
      ${mine.filter(i=>i.practiceId===p.id).length?`<span class="pill green">eigener Eintrag vorhanden</span>`:""}</div>
    </div>`).join("");

  const impressionRows=mine.map(i=>`
    <div class="list-item"><div><strong>${esc(i.practiceTitle||"Praxisauftrag")}</strong><small>${esc(i.date||"")} · ${esc(i.title||"Mein Eindruck")}</small></div>
    <button class="secondary" onclick="openPracticeImpression('${esc(i.id)}')">Öffnen</button></div>`).join("");

  const questionRows=questions.map(q=>`
    <div class="list-item"><div><strong>${esc(q.authorName||"Schüler/in")}</strong><small>${esc(q.text||"")}</small></div><span class="pill">Frage</span></div>`).join("");

  const projectRows=projects.slice(0,6).map(p=>`
    <div class="list-item"><div><strong>${esc(p.title||"Projekt")}</strong><small>${esc(p.team||"")} · Praxispartner: ${esc(p.partner||"—")}</small></div><span class="pill">${Number(p.progress||0)}%</span></div>`).join("");

  const cloud=words.length?words.map(([w,n])=>`<span class="fpa-cloud-word" style="font-size:${16+Math.round(n/max*24)}px">${esc(w)}</span>`).join(""):`<div class="empty">Noch keine Begriffe gesammelt.</div>`;

  return `${pageHead("SCHULE ↔ PRAXIS","fpA","Praxisaufträge, Eindrücke, Fragen, Projekte und KI-Innovationspartnerschaften an einem Ort.",`<button class="primary" onclick="openPracticeForm()">＋ Praxisauftrag</button>`)}
  <style>
    .fpa-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}.fpa-tile{min-height:180px;cursor:pointer}.fpa-tile .emoji{font-size:30px}.fpa-tile h2{margin:8px 0 5px}.fpa-tile p{margin:0;color:var(--muted)}.fpa-section{margin-top:16px}.fpa-cloud{min-height:230px;display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:10px 16px;padding:22px;border-radius:14px;background:var(--soft,#f7f7f5)}.fpa-cloud-word{font-weight:700}.fpa-downloads{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.fpa-download-card{display:flex;align-items:center;gap:12px;padding:15px;border:1px solid var(--line,#ddd);border-radius:12px;background:#fff}@media(max-width:900px){.fpa-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:600px){.fpa-grid,.fpa-downloads{grid-template-columns:1fr}}
  </style>
  <div class="fpa-grid">
    <section class="card fpa-tile" onclick="document.getElementById('fpaPraxis').scrollIntoView({behavior:'smooth'})"><span class="emoji">🧭</span><h2>Praxisaufträge</h2><p>Aufträge bearbeiten und eigene Eindrücke speichern.</p></section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaFragen').scrollIntoView({behavior:'smooth'})"><span class="emoji">❓</span><h2>Fragen</h2><p>Fragen aus dem Praktikum sammeln und teilen.</p></section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaProjekte').scrollIntoView({behavior:'smooth'})"><span class="emoji">🚀</span><h2>Projekte</h2><p>Praxisbeobachtungen zu Projektideen entwickeln.</p></section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaDownloads').scrollIntoView({behavior:'smooth'})"><span class="emoji">📥</span><h2>Formulare & Downloads</h2><p>Bewertungsbogen und Wochenbericht.</p></section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaCloud').scrollIntoView({behavior:'smooth'})"><span class="emoji">☁️</span><h2>Praxis-Eindrücke</h2><p>Gemeinsame Wortwolke aus euren Erfahrungen.</p></section>
    <section class="card fpa-tile" onclick="location.hash='ki'"><span class="emoji">🤖</span><h2>KI-Innovationspartnerschaften</h2><p>KI-Ideen mit Praxispartnern entwickeln und erproben.</p></section>
  </div>

  <section id="fpaPraxis" class="card fpa-section"><div class="page-head"><div><div class="kicker">PRAXISAUFTRÄGE</div><h2>Praxisaufträge & meine Eindrücke</h2><p>Zu jedem Auftrag persönliche Beobachtungen speichern.</p></div></div><div class="grid grid-2">${practiceCards||`<div class="empty">Noch keine Praxisaufträge vorhanden.</div>`}</div><div class="card"><h3>📚 Meine Praxis-Eindrücke</h3><div class="list">${impressionRows||`<div class="empty">Noch keinen Praxiseindruck gespeichert.</div>`}</div></div></section>

  <section id="fpaCloud" class="card fpa-section"><div class="page-head"><div><div class="kicker">REFLEXION</div><h2>☁️ Was nehme ich aus der Praxis mit?</h2><p>Häufig genannte Begriffe erscheinen größer.</p></div><button class="primary" onclick="openPracticeWordCloudForm()">＋ Eindruck hinzufügen</button></div><div class="fpa-cloud">${cloud}</div></section>

  <section id="fpaDownloads" class="card fpa-section"><div class="page-head"><div><div class="kicker">UNTERLAGEN</div><h2>📥 Formulare & Downloads</h2><p>Die zentralen Dokumente für die fpA.</p></div></div><div class="fpa-downloads">
    <div class="fpa-download-card"><span style="font-size:26px">📄</span><div style="flex:1"><strong>Bewertungsbogen des Praktikumsbetriebes</strong><small>Vorgegebenes Formular</small></div><button class="secondary" onclick="downloadFpaDocument('bewertung')">Download</button></div>
    <div class="fpa-download-card"><span style="font-size:26px">📋</span><div style="flex:1"><strong>Wochenbericht</strong><small>Vorgegebenes Formular</small></div><button class="secondary" onclick="downloadFpaDocument('wochenbericht')">Download</button></div>
  </div></section>

  <section id="fpaFragen" class="card fpa-section"><div class="page-head"><div><div class="kicker">FRAGEN</div><h2>Fragen aus der Praxis</h2></div><button class="primary" onclick="openPostForm('question')">＋ Frage stellen</button></div><div class="list">${questionRows||`<div class="empty">Noch keine Fragen vorhanden.</div>`}</div></section>

  <section id="fpaProjekte" class="card fpa-section"><div class="page-head"><div><div class="kicker">PROJEKTE</div><h2>Praxisprojekte</h2></div><button class="primary" onclick="openProjectForm()">＋ Projekt anlegen</button></div><div class="list">${projectRows||`<div class="empty">Noch keine Projekte vorhanden.</div>`}</div></section>

  <section class="card fpa-section"><div class="page-head"><div><div class="kicker">INNOVATION</div><h2>KI-Innovationspartnerschaften</h2><p>Praxisprobleme erkennen, KI-Ideen entwickeln und gemeinsam erproben.</p></div><button class="primary" onclick="location.hash='ki'">KI-Innovationslabor öffnen →</button></div></section>
  ${footer()}`;
}

function fpADisplayDate(v){if(!v)return"—";if(v?.seconds)return new Date(v.seconds*1000).toLocaleDateString("de-DE");return String(v)}

async function openPracticeWordCloudForm(){modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">PRAXIS · WORTWOLKE</div><h2>Was nehme ich aus der Praxis mit?</h2><div class="form"><label>Meine Begriffe<textarea id="pwcWords" rows="4" placeholder="z. B. Teamarbeit, Verantwortung, Kommunikation"></textarea></label><div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="savePracticeWordCloud()">Speichern</button></div></div>`)}

async function savePracticeWordCloud(){const words=$("pwcWords")?.value.trim()||"";if(!words){toast("Bitte mindestens einen Begriff eingeben.");return}try{await addDoc(collection(db,"practiceWordCloud"),{uid:currentUser.uid,studentName:profile?.displayName||currentUser?.email||"Schüler/in",words,createdAt:serverTimestamp()});closeModal();await render();toast("Deine Eindrücke wurden gespeichert.")}catch(e){console.error(e);toast("Der Beitrag konnte nicht gespeichert werden.")}}

function downloadFpaDocument(type){const documents={bewertung:"",wochenbericht:""};if(!documents[type]){toast("Für dieses Formular ist noch keine PDF-Datei hinterlegt.");return}window.open(documents[type],"_blank","noopener,noreferrer")}

function openPracticeImpressionForm(practiceId){getCollection("practice","createdAt",true).then(items=>{const p=items.find(x=>x.id===practiceId);if(!p){toast("Praxisauftrag nicht gefunden.");return}modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">PRAXIS · EINDRUCK</div><h2>${esc(p.title||"Praxisauftrag")}</h2><p>${esc(p.text||"")}</p><div class="form"><label>Titel<input id="piTitle" placeholder="Meine Beobachtung"></label><label>Datum<input id="piDate" type="date" value="${new Date().toISOString().slice(0,10)}"></label><label>Mein Eindruck<textarea id="piText" rows="6"></textarea></label><label>Was nehme ich mit?<textarea id="piNext" rows="4"></textarea></label><div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="addPracticeImpression('${esc(p.id)}','${esc(p.title||"Praxisauftrag")}')">Speichern</button></div></div>`)})}

async function addPracticeImpression(practiceId,practiceTitle){const body=$("piText")?.value.trim()||"";if(!body){toast("Bitte deinen Eindruck eintragen.");return}try{await addDoc(collection(db,"practiceImpressions"),{uid:currentUser.uid,studentName:profile?.displayName||currentUser?.email||"Schüler/in",practiceId,practiceTitle,title:$("piTitle")?.value.trim()||"Mein Eindruck",date:$("piDate")?.value||new Date().toISOString().slice(0,10),text:body,next:$("piNext")?.value.trim()||"",createdAt:serverTimestamp()});closeModal();await render();toast("Praxiseindruck gespeichert.")}catch(e){console.error(e);toast("Praxiseindruck konnte nicht gespeichert werden.")}}

async function openPracticeImpression(id){const items=await getCollection("practiceImpressions","createdAt",true);const i=items.find(x=>x.id===id&&x.uid===currentUser.uid);if(!i){toast("Eintrag nicht gefunden.");return}modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">PRAXIS-EINDRUCK</div><h2>${esc(i.title||"Mein Eindruck")}</h2><p><strong>Praxisauftrag:</strong> ${esc(i.practiceTitle||"")}</p><div class="card"><strong>Mein Eindruck</strong><p style="white-space:pre-wrap">${esc(i.text||"")}</p></div>${i.next?`<div class="card"><strong>Was nehme ich mit?</strong><p style="white-space:pre-wrap">${esc(i.next)}</p></div>`:""}<div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button></div>`)}
...
window.openPracticeImpression=openPracticeImpression;
window.addPracticeImpression=addPracticeImpression;
window.openPracticeImpressionForm=openPracticeImpressionForm;
window.downloadFpaDocument=downloadFpaDocument;
window.savePracticeWordCloud=savePracticeWordCloud;
window.openPracticeWordCloudForm=openPracticeWordCloudForm;
...
<label>Reflexion<textarea id="jText" rows="5"></textarea></label><label>Stimmung<select id="jMood"><option>                </option><option>
