...
async function openJournalEntry(id){
  try{
    ...
    modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">LERNJOURNAL · ${esc(fmtDate(j.createdAt))}</div><h2>${esc(j.title||"Reflexion")}</h2>${j.mood?`<div style="font-size:26px;margin:8px 0">${esc(j.mood)}</div>`:""}<div class="journal-entry-full">${esc(j.text||"—").replace(/\n/g,"<br>")}</div><div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button><button class="secondary" onclick="printSingleJournalPDF('${j.id}')">📄 PDF</button></div>`);
  }catch(e){console.error("Lernjournal-Eintrag:",e);toast("Der Eintrag konnte nicht geöffnet werden.")}
}
