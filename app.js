...
const JOURNAL_UI_STYLE=`<style id="journal-ui">
.journal-actions-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;margin-top:16px}
...
.journal-action-card h2{margin:8px 0}.journal-action-card p{color:var(--muted);line-height:1.55}.journal-action-card .grow{flex:1}.journal-action-card .emoji{font-size:34px}
.journal-library-card{margin-top:18px}.journal-library-head{display:flex;justify-content:space-between;align-items:flex-end;gap:16px;margin-bottom:14px}
.journal-table{width:100%;border:1px solid var(--line,#dfe5e2);border-radius:12px;overflow:hidden}.journal-table-head,.journal-table-row{display:grid;grid-template-columns:140px minmax(0,1fr) 86px;align-items:center;gap:12px}
.journal-table-head{padding:10px 13px;background:#f3f6f4;color:var(--muted);font-size:10px;font-weight:900;letter-spacing:.08em}.journal-table-row{min-height:56px;padding:7px 10px 7px 13px;border-top:1px solid #e7ebe9}
...
.journal-date{font-size:12px;color:var(--muted);white-space:nowrap}.journal-title{font-size:13px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.journal-pdf{justify-self:end}.journal-empty{padding:28px;text-align:center;color:var(--muted)}
.journal-textarea{width:100%;min-height:420px;resize:vertical;line-height:1.65;padding:14px;box-sizing:border-box}.journal-hint{margin-top:10px;padding:12px 14px;background:#f3f6f4;border-radius:10px;color:var(--muted);font-size:12px;line-height:1.55}
@media(max-width:700px){.journal-actions-grid{grid-template-columns:1fr}.journal-table-head,.journal-table-row{grid-template-columns:82px minmax(0,1fr) 62px;gap:7px}.journal-open{grid-template-columns:1fr;gap:2px}.journal-pdf{padding:6px 7px;font-size:11px}}
</style>`;
const JOURNAL_EXTENDED_CSS=`<style id="journal-extended-css">
.journal-actions-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;margin-top:16px}
.journal-action-card{padding:24px;border:1px solid var(--line,#dfe5e2);border-radius:16px;background:#fff;display:flex;flex-direction:column;min-height:220px}
.journal-action-card h2{margin:8px 0}.journal-action-card p{color:var(--muted);line-height:1.55}.journal-action-card .grow{flex:1}.journal-action-card .emoji{font-size:34px}
.journal-library-card{margin-top:18px}.journal-library-head{display:flex;justify-content:space-between;align-items:flex-end;gap:16px;margin-bottom:14px}
...
.journal-table-row{min-height:56px;padding:7px 10px 7px 13px;border-top:1px solid #e7ebe9}
.journal-open{width:100%;border:0;background:transparent;text-align:left;display:grid;grid-template-columns:140px minmax(0,1fr);gap:12px;align-items:center;padding:7px 0;cursor:pointer;font:inherit}
.journal-date{font-size:12px;color:var(--muted);white-space:nowrap}.journal-title{font-size:13px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.journal-pdf{justify-self:end}
.journal-empty{padding:28px;text-align:center;color:var(--muted)}
.journal-textarea{width:100%;min-height:460px;height:460px;resize:vertical;line-height:1.7;padding:15px;box-sizing:border-box;white-space:pre-wrap}
.journal-hint{margin-top:10px;padding:13px 15px;background:#f3f6f4;border-radius:10px;color:var(--muted);font-size:12px;line-height:1.6}
@media(max-width:700px){.journal-actions-grid{grid-template-columns:1fr}.journal-table-head,.journal-table-row{grid-template-columns:82px minmax(0,1fr) 62px;gap:7px}.journal-open{grid-template-columns:1fr;gap:2px}.journal-pdf{padding:6px 7px;font-size:11px}}
</style>`;
...
<div class="card journal-library-card" id="journalLibraryPanel">
  <div class="journal-library-head">
    <div>
      ...
      <h2>Meine Lernjournal-Einträge</h2>
      <p>${entries.length} gespeicherte Einträge</p>
    </div>
  ...
  ${loadError
    ...
    : entries.length
      ? `<div class="journal-table">
          <div class="journal-table-head"><span>DATUM</span><span>TITEL</span><span>PDF</span></div>
          ${rows}
        </div>`
...
function openJournalForm(){
  modal(`
    ...
    <div class="form">
      <label>Titel
        <input id="jTitle" placeholder="z. B. Meine Erkenntnisse aus dieser Woche">
      </label>

      <label>Ausführliche Reflexion
        <textarea id="jText" class="journal-textarea" rows="18" placeholder="Was habe ich gelernt?
...
Schreibe gerne ausführlich und in mehreren Absätzen."></textarea>
      </label>
      ...
      <div class="journal-hint">
        <b>💡 Dein Lernjournal</b><br>
        Beschreibe nicht nur, was du gemacht hast. Halte auch Gedanken,
        Schwierigkeiten, Erfolge, Erkenntnisse und nächste Schritte fest.
      </div>

      <div class="form-actions">
        <button class="secondary" onclick="window.closeModal()">Abbrechen</button>
        <button class="primary" onclick="window.addJournal()">💾 Speichern</button>
      </div>
    </div>
  `);
}
async function addJournal(){
  try{
    const title=$("jTitle").value.trim()||"Reflexion";
...
function journalPrint(entries,title){
  const body=entries.map(j=>`<article style="margin-bottom:28px;padding-bottom:18px;border-bottom:1px solid #ddd"><h2>${esc(j.title||"Reflexion")}</h2><div style="color:#666;font-size:12px;margin-bottom:10px">${esc(fmtDate(j.createdAt))}</div><div style="line-height:1.7">${esc(j.text||"—").replace(/\n/g,"<br>")}</div></article>`).join("");
  const w=window.open("","_blank"); if(!w){toast("Druckfenster wurde blockiert.");return}
  w.document.write(`<!doctype html><html lang="de"><head><meta charset="utf-8"><title>${esc(title)}</title></head><body>${body}</body></html>`);
  w.document.close();w.focus();setTimeout(()=>w.print(),300);
}
async function printSingleJournalPDF(id){const j=(await getMyJournalEntries()).find(x=>x.id===id);if(j)journalPrint([j],"Lernjournal")}
async function printMyJournalPDF(){journalPrint(await getMyJournalEntries(),"Meine Lernjournal-Bibliothek")}
async function init(){
  if(!configReady){
    showAuth();
  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      clearListeners();
      cwindow.openJournalForm=openJournalForm;
