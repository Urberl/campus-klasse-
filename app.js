...
function showApp(){
  ...
  render().catch(e=>console.error("Startseite konnte nicht geladen werden:",e));
}
...
async function render(){
  ...
  try{
    ...
    content.innerHTML=`<div class="card"><h3>Willkommen auf dem Campus.</h3><p>Die Anmeldung war erfolgreich. Der gewünschte Bereich konnte gerade nicht geladen werden.</p><div class="form-actions"><button class="primary" onclick="window.go('start')">← Campus-Startseite</button><button class="secondary" onclick="location.reload()">Neu laden</button></div></div>`;
  }
  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      try{
        ...
        profile={uid:user.uid,email:user.email||"",displayName:user.displayName||user.email||"Campus-Mitglied",role:"student"};
      }
      showApp();
    });
  }catch(e){
