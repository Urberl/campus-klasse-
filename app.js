...
async function renderJournal(){
  ...
  const rows=entries.map(entry=>`
    <div class="journal-list-item">
      <button class="journal-list-open" onclick="openJournalEntry('${entry.id}')">
        <span class="journal-list-date">${esc(fmtDate(entry.createdAt))}</span>
        <span class="journal-list-title">${esc(entry.title||"Reflexion")}</span>
      </button>
      <button class="secondary journal-list-pdf" onclick="printSingleJournalPDF('${entry.id}')">📄 PDF</button>
    </div>`).join("");

  return `${JOURNAL_LIST_STYLE}
${pageHead("REFLEXION","Lernjournal","Dein Lernweg, Reflexionen und nächste Schritte.",teacherButton)}

<div class="journal-create-card">
  <div class="kicker">✍️ LERNJOURNAL</div>
  <h2>Neuen Lernjournal-Eintrag erstellen</h2>
  <p>Halte fest, was du gelernt hast, was dich beschäftigt und welche nächsten Schritte du gehen möchtest.</p>
  <button class="primary" onclick="openJournalForm()">＋ Lernjournal-Eintrag erstellen</button>
</div>

<div class="journal-library-card" id="journalLibraryPanel">
  <div class="journal-library-toolbar">
    <div>
      <span class="badge">📚 LERNJOURNAL-BIBLIOTHEK</span>
      <h2>Meine Einträge</h2>
      <p>${entries.length} gespeicherte${entries.length===1?"r":""} Einträge</p>
    </div>
    ${entries.length?`<button class="secondary" onclick="printMyJournalPDF()">📄 Alle als PDF</button>`:""}
  </div>

  ${entries.length
    ? `<div class="journal-list">
         <div class="journal-list-header"><span>DATUM</span><span>TITEL</span><span>PDF</span></div>
         ${rows}
       </div>`
    : `<div class="journal-empty"><strong>Noch keine Lernjournal-Einträge</strong><div>Erstelle deinen ersten Eintrag – er erscheint anschließend hier als Liste.</div></div>`
  }
</div>
${footer()}`;
}
...
async function openJournalEntry(id){
  try{
    ...
    modal(`<button class="modal-close" onclick="closeModal()">×</button>
      <div class="kicker">LERNJOURNAL · ${esc(fmtDate(entry.createdAt))}</div>
      <h2>${esc(entry.title||"Reflexion")}</h2>
      ${entry.mood?`<div style="font-size:26px;margin:8px 0">${esc(entry.mood)}</div>`:""}
      <div class="card" style="margin-top:14px"><div style="line-height:1.7">${esc(entry.text||"—").replace(/\n/g,"<br>")}</div></div>
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Schließen</button>
        <button class="secondary" onclick="printSingleJournalPDF('${entry.id}')">📄 PDF</button>
      </div>`);
  }catch(e){console.error("Lernjournal öffnen:",e);toast("Der Eintrag konnte nicht geöffnet werden.")}
}
