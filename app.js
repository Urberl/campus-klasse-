...
async function renderPraktikum(){
  ...
  return `${pageHead("SCHULE ↔ PRAXIS","fpA","Praxisaufträge, Beobachtungen und Reflexionen verbinden Schule und Praktikumsbetrieb.",`<button class="primary" onclick="openPracticeForm()">＋ Praxisauftrag</button>`)}
...
async function renderKI(){
  /*
   * KI-Innovationspartnerschaften sind ein eigenständiger Bereich.
   * Die drei Kacheln bilden einen klaren Prozess:
   * 1. Herausforderungen aus Betrieben
   * 2. Bearbeitung durch Schülerteams / Einzelschüler
   * 3. Ergebnisse, Ideen und Produkte
   *
   * Die Daten werden über eigene Collections getrennt von fpA-Praxisaufträgen,
   * Fragen, Projekten und Kompetenznetzwerk gespeichert.
   */
  ...
  const openChallenges=challenges.filter(c=>c.status!=="abgeschlossen");
  const activeSolutions=solutions.filter(s=>s.status!=="abgeschlossen");

  return `${pageHead(
    "KI-INNOVATIONSPARTNERSCHAFTEN",
    "KI-Innovationspartnerschaften",
    "Herausforderung aus der Praxis → Schülerteam → KI-gestützte Lösung → Ergebnis.",
    `<button class="primary" onclick="openKIChallengeForm()">＋ Herausforderung eintragen</button>`
  )}

  <style>
    .ki-flow{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}
    .ki-tile{min-height:250px;cursor:pointer;position:relative}
    .ki-number{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;background:var(--soft,#f1f3ef);margin-bottom:12px}
    .ki-tile h2{margin:4px 0 8px}
    .ki-tile p{color:var(--muted)}
    .ki-count{margin-top:18px}
    .ki-library{margin-top:18px}
    .ki-entry{padding:14px;border:1px solid var(--line,#ddd);border-radius:12px;background:#fff;margin-bottom:10px}
    .ki-entry-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}
    .ki-entry small{color:var(--muted)}
    .ki-meta{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}
    .ki-result{border-left:4px solid var(--accent,#7b9b82)}
    .ki-empty{padding:22px;text-align:center;color:var(--muted)}
    @media(max-width:850px){.ki-flow{grid-template-columns:1fr}}
  </style>

  <div class="ki-flow">

    <section class="card ki-tile" onclick="openKIChallengesLibrary()">
      <div class="ki-number">1</div>
      <h2>🏢 Herausforderungen in den Praktikumsbetrieben</h2>
      <p>
        Betriebe beschreiben konkrete Herausforderungen aus ihrem Arbeitsalltag,
        für die eine KI-gestützte Lösung interessant sein könnte.
      </p>
      <div class="ki-count"><span class="pill">${openChallenges.length} offene Herausforderungen</span></div>
    </section>

    <section class="card ki-tile" onclick="openKISolutionsLibrary()">
      <div class="ki-number">2</div>
      <h2>🧑‍🤝‍🧑 Schülerteam & Lösungsentwicklung</h2>
      <p>
        Einzelne Schüler oder Teams übernehmen eine Herausforderung und
        dokumentieren systematisch, wer was macht und wie die Lösung entsteht.
      </p>
      <div class="ki-count"><span class="pill">${activeSolutions.length} laufende Bearbeitungen</span></div>
    </section>

    <section class="card ki-tile" onclick="openKIResultsLibrary()">
      <div class="ki-number">3</div>
      <h2>💡 Ergebnisse, Ideen & Produkte</h2>
      <p>
        Entstandene Ideen, Prototypen, Konzepte, Prompts, Materialien und
        andere Ergebnisse werden als Bibliothek gesammelt.
      </p>
      <div class="ki-count"><span class="pill">${results.length} Ergebnisse</span></div>
    </section>

  </div>

  <div class="card ki-library">
    <div class="page-head">
      <div>
        <div class="kicker">KI-INNOVATIONSPARTNERSCHAFTEN</div>
        <h2>Der Weg von der Praxis zur Innovation</h2>
        <p>
          Die drei Bereiche sind bewusst getrennt: Erst wird das Praxisproblem
          sichtbar, dann übernimmt ein Schülerteam die Bearbeitung und am Ende
          wird das entstandene Ergebnis dokumentiert.
        </p>
      </div>
    </div>
    <div class="grid grid-3">
      <div class="card"><strong>1 · Praxisproblem</strong><small>Betrieb beschreibt die Herausforderung und den gewünschten Nutzen.</small></div>
      <div class="card"><strong>2 · Entwicklung</strong><small>Schülerteam dokumentiert Analyse, KI-Einsatz, Arbeitsschritte und Verantwortlichkeiten.</small></div>
      <div class="card"><strong>3 · Ergebnis</strong><small>Idee, Prototyp, Produkt oder Konzept wird veröffentlicht und nachvollziehbar dokumentiert.</small></div>
    </div>
  </div>
...
function openKIChallengeForm(){
  modal(`
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">KI-INNOVATION · BETRIEB</div>
    <h2>Herausforderung aus dem Praktikumsbetrieb</h2>
    <p>Beschreibe ein konkretes Problem aus der Praxis, bei dem eine KI-Unterstützung sinnvoll sein könnte.</p>
    <div class="form">
      <label>Betrieb / Einrichtung
        <input id="kiCCompany" placeholder="Name des Betriebs">
      </label>
      <label>Ansprechperson
        <input id="kiCContact" placeholder="Name und ggf. Funktion">
      </label>
      <label>Titel der Herausforderung
        <input id="kiCTitle" placeholder="Kurz und konkret">
      </label>
      <label>Herausforderung
        <textarea id="kiCDescription" rows="5" placeholder="Was ist die konkrete Herausforderung im Arbeitsalltag?"></textarea>
      </label>
      <label>Wer ist davon betroffen?
        <textarea id="kiCTarget" rows="3" placeholder="Personen, Zielgruppe, Mitarbeitende …"></textarea>
      </label>
      <label>Was wäre eine gute Lösung / ein guter Nutzen?
        <textarea id="kiCGoal" rows="4" placeholder="Woran würde man erkennen, dass die Lösung hilfreich ist?"></textarea>
      </label>
      <label>Datenschutz / besondere Hinweise
        <textarea id="kiCPrivacy" rows="3" placeholder="Welche Daten dürfen nicht verwendet werden? Welche Rahmenbedingungen sind wichtig?"></textarea>
      </label>
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Abbrechen</button>
        <button class="primary" onclick="saveKIChallenge()">Herausforderung speichern</button>
      </div>
    </div>`);
}

async function saveKIChallenge(){
  const title=$("kiCTitle")?.value.trim()||"";
  const description=$("kiCDescription")?.value.trim()||"";
  if(!title||!description){toast("Bitte Titel und Herausforderung ausfüllen.");return}
  try{
    await addDoc(collection(db,"kiChallenges"),{
      module:"kiInnovationspartnerschaften",
      company:$("kiCCompany")?.value.trim()||"",
      contact:$("kiCContact")?.value.trim()||"",
      title,
      description,
      target:$("kiCTarget")?.value.trim()||"",
      goal:$("kiCGoal")?.value.trim()||"",
      privacy:$("kiCPrivacy")?.value.trim()||"",
      status:"offen",
...
async function openKIChallengesLibrary(){
  let items=[];
  try{items=await getCollection("kiChallenges","createdAt",true)}catch(e){console.error(e)}
  const open=items.filter(c=>c.status!=="abgeschlossen");
  modal(`
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">BIBLIOTHEK · HERAUSFORDERUNGEN</div>
    <h2>Herausforderungen aus Praktikumsbetrieben</h2>
    <div class="list">
      ${open.map(c=>`
        <div class="ki-entry">
          <div class="ki-entry-head">
            <div><strong>${esc(c.title||"Herausforderung")}</strong><small>${esc(c.company||"Praktikumsbetrieb")}</small></div>
            <span class="pill">${esc(c.status||"offen")}</span>
          </div>
          <p>${esc(c.description||"")}</p>
          <div class="ki-meta">
            ${c.goal?`<span class="pill">Ziel: ${esc(c.goal).slice(0,80)}</span>`:""}
            <button class="primary" onclick="openKITakeChallenge('${esc(c.id)}')">Herausforderung übernehmen</button>
          </div>
        </div>`).join("")||`<div class="ki-empty">Keine offenen Herausforderungen vorhanden.</div>`}
    </div>
...
async function openKITakeChallenge(id){
  ...
  if(!c){toast("Herausforderung nicht gefunden.");return}
  modal(`
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">KI-INNOVATION · BEARBEITUNG</div>
    <h2>Herausforderung übernehmen</h2>
    <p><strong>${esc(c.title||"")}</strong></p>
    <p>${esc(c.description||"")}</p>
    <div class="form">
      <label>Bearbeitungsform
        <select id="kiSMode"><option value="team">Schülerteam</option><option value="single">Einzelschüler/in</option></select>
      </label>
      <label>Name des Teams / Schüler/in
        <input id="kiSTeam" placeholder="Name">
      </label>
      <label>Weitere Mitglieder
        <textarea id="kiSMembers" rows="3" placeholder="Namen der Teammitglieder"></textarea>
      </label>
      <label>Erste Idee / Lösungsansatz
        <textarea id="kiSIdea" rows="4" placeholder="Wie könnte KI bei dieser Herausforderung helfen?"></textarea>
      </label>
      <label>Verantwortlichkeiten
        <textarea id="kiSRoles" rows="4" placeholder="Wer übernimmt welchen Teil?"></textarea>
      </label>
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Abbrechen</button>
        <button class="primary" onclick="saveKISolution('${esc(c.id)}')">Bearbeitung starten</button>
      </div>
    </div>`);
}

async function saveKISolution(challengeId){
  const name=$("kiSTeam")?.value.trim()||"";
  if(!name){toast("Bitte Teamname oder Name eintragen.");return}
  try{
    await addDoc(collection(db,"kiSolutions"),{
      module:"kiInnovationspartnerschaften",
      challengeId,
      mode:$("kiSMode")?.value||"team",
      team:$("kiSTeam")?.value.trim()||"",
      members:$("kiSMembers")?.value.trim()||"",
      idea:$("kiSIdea")?.value.trim()||"",
      responsibilities:$("kiSRoles")?.value.trim()||"",
      status:"in Bearbeitung",
      createdBy:currentUser.uid,
      createdAt:serverTimestamp()
    });
    closeModal();await render();toast("Bearbeitung wurde gestartet.");
  }catch(e){console.error(e);toast("Bearbeitung konnte nicht gespeichert werden.")}
}

async function openKISolutionsLibrary(){
  let items=[];
  try{items=await getCollection("kiSolutions","createdAt",true)}catch(e){console.error(e)}
  modal(`
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">BIBLIOTHEK · BEARBEITUNGEN</div>
    <h2>Wer bearbeitet welche Herausforderung?</h2>
    <div class="list">
      ${items.map(s=>`
        <div class="ki-entry">
          <div class="ki-entry-head">
            <div><strong>${esc(s.team||"Schüler/in")}</strong><small>${s.mode==="team"?"Schülerteam":"Einzelschüler/in"}</small></div>
            <span class="pill">${esc(s.status||"in Bearbeitung")}</span>
          </div>
          <p><strong>Aufgabe:</strong> ${esc(s.challengeId||"")}</p>
          ${s.members?`<small><strong>Mitglieder:</strong> ${esc(s.members)}</small>`:""}
          ${s.responsibilities?`<p><strong>Verantwortlichkeiten:</strong> ${esc(s.responsibilities)}</p>`:""}
          ${s.idea?`<p><strong>Erster Lösungsansatz:</strong> ${esc(s.idea)}</p>`:""}
        </div>`).join("")||`<div class="ki-empty">Noch keine Bearbeitungen vorhanden.</div>`}
    </div>
    <div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button></div>`);
}
...
function openKIResultForm(){
  modal(`
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">KI-INNOVATION · ERGEBNIS</div>
    <h2>Ergebnis dokumentieren</h2>
    <div class="form">
      <label>Titel des Ergebnisses
        <input id="kiRTitle" placeholder="z. B. Prototyp für …">
      </label>
      <label>Art des Ergebnisses
        <select id="kiRType"><option>Idee</option><option>Konzept</option><option>Prototyp</option><option>Produkt</option><option>Material</option><option>Prompt / KI-Workflow</option><option>Sonstiges</option></select>
      </label>
      <label>Beschreibung
        <textarea id="kiRDescription" rows="5"></textarea>
      </label>
      <label>Entstanden durch / Team
        <input id="kiRTeam">
      </label>
      <label>Praxispartner
        <input id="kiRPartner">
      </label>
      <label>Link / Datei / weitere Informationen
        <input id="kiRLink" placeholder="Optional">
      </label>
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Abbrechen</button>
        <button class="primary" onclick="saveKIResult()">Ergebnis speichern</button>
      </div>
    </div>`);
}

async function saveKIResult(){
  const title=$("kiRTitle")?.value.trim()||"";
  if(!title){toast("Bitte einen Titel eingeben.");return}
  try{
    await addDoc(collection(db,"kiResults"),{
      module:"kiInnovationspartnerschaften",
      title,
      type:$("kiRType")?.value||"Idee",
      description:$("kiRDescription")?.value.trim()||"",
      team:$("kiRTeam")?.value.trim()||"",
      partner:$("kiRPartner")?.value.trim()||"",
      link:$("kiRLink")?.value.trim()||"",
      createdBy:currentUser.uid,
      createdAt:serverTimestamp()
    });
...
async function openKIResultsLibrary(){
  let items=[];
  try{items=await getCollection("kiResults","createdAt",true)}catch(e){console.error(e)}
  modal(`
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">BIBLIOTHEK · ERGEBNISSE</div>
    <h2>Ergebnisse, Ideen & Produkte</h2>
    <div class="list">
      ${items.map(r=>`
        <div class="ki-entry ki-result">
          <div class="ki-entry-head"><div><strong>${esc(r.title||"Ergebnis")}</strong><small>${esc(r.type||"")}</small></div></div>
          <p>${esc(r.description||"")}</p>
          <div class="ki-meta">
            ${r.team?`<span class="pill">Team: ${esc(r.team)}</span>`:""}
            ${r.partner?`<span class="pill">Praxispartner: ${esc(r.partner)}</span>`:""}
            ${r.link?`<a class="secondary" href="${esc(r.link)}" target="_blank" rel="noopener">Öffnen</a>`:""}
          </div>
        </div>`).join("")||`<div class="ki-empty">Noch keine Ergebnisse vorhanden.</div>`}
    </div>
    <div class="form-actions">
      <button class="secondary" onclick="closeModal()">Schließen</button>
      <button class="primary" onclick="closeModal();setTimeout(openKIResultForm,50)">＋ Ergebnis hinzufügen</button>
    </div>`);
}
