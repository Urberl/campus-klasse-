...
function showApp(){
  ...
  const teamNav=$("sidebar")?.querySelector('[data-page="team"]');
  if(teamNav){
    teamNav.hidden=!isTeacher();
    const label=teamNav.lastChild;
    if(label) label.textContent=" Lehrkräfte Klassenteam";
  }
  render();
}
...
async function render(){
  ...
  let p=location.hash.replace("#","")||"start";
  if(p==="team" && !isTeacher()){
    p="start";
