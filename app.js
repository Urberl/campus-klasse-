...
function updateTeacherTeamNav(){
  ...
  links.forEach(link=>{
    // Nur dieser eine Navigationspunkt wird umbenannt. Alle anderen Menüpunkte bleiben unverändert.
    const walker=document.createTreeWalker(link,NodeFilter.SHOW_TEXT);
    const nodes=[];let n;while(n=walker.nextNode()) nodes.push(n);
    const textNodes=nodes.filter(node=>node.textContent.trim());
    if(textNodes.length){
      textNodes.forEach((node,i)=>{ node.textContent=i===textNodes.length-1 ? " Lehrkräfte Klassenteam" : ""; });
    }
...
async function renderLernwerkstatt(){
  const resources=[
         ...
         ["     ","Fragen und Hilfe","Wenn du nicht weiterkommst: fragen und teilen.","forum"]
  ];
