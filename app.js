...
onclick="go('forum')">Campus-Forum</button></div></section>
 ...
 <div class="grid grid-4">
    ...
    ${tile("       ","fpA","Praxisaufträge und Praxisreflexion.","praktikum")}
...
async function renderPraktikum(){
  ...
  try{
    ...
    // ABSOLUTE TRENNUNG: Nur explizit als fpA markierte Aufträge.
  ...
  return `${pageHead(
    "SCHULE ↔ PRAXIS",
    "fpA",
    "Ausschließlich Praxisaufträge, die für die fpA von der Lehrkraft angelegt wurden.",
    `<button class="primary" onclick="openPracticeForm()">＋ Praxisauftrag erstellen</button>`
  )}
  <style>
    .fpa-note{padding:18px;border-radius:14px;background:var(--soft,#f7f7f5);margin-bottom:16px}
    .fpa-card{min-height:180px}
    .fpa-tools{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-top:16px}
    .fpa-tools .card{min-height:130px}
    @media(max-width:800px){.fpa-tools{grid-template-columns:1fr}}
  </style>

  <div class="fpa-note">
    <strong>🧭 Praxisaufträge der Lehrkraft</strong>
    <p style="margin:7px 0 0">
      Dieser Bereich ist bewusst isoliert. Einträge aus Canva, Kompetenzwerkstatt,
      allgemeinen Projekten, Forum oder anderen Modulen werden hier nicht angezeigt.
    </p>
  </div>

  <div class="grid grid-2">
    ${data.map(p=>`
      <div class="card fpa-card">
        ...
        <p>${esc(p.text||"")}</p>
        <small style="margin-top:auto;color:var(--muted)">👩‍🏫 fpA-Auftrag der Lehrkraft</small>
      </div>
    `).join("")||`
      <div class="empty">
        <strong>Noch keine fpA-Praxisaufträge.</strong>
        <p>Die Aufträge erscheinen hier, sobald sie von der Lehrkraft angelegt wurden.</p>
      </div>`}
  </div>

  <div class="fpa-tools">
    <a class="card tile" href="#praxisfragen">
      <span class="emoji">❓</span>
      <strong>Fragen aus der Praxis</strong>
      <small>Eigenständiges Tool – getrennt von den Praxisaufträgen.</small>
    </a>
    <a class="card tile" href="#praxisprojekte">
      <span class="emoji">🚀</span>
      <strong>Projekte in der Praxis</strong>
      <small>Eigenständiges Tool – getrennt von den Praxisaufträgen.</small>
    </a>
    <a class="card tile" href="#ki">
      <span class="emoji">🤖</span>
      <strong>KI-Innovationspartnerschaften</strong>
      <small>Eigenständiger Bereich – nicht Teil der Praxisaufträge.</small>
    </a>
  </div>
...
async function renderPraxisFragen(){
  ...
  const questions=posts.filter(p=>p.type==="question" && p.module==="praxisfragen");
  return `${pageHead("fpA · EIGENES TOOL","Fragen aus der Praxis","Ein vollständig getrennter Bereich für Fragen aus dem Praktikum.",`<button class="primary" onclick="openPostForm('question')">＋ Frage</button>`)}
  <div class="card"><div class="notice"><strong>❓ Fragen aus der Praxis</strong><p style="margin-bottom:0">Hier werden ausschließlich Beiträge dieses Tools angezeigt.</p></div>
  <div class="list" style="margin-top:12px">${questions.map(postHTML).join("")||`<div class="empty">Noch keine Fragen aus der Praxis.</div>`}</div></div>${footer()}`;
}

async function renderPraxisProjekte(){
  let projects=[];
  try{projects=await getCollection("projects","createdAt",true)}catch(e){console.error(e)}
  const own=projects.filter(p=>p.module==="praxisprojekte");
  return `${pageHead("fpA · EIGENES TOOL","Projekte in der Praxis","Ein vollständig getrennter Bereich für Praxisprojekte – nicht das Kompetenznetzwerk.",`<button class="primary" onclick="openProjectForm()">＋ Praxisprojekt</button>`)}
  <div class="card"><div class="notice"><strong>🚀 Projekte in der Praxis</strong><p style="margin-bottom:0">Hier werden ausschließlich Projekte dieses Tools angezeigt.</p></div>
  <div class="grid grid-3" style="margin-top:12px">${own.map(p=>`<div class="card"><h3>${esc(p.title||"Praxisprojekt")}</h3><p>${esc(p.goal||"")}</p><small>${esc(p.team||"")} · ${esc(p.partner||"—")}</small></div>`).join("")||`<div class="empty">Noch keine Projekte in der Praxis.</div>`}</div></div>${footer()}`;
}
...
render();toast("fpA-Praxisauftrag gespeichert.")}catch(e){toast("Praxisauftrag konnte nicht gespeichert werden.")}}
