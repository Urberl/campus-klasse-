...
async function renderJournal(){
  ...
  const libraryRows=data.map(j=>`
    <div class="journal-library-row">
      <div class="journal-library-date">${esc(journalDisplayDate(j))}</div>
      <button class="journal-library-title" onclick="openJournalEntry('${j.id}')">${esc(j.title||"Lernjournal")}</button>
      <button class="journal-pdf-btn" onclick="printJournalEntry('${j.id}')">📄 PDF</button>
    </div>
  `).join("");
  ...
  <div class="journal-two-tiles">
    ...
    <section class="card journal-tile">
      ...
      <div class="form journal-form-grid">
        ...
        <label>Titel
          <input id="jTitle" placeholder="z. B. Heute habe ich verstanden …">
        </label>
    ...
    <section class="card journal-tile">
      <div class="journal-tile-head">
        ...
        ${data.length?`<button class="secondary" onclick="printMyJournals()">📚 Alle als PDF</button>`:""}
      </div>
      ...
      <div class="journal-library">
        ...
        ${libraryRows || `<div class="journal-empty"><strong>Noch kein Lernjournal vorhanden.</strong><br>Erstelle links deinen ersten Eintrag.</div>`}
      </div>
    </section>
...
function openJournalForm(){
  // Compatibility action: the new journal is entered directly in the first tile.
  location.hash="journal";
  setTimeout(()=>document.getElementById("jTitle")?.focus(),80);
}
...
async function addJournal(){
  ...
  try{
    await addDoc(collection(db,"journal"),{
      ...
      title,
      journalDate:journalDate||new Date().toISOString().slice(0,10),
      workedOn,
      learned,
      difficult,
      helpful,
      ...
      mood,
      satisfaction,
      // Backward-compatible text field for older views/data.
...
function openJournalEntry(id){
  getMyJournalEntries().then(entries=>{
    ...
    if(!j){toast("Lernjournal nicht gefunden.");return;}
    ...
    modal(`<button class="modal-close" onclick="closeModal()">×</button>
      ...
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Schließen</button>
        <button class="primary" onclick="closeModal();printJournalEntry('${id}')">📄 Als PDF</button>
      </div>`);
  }).catch(e=>{
    console.error(e);
