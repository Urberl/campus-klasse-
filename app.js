/* CAMPUS APP VERSION: 5.1 – LERNRESSOURCEN OHNE STORAGE */
...
function openResourceForm(){
 ...
 <div class="form">
  ...
  <label>Link <span id="rProvider"></span><input id="rUrl" type="url" placeholder="https://…" required></label>
  <label>Schlagworte<input id="rTags" placeholder="z. B. PP11, Üben, Fallbeispiel"></label>
  <div class="notice">💡 Die Bibliothek arbeitet vorerst ohne Firebase Storage. TaskCards, fobizz und andere Lernangebote werden über ihre Links eingebunden. Dateien können später ergänzt werden.</div>
...
async function saveResource(){
 ...
 if(!url||!resourceUrlValid(url)){toast("Bitte einen gültigen http/https-Link eingeben.");return}
 const btn=$("saveResourceBtn"); if(btn) btn.disabled=true;
 try{
   await loadFirebase();
   await addDoc(collection(db,"learningResources"),{
     title,type,description,subject,competency,duration,tags,url,
...
async function renderRessourcen(){
 ...
 return `${pageHead("LERNWERKSTATT","Lernressourcen","Alles, was dich beim Lernen unterstützt – TaskCards, fobizz und weitere Lernangebote.",teacher?`<button class="primary" onclick="openResourceForm()">＋ Lernressource</button>`:`<a class="secondary button" href="#lernwerkstatt">← Lernwerkstatt</a>`)}
 ...
 <div id="resourceGrid" class="resource-grid">${resources.length?resources.map(resourceCard).join(""):`<div class="empty" style="grid-column:1/-1"><strong>Noch keine Lernressourcen</strong><br>${teacher?"Lege die erste TaskCard, fobizz-Ressource oder externe Lernressource an.":"Hier werden bald Lernangebote für dich bereitstehen."}</div>`}</div>
 ${footer()}`;
}
