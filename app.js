...
function forumInjectStyles(){
  if($("forumFixStyles")) return;
  const style=document.createElement("style");
  style.id="forumFixStyles";
  style.textContent=`
    .forum-error{background:#fff0ef;border:1px solid #d77b73;color:#7b2420}
    #modal .primary:disabled{opacity:.65;cursor:wait}
  `;
...
async function forumErrorMessage(err){
 const code=err?.code||"unbekannt";
 const map={
   "permission-denied":"Keine Berechtigung für das Campus-Forum. Bitte die Firestore-Regeln für „posts“ prüfen.",
   "failed-precondition":"Firestore benötigt noch eine Konfiguration bzw. einen Index.",
   "unavailable":"Firebase ist momentan nicht erreichbar. Bitte Internetverbindung prüfen.",
   "network-request-failed":"Netzwerkfehler. Bitte Internetverbindung prüfen.",
   "unauthenticated":"Deine Anmeldung ist abgelaufen. Bitte neu anmelden."
 };
 return map[code]||`Forum-Fehler (${code}). Bitte Browser-Konsole prüfen.`;
}
...
async function addPost(){
 const textEl=$("pText"), typeEl=$("pType"), button=document.querySelector("#modal .primary");
 const text=textEl?.value.trim()||"";
 if(!text){toast("Bitte zuerst eine Nachricht eingeben.");textEl?.focus();return}
 ...
 try{
   ...
   const type=typeEl?.value||"question";
   const authorName=profile?.displayName||currentUser?.displayName||currentUser?.email||"Campus-Mitglied";
   ...
   closeModal();
   await render();
   toast("✅ Nachricht veröffentlicht.");
 }catch(e){
   console.error("Campus-Forum – Nachricht konnte nicht gespeichert werden:",e);
   const message=await forumErrorMessage(e);
   if(button){
   ...
   const box=$("modal");
   if(box){
     const old=box.querySelector(".forum-error");
     if(old) old.remove();
     const p=document.createElement("div");
     p.className="forum-error notice";
     p.style.marginTop="12px";
     p.innerHTML="<b>Nachricht konnte nicht gesendet werden.</b><br>"+esc(message)+"<br><small>Fehlercode: "+esc(e?.code||"unbekannt")+"</small>";
...
async function likePost(id){
 try{
   ...
   console.error("Forum Like:",e);
   toast(await forumErrorMessage(e));
 }
}

async function commentPost(id){
 const input=$("comment-"+id), text=input?.value.trim()||"";
 if(!text)return;
 try{
   await ensureForumReady();
