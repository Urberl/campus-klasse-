...
async function renderLernstand(){
  ...
  const completed=tasks.filter(t=>lernstandLatest(attempts,t.id)).length;
  const latestAll=attempts.slice().sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));
  const latest=latestAll[0];
  const totalLatest=latest?Number(latest.total)||0:null;
  ...
  return `${pageHead(
    ...
    isTeacher()?`<button class="primary" onclick="openLernstandEditor()">＋ Lernstandsmessung bearbeiten</button>`:""
  )}
  <style>
    .ls-intro{display:grid;grid-template-columns:1.35fr .65fr;gap:16px;margin-bottom:16px}
    .ls-intro-card{min-height:170px}
    .ls-flow{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin-top:14px}
    .ls-flow div{padding:12px;border:1px solid var(--line,#ddd);border-radius:12px;background:#fff}
    .ls-flow b{display:block;margin-bottom:4px}
    .ls-area-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}
    .ls-area{min-width:0}
    .ls-area-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px}
    .ls-area-head h2{margin:3px 0 4px}
    .ls-item{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:13px 0;border-top:1px solid var(--line,#ddd)}
    .ls-item-main{min-width:0}
    .ls-item-main strong{display:block}
    .ls-item-main small{display:block;color:var(--muted);margin-top:3px}
    .ls-item-actions{display:flex;align-items:center;gap:7px;flex-wrap:wrap;justify-content:flex-end}
    .ls-competence-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin-top:12px}
    .ls-comp-card{border:1px solid var(--line,#ddd);border-radius:12px;padding:11px;background:#fff}
    .ls-comp-card strong{font-size:12px;display:block}
    .ls-comp-card small{color:var(--muted)}
    .ls-mini-bar{display:block;height:6px;background:#edf0f2;border-radius:99px;overflow:hidden;margin-top:8px}
    .ls-mini-bar i{display:block;height:100%;background:var(--brand,#168fd0)}
    .ls-modal-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
    .ls-task-box{border:1px solid var(--line,#ddd);border-radius:12px;padding:14px;margin-top:10px;background:#fff}
    .ls-task-box h4{margin:0 0 7px}
    .ls-points{font-weight:800}
    .ls-progress{display:flex;gap:5px;margin:10px 0}
    .ls-progress span{height:7px;flex:1;border-radius:99px;background:#e9ecef}
    .ls-progress span.on{background:var(--brand,#168fd0)}
    .ls-solution{margin-top:10px;padding:12px;border-radius:10px;background:#f5f7f8;border:1px solid var(--line,#ddd)}
    .ls-history{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}
    .ls-history .card{margin:0}
    .ls-pillline{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}
    @media(max-width:900px){.ls-intro{grid-template-columns:1fr}.ls-area-grid{grid-template-columns:1fr}.ls-flow{grid-template-columns:1fr}.ls-competence-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.ls-modal-grid{grid-template-columns:1fr}}
    @media(max-width:600px){.ls-competence-grid{grid-template-columns:1fr}.ls-item{align-items:flex-start;flex-direction:column}.ls-item-actions{justify-content:flex-start}}
  </style>

  <div class="ls-intro">
    <section class="card ls-intro-card">
      <span class="badge">WAS IST DAS?</span>
      <h2>Dein Lernstand wird sichtbar.</h2>
      <p>Nach jedem abgeschlossenen Thema kannst du deinen Lernstand überprüfen. Jede Messung prüft dieselben fünf Kompetenzdimensionen. Dadurch werden Fortschritte über das Schuljahr hinweg vergleichbar.</p>
      <div class="ls-flow">
        ${LERNSTAND_COMPETENCIES.map((c,i)=>`<div><b>${i+1}. ${c.label}</b><small>bis 3 Punkte</small></div>`).join("")}
      </div>
    </section>
    <section class="card ls-intro-card">
      <span class="badge">15 PUNKTE</span>
      <h2>Einheitliches Bewertungsschema</h2>
      <p>Jede Kompetenzdimension wird mit bis zu <strong>3 Punkten</strong> bewertet. Zusammen ergeben sich maximal <strong>15 Punkte</strong>.</p>
      <div class="list">
        <div class="list-item"><strong>12–15 Punkte</strong><span class="pill green">Auf Kurs</span></div>
        <div class="list-item"><strong>8–11 Punkte</strong><span class="pill yellow">Klärungsbedarf</span></div>
        <div class="list-item"><strong>0–7 Punkte</strong><span class="pill red">Handlungsbedarf</span></div>
      </div>
    </section>
  </div>

  <div class="card" style="margin-bottom:16px">
    <div class="kicker">KOMPETENZENTWICKLUNG</div>
    <h2>Was wird im Laufe des Schuljahres sichtbar?</h2>
    <p>Die Ergebnisse jeder Lernstandsmessung werden gespeichert. So kann die Entwicklung der fünf Kompetenzdimensionen über mehrere Themen hinweg nachvollzogen werden – nicht nur eine einzelne Punktzahl.</p>
    <div class="ls-competence-grid">
      ${LERNSTAND_COMPETENCIES.map(c=>{
        const series=lernstandCompetenceSeries(attempts,c.id);
        const last=series.length?series[series.length-1]:null;
        return `<div class="ls-comp-card"><strong>${esc(c.label)}</strong><small>${last===null?"Noch kein Ergebnis":last+"/3 Punkte zuletzt"}</small>${last===null?"":lernstandBar(last,3)}</div>`;
      }).join("")}
    </div>
    ${latest?`<div class="notice" style="margin-top:14px"><strong>Letzter Lernstand: ${latest.total}/15 · ${lernstandStatusText(latest.total)}</strong><p style="margin-bottom:0">Versuch ${latest.attempt} bei „${esc(latest.title||"Lernstandsmessung")}".</p></div>`:`<div class="notice" style="margin-top:14px"><strong>Noch keine Lernstandsmessung abgeschlossen.</strong><p style="margin-bottom:0">Starte nach dem nächsten Thema mit der passenden Kompetenzüberprüfung.</p></div>`}
  </div>

  <div class="ls-area-grid">
    ${["lb1","lb2","lb3","lb4"].map(areaId=>`
      <section class="card ls-area">
        <div class="ls-area-head">
          <div><span class="badge">${LERNSTAND_AREAS[areaId].icon} LERNBEREICH</span><h2>${esc(LERNSTAND_AREAS[areaId].title)}</h2></div>
          <span class="pill">${byArea[areaId].length} Messungen</span>
        </div>
        ${byArea[areaId].map(t=>{
          const a=lernstandLatest(attempts,t.id);
          const count=lernstandAttemptCount(attempts,t.id);
          return `<div class="ls-item">
            <div class="ls-item-main"><strong>${t.nr}. ${esc(t.title)}</strong><small>${count?`letzter Stand: ${a.total}/15 · Versuch ${a.attempt}`:"noch nicht bearbeitet"}</small></div>
            <div class="ls-item-actions">
              ${a?`<span class="pill ${lernstandStatus(a.total)}">${lernstandStatusText(a.total)}</span>`:""}
              <button class="secondary" onclick="openLernstand('${t.id}')">${a?"Weiter / ansehen":"Starten"} →</button>
            </div>
          </div>`;
        }).join("")}
      </section>
...
function openLernstand(id){
  getLernstandTasks().then(async tasks=>{
    ...
    const nextAttempt=Math.min(3,count+1);
    const showSolution=nextAttempt>=3;
    const prior=lernstandLatest(attempts,id);
    modal(`<button class="modal-close" onclick="closeModal()">×</button>
      <div class="kicker">${LERNSTAND_AREAS[t.learningArea].icon} LERNSTANDSMESSUNG ${t.nr}/26</div>
      <h2>${esc(t.title)}</h2>
      <p>${esc(t.description||"Kompetenzüberprüfung mit fünf Kompetenzdimensionen.")}</p>
      <div class="notice"><strong>Versuch ${nextAttempt} von 3</strong><p style="margin-bottom:0">Jede Kompetenzdimension zählt maximal 3 Punkte. Nach dem dritten Versuch werden die Musterlösungen eingeblendet.</p></div>
      <div class="ls-progress">${[1,2,3].map(n=>`<span class="${n<=count?"on":""}"></span>`).join("")}</div>
      <div class="form">
        ${t.tasks.map((q,i)=>`<div class="ls-task-box">
          <h4>${i+1}. ${esc(q.label)} <span class="ls-points">· ${q.points} P.</span></h4>
          <p>${esc(q.prompt)}</p>
          <textarea id="lsAnswer_${q.id}" rows="4" placeholder="Deine Antwort …"></textarea>
          ${showSolution&&q.solution?`<div class="ls-solution"><strong>💡 Musterlösung</strong><p style="white-space:pre-wrap;margin-bottom:0">${esc(q.solution)}</p></div>`:""}
        </div>`).join("")}
        <div class="form-actions">
          <button class="secondary" onclick="closeModal()">Abbrechen</button>
          <button class="primary" onclick="submitLernstand('${t.id}',${nextAttempt})">${nextAttempt>=3?"3. Versuch speichern":"Versuch speichern"}</button>
        </div>
      </div>`);
  });
}
...
async function submitLernstand(taskId,attempt){
  ...
  if(Object.values(answers).every(v=>!v)){toast("Bitte bearbeite mindestens eine Aufgabe.");return}
  try{
    await addDoc(collection(db,"lernstandVersuche"),{
      uid:currentUser.uid,displayName:profile?.displayName||currentUser?.email||"Schüler/in",
      taskId:t.id,title:t.title,nr:t.nr,learningArea:t.learningArea,attempt,
      answers,competencies:{},total:0,status:"noch nicht bewertet",createdAt:serverTimestamp()
    });
    closeModal();
    toast("Versuch gespeichert. Die Bewertung kann anschließend ergänzt werden.");
    await render();
...
async function openLernstandEditor(){
  ...
  const tasks=await getLernstandTasks();
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">👩‍🏫 LERNSTANDSMESSUNGEN</div>
    <h2>26 Kompetenzüberprüfungen verwalten</h2>
    <p>Die 26 Themen sind bereits den vier Lernbereichen zugeordnet. Jede Messung besitzt dieselben fünf Kompetenzdimensionen mit je 3 Punkten.</p>
    <div class="list">${tasks.map(t=>`<div class="ls-item">
      <div class="ls-item-main"><strong>${t.nr}. ${esc(t.title)}</strong><small>${esc(LERNSTAND_AREAS[t.learningArea].title)}</small></div>
      <button class="secondary" onclick="openLernstandTaskEditor('${t.id}')">Aufgaben bearbeiten</button>
    </div>`).join("")}</div>
...
async function openLernstandTaskEditor(id){
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">👩‍🏫 ${t.nr}/26 · ${esc(LERNSTAND_AREAS[t.learningArea].title)}</div>
    <h2>${esc(t.title)}</h2>
    <div class="form">
      <label>Kurzer Überblick / Beschreibung<textarea id="lsEditDescription" rows="3">${esc(t.description||"")}</textarea></label>
      ${t.tasks.map(q=>`<div class="ls-task-box">
        <h4>${esc(q.label)} · 3 Punkte</h4>
        <label>Aufgabe<textarea id="lsEditPrompt_${q.id}" rows="4">${esc(q.prompt||"")}</textarea></label>
        <label>Musterlösung<textarea id="lsEditSolution_${q.id}" rows="4">${esc(q.solution||"")}</textarea></label>
      </div>`).join("")}
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Abbrechen</button>
        <button class="primary" onclick="saveLernstandTask('${t.id}')">Speichern</button>
      </div>
    </div>`);
}
...
async function saveLernstandTask(id){
  ...
  const updated={...base,description:$("lsEditDescription")?.value.trim()||base.description,tasks:base.tasks.map(q=>({
    ...q,
    prompt:$("lsEditPrompt_"+q.id)?.value.trim()||"",
    solution:$("lsEditSolution_"+q.id)?.value.trim()||""
  }))};
  try{
    const payload={taskId:id,title:updated.title,nr:updated.nr,learningArea:updated.learningArea,description:updated.description,tasks:updated.tasks,updatedBy:currentUser.uid,updatedAt:serverTimestamp()};
    await setDoc(doc(db,"lernstandMessungen",id),payload,{merge:true});
    const local=lernstandStoredTasks();local[id]=updated;localStorage.setItem("campus_lernstand_tasks",JSON.stringify(local));
    closeModal();await render();toast("Lernstandsmessung gespeichert.");
  }catch(e){console.error("Lernstand bearbeiten:",e);toast("Lernstandsmessung konnte nicht gespeichert werden.")}
}

window.openLernstand=openLernstand;
window.submitLernstand=submitLernstand;
window.openLernstandEditor=openLernstandEditor;
window.openLernstandTaskEditor=openLernstandTaskEditor;
window.saveLernstandTask=saveLernstandTask;
