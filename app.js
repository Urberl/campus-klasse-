...
async function renderPraktikum(){
  ...
  try{practice=await getCollection("practice","createdAt",true)}catch(e){console.error("Praxisaufträge:",e)}
  try{impressions=await getCollection("practiceImpressions","createdAt",true)}catch(e){console.error("Praxis-Eindrücke:",e)}
  try{
    const posts=await getCollection("posts","createdAt",true);
    questions=posts.filter(p=>p.type==="question").slice(0,6);
  }catch(e){console.error("Fragen:",e)}
  try{projects=await getCollection("projects","createdAt",true)}catch(e){console.error("Projekte:",e)}
  try{cloudEntries=await getCollection("practiceWordCloud","createdAt",true)}catch(e){console.error("Wortwolke:",e)}

  const myImpressions=impressions.filter(i=>i.uid===currentUser.uid);

  // Aggregate the shared practice impressions into a simple, deterministic
  // word-cloud representation. Students submit short terms; repeated terms
  // become larger.
  const cloudCounts={};
  cloudEntries.forEach(e=>{
    const words=String(e.words||"").split(/[,\n;]+/).map(w=>w.trim().toLowerCase()).filter(w=>w.length>=3);
    words.forEach(w=>{
      if(w.length>28)return;
      cloudCounts[w]=(cloudCounts[w]||0)+1;
    });
  });
  const cloudWords=Object.entries(cloudCounts)
    .sort((a,b)=>b[1]-a[1])
    .slice(0,40);
  const maxCloud=Math.max(1,...cloudWords.map(x=>x[1]));

  const practiceCards=practice.map(p=>{
    const mine=myImpressions.filter(i=>i.practiceId===p.id);
    return `<div class="card">
      <span class="pill ${p.state==="offen"?"orange":"green"}">${esc(p.state||"geplant")}</span>
      <h3>${esc(p.title||"Praxisauftrag")}</h3>
      <p>${esc(p.text||"")}</p>
      <small style="display:block;color:var(--muted);margin:8px 0">${esc(p.date||"")}</small>
      <div class="form-actions">
        <button class="primary" onclick="openPracticeImpressionForm('${esc(p.id)}')">＋ Mein Eindruck</button>
        ${mine.length?`<span class="pill green">${mine.length} eigener ${mine.length===1?"Eintrag":"Einträge"}</span>`:""}
      </div>
    </div>`;
  }).join("");

  const impressionRows=myImpressions.map(i=>`
    <div class="list-item">
      <div>
        <strong>${esc(i.practiceTitle||"Praxisauftrag")}</strong>
        <small>${esc(i.date||fpADisplayDate(i.createdAt))} · ${esc(i.title||"Mein Eindruck")}</small>
      </div>
      <button class="secondary" onclick="openPracticeImpression('${esc(i.id)}')">Öffnen</button>
    </div>
  `).join("");

  const questionRows=questions.map(q=>`
    <div class="list-item">
      <div><strong>${esc(q.authorName||"Schüler/in")}</strong><small>${esc(q.text||"")}</small></div>
      <span class="pill">Frage</span>
    </div>
  `).join("");

  const projectRows=projects.slice(0,6).map(p=>`
    <div class="list-item">
      <div><strong>${esc(p.title||"Projekt")}</strong><small>${esc(p.team||"")} · Praxispartner: ${esc(p.partner||"—")}</small></div>
      <span class="pill">${Number(p.progress||0)}%</span>
    </div>
  `).join("");

  const cloudHtml=cloudWords.length
    ? cloudWords.map(([word,count])=>{
        const size=16+Math.round((count/maxCloud)*24);
        return `<span class="fpa-cloud-word" style="font-size:${size}px" title="${count} Nennung${count===1?"":"en"}">${esc(word)}</span>`;
      }).join("")
    : `<div class="empty">Noch keine Begriffe gesammelt. Trage deine ersten Eindrücke ein.</div>`;

  return `${pageHead(
    "SCHULE ↔ PRAXIS",
    "fpA",
    "Praxisaufträge, Eindrücke, Fragen, Projekte und KI-Innovationspartnerschaften an einem Ort.",
    `<button class="primary" onclick="openPracticeForm()">＋ Praxisauftrag</button>`
  )}

  <style>
    .fpa-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}
    .fpa-tile{min-height:190px;cursor:pointer}
    .fpa-tile .emoji{font-size:30px}
    .fpa-tile h2{margin:8px 0 5px}
    .fpa-tile p{margin:0;color:var(--muted)}
    .fpa-section{margin-top:16px}
    .fpa-library{margin-top:12px}
    .fpa-cloud{
      min-height:250px;
      display:flex;
      flex-wrap:wrap;
    ...
    .fpa-download-icon{font-size:26px}
    .fpa-download-card strong{display:block}
    .fpa-download-card small{display:block;color:var(--muted);margin-top:3px}
    @media(max-width:900px){.fpa-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:600px){
      .fpa-grid,.fpa-downloads{grid-template-columns:1fr}
    }
  </style>

  <div class="fpa-grid">

    <section class="card fpa-tile" onclick="document.getElementById('fpaPraxis').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">🧭</span>
      <h2>Praxisaufträge</h2>
      <p>Arbeitsaufträge aus der Praxis bearbeiten und eigene Eindrücke festhalten.</p>
      <span class="pill" style="margin-top:14px">${practice.length} Aufträge</span>
    </section>

    <section class="card fpa-tile" onclick="document.getElementById('fpaFragen').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">❓</span>
      <h2>Fragen</h2>
      <p>Fragen aus dem Praktikum sammeln, teilen und gemeinsam weiterdenken.</p>
      <span class="pill" style="margin-top:14px">${questions.length} aktuelle Fragen</span>
    </section>

    <section class="card fpa-tile" onclick="document.getElementById('fpaProjekte').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">🚀</span>
      <h2>Projekte</h2>
      <p>Aus Beobachtungen und Fragestellungen entstehen eigene Praxisprojekte.</p>
      <span class="pill" style="margin-top:14px">${projects.length} Projekte</span>
    </section>

    <section class="card fpa-tile" onclick="document.getElementById('fpaDownloads').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">📥</span>
      <h2>Formulare & Downloads</h2>
      <p>Die wichtigen Unterlagen für die fpA zentral zum Öffnen und Herunterladen.</p>
      <span class="pill" style="margin-top:14px">2 Dokumente</span>
    </section>

    <section class="card fpa-tile" onclick="document.getElementById('fpaCloud').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">☁️</span>
      <h2>Meine Praxis-Eindrücke</h2>
      <p>Wörter und Begriffe sammeln, die eure Erfahrungen aus der Praxis sichtbar machen.</p>
      <span class="pill" style="margin-top:14px">${cloudEntries.length} Beiträge</span>
    </section>

    <section class="card fpa-tile" onclick="location.hash='ki'">
      <span class="emoji">🤖</span>
      <h2>KI-Innovationspartnerschaften</h2>
      <p>Gemeinsam mit Praxispartnern KI-Ideen entwickeln, erproben und auf ihren Nutzen prüfen.</p>
      <span class="pill" style="margin-top:14px">Zum Innovationsbereich →</span>
    </section>

  </div>

  <section id="fpaPraxis" class="card fpa-section">
    <div class="page-head" style="margin-bottom:10px">
      <div>
        <div class="kicker">PRAXISAUFTRÄGE</div>
        <h2>Praxisaufträge & meine Eindrücke</h2>
        <p>Zu jedem Praxisauftrag kannst du deine persönlichen Beobachtungen und Erfahrungen speichern.</p>
      </div>
    </div>
    <div class="grid grid-2">
      ${practiceCards||`<div class="empty">Noch keine Praxisaufträge vorhanden.</div>`}
    </div>
    <div class="card fpa-library">
      <h3>📚 Meine Praxis-Eindrücke</h3>
      <p>Hier werden deine persönlichen Eindrücke dauerhaft gesammelt.</p>
      <div class="list">
        ${impressionRows||`<div class="empty">Du hast noch keinen Praxiseindruck gespeichert.</div>`}
      </div>
    </div>
  </section>

  <section id="fpaCloud" class="card fpa-section">
    <div class="page-head" style="margin-bottom:10px">
      <div>
        <div class="kicker">REFLEXION</div>
        <h2>☁️ Wortwolke: Was nehme ich aus der Praxis mit?</h2>
        <p>Die Begriffe aller Beiträge werden gemeinsam sichtbar. Häufig genannte Begriffe erscheinen größer.</p>
      </div>
      <div class="actions">
        <button class="primary" onclick="openPracticeWordCloudForm()">＋ Eindruck hinzufügen</button>
      </div>
    </div>
    <div class="fpa-cloud">${cloudHtml}</div>
  </section>

  <section id="fpaDownloads" class="card fpa-section">
    <div class="page-head" style="margin-bottom:12px">
      <div>
        <div class="kicker">UNTERLAGEN</div>
        <h2>📥 Formulare & Downloads</h2>
        <p>Die zentralen Dokumente für die fpA.</p>
      </div>
    </div>
    <div class="fpa-downloads">
      <div class="fpa-download-card">
        <div class="fpa-download-icon">📄</div>
        <div style="flex:1">
          <strong>Bewertungsbogen des Praktikumsbetriebes</strong>
          <small>Vorgegebenes Formular des Praktikumsbetriebes</small>
        </div>
        <button class="secondary" onclick="downloadFpaDocument('bewertung')">Download</button>
      </div>
      <div class="fpa-download-card">
        <div class="fpa-download-icon">📋</div>
        <div style="flex:1">
          <strong>Wochenbericht</strong>
          <small>Vorgegebenes Formular für den Praktikums-Wochenbericht</small>
        </div>
        <button class="secondary" onclick="downloadFpaDocument('wochenbericht')">Download</button>
      </div>
    </div>
    <div class="notice" style="margin-top:12px">
      <strong>Hinweis für die Einrichtung:</strong>
      Hinterlege die beiden PDF-Dateien in den unten im Code vorgesehenen Dokument-Links.
    </div>
  </section>

  <section id="fpaFragen" class="card fpa-section">
    <div class="page-head" style="margin-bottom:10px">
      <div>
        <div class="kicker">FRAGEN</div>
        <h2>Fragen aus der Praxis</h2>
        <p>Was beschäftigt dich im Praktikum? Teile deine Frage mit der Campusgemeinschaft.</p>
      </div>
      <div class="actions"><button class="primary" onclick="openPostForm('question')">＋ Frage stellen</button></div>
    </div>
    <div class="list">${questionRows||`<div class="empty">Noch keine Fragen vorhanden.</div>`}</div>
  </section>

  <section id="fpaProjekte" class="card fpa-section">
    <div class="page-head" style="margin-bottom:10px">
      <div>
        <div class="kicker">PROJEKTE</div>
        <h2>Praxisprojekte</h2>
        <p>Praxisbeobachtungen werden zu konkreten Projektideen.</p>
      </div>
      <div class="actions"><button class="primary" onclick="openProjectForm()">＋ Projekt anlegen</button></div>
    </div>
    <div class="list">${projectRows||`<div class="empty">Noch keine Projekte vorhanden.</div>`}</div>
  </section>

  <section class="card fpa-section" style="border-left:4px solid var(--accent,#7b9b82)">
    <div class="page-head" style="margin-bottom:8px">
      <div>
        <div class="kicker">INNOVATION</div>
        <h2>KI-Innovationspartnerschaften</h2>
        <p>Reale Herausforderungen erkennen, KI-Ideen entwickeln und gemeinsam mit Praxispartnern erproben.</p>
      </div>
      <div class="actions"><button class="primary" onclick="location.hash='ki'">KI-Innovationslabor öffnen →</button></div>
    </div>
    <div class="grid grid-3">
      <div class="card tile"><span class="emoji">🔎</span><strong>Praxisproblem erkennen</strong><small>Eine konkrete Herausforderung aus dem Betrieb beschreiben.</small></div>
      <div class="card tile"><span class="emoji">💡</span><strong>KI-Idee entwickeln</strong><small>Überlegen, wo KI sinnvoll unterstützen könnte.</small></div>
      <div class="card tile"><span class="emoji">🤝</span><strong>Gemeinsam erproben</strong><small>Idee mit dem Praxispartner prüfen, testen und weiterentwickeln.</small></div>
    </div>
  </section>
...
async function openPracticeWordCloudForm(){
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">PRAXIS · WORTWOLKE</div>
    <h2>Was nehme ich aus der Praxis mit?</h2>
    <p>Gib 1–5 kurze Begriffe ein, die deine Erfahrungen aus dem Praktikum beschreiben.</p>
    <div class="form">
      <label>Meine Begriffe
        <textarea id="pwcWords" rows="4" placeholder="z. B. Teamarbeit, Verantwortung, Kinder, Kommunikation, Geduld"></textarea>
      </label>
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Abbrechen</button>
        <button class="primary" onclick="savePracticeWordCloud()">Speichern</button>
      </div>
    </div>`);
}

async function savePracticeWordCloud(){
  const words=$("pwcWords")?.value.trim()||"";
  if(!words){
    toast("Bitte mindestens einen Begriff eingeben.");
    return;
  }
  try{
    await addDoc(collection(db,"practiceWordCloud"),{
      uid:currentUser.uid,
      studentName:profile?.displayName||currentUser?.email||"Schüler/in",
      words,
      createdAt:serverTimestamp()
    });
    closeModal();
    await render();
    toast("Deine Eindrücke wurden zur Wortwolke hinzugefügt.");
  }catch(e){
    console.error("Wortwolke:",e);
    toast("Der Beitrag konnte nicht gespeichert werden.");
  }
}

function downloadFpaDocument(type){
  // These are intentionally centralised so the school can replace the two
  // links with its official PDFs without touching the rest of the fpA page.
  const documents={
    bewertung:"",
    wochenbericht:""
  };
  ...
  if(!url){
    toast("Für dieses Formular ist noch keine PDF-Datei hinterlegt.");
    return;
  }
