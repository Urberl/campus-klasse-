    ...
    const rows=users.map(u=>{
      ...
      const actions=(u.role==="student" || isAdmin()) ? `
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          ${status!=="approved"?`<button class="secondary" onclick="setUserStatus('${u.uid}','approved')">✓ Freischalten</button>`:""}
          ${status!=="blocked"?`<button class="secondary" onclick="setUserStatus('${u.uid}','blocked')">Sperren</button>`:""}
          ${status==="blocked"?`<button class="secondary" onclick="setUserStatus('${u.uid}','pending')">Reaktivieren</button>`:""}
...
class="list-item"><div><strong>${esc(p.title||p.text)}</strong>${p.title?`<small>${esc(p.text)} · ${fmtDate(p.createdAt)}</small>`:`<small>${fmtDate(p.createdAt)}</small>`}</div><div style="display:flex;align-items:center;gap:8px"><span class="pill">Info</span>${isTeacher()?`<button class="secondary" onclick="deletePost('${p.id}')">Löschen</button>`:""}</div>
...
async function likePost(id){
  try{
    ...
    document.querySelectorAll(`button[onclick="likePost('${id}')"]`).forEach(b=>{
      const m=String(b.textContent||"").match(/(\d+)\s*\)?$/);
      const next=(m?Number(m[1]):0)+1;
      b.textContent=`♡ Gefällt mir (${next})`;
      b.disabled=true;
    });
  }catch(e){console.error("Gefällt mir:",e);toast(e?.code==="permission-denied"?"Gefällt mir ist in den Firebase-Regeln nicht freigegeben.":"Gefällt mir konnte nicht gespeichert werden.")}
}
