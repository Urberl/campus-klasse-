...
/* Lernjournal compact styles */
.journal-note{background:var(--soft-green);border:1px solid var(--line);border-radius:14px;padding:12px}
.check-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:7px}
.check-grid label{border:1px solid var(--line);border-radius:10px;padding:8px;background:var(--bg);font-size:13px}
...
async function savePP11Note(areaId){
  ...
  try{
    ...
    await setDoc(ref,{uid:currentUser.uid,areaId,note:noteEl.value.trim(),shareWithTeacher:!!shareEl?.checked,updatedAt:serverTimestamp()},{merge:true});
...
async function renderPP11TeacherOverview(){
  if(!isTeacher()) return "";
  try{
    const [snap,noteSnap]=await Promise.all([getDocs(collection(db,"selfAssessments")),getDocs(collection(db,"selfAssessmentNotes"))]);
    const all=snap.docs.map(d=>d.data()), notes=noteSnap.docs.map(d=>d.data()).filter(x=>x.shareWithTeacher===true && String(x.note||"").trim());
    const byId={};
    all.forEach(x=>{byId[x.competencyId]??=[];byId[x.competencyId].push(Number(x.rating||0))});
    const items=pp11AllItems();
    const weak=items.map(i=>{const vals=byId[i.id]||[];const avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;return {...i,avg,n:vals.length}}).filter(x=>x.n).sort((a,b)=>a.avg-b.avg).slice(0,6);
    return `<div class="card pp11-teacher-view" style="margin-top:12px"><h3>👩‍🏫 Für die Lehrkraft</h3><p>Hier erscheinen nur Notizen, die Schülerinnen und Schüler ausdrücklich freigegeben haben.</p>
      ${notes.length?`<div class="list">${notes.slice(0,8).map(x=>`<div class="list-item"><div><strong>${esc(x.note)}</strong><small>${esc(x.areaId||"Lernbereich")}</small></div><span class="pill">freigegeben</span></div>`).join("")}</div>`:`<div class="empty">Noch keine freigegebenen Lernnotizen.</div>`}
      ${weak.length?`<h4 style="margin-top:14px">Häufiger Klärungsbedarf in der Gruppe</h4><div class="pp11-teacher-grid">${weak.map(x=>`<div class="pp11-teacher-card"><small>${esc(x.areaTitle)} · ${esc(x.sectionTitle)}</small><strong>${esc(x.text)}</strong><span class="pill ${x.avg>=3.5?"green":x.avg>=2?"":"red"}">Ø ${x.avg.toFixed(1)} · ${x.n}</span></div>`).join("")}</div>`:""}
    </div>`;
  }catch(e){console.error("PP11 Lehrerübersicht:",e);return ""}
}
...
async function renderLernwerkstatt(){
  ...
  return `${pageHead("SELBSTSTÄNDIG LERNEN","Lernwerkstatt","Lernaufträge, Lernimpulse, Materialien, Methoden, digitale Tools und KI – plus dein persönlicher Lernstand.")}
  ...
  <div class="grid grid-3" style="margin-bottom:12px">
    ...
    ${tile("🧰","Materialien & Methoden","Materialien, Methoden und digitale Werkzeuge für dein Lernen.","praktikum")}
  </div>
  ...
  <div class="card pp11-stand-card">
    <div class="pp11-stand-head"><div><span class="badge">NEU · PP 11 · LEHRPLAN 2026/27</span><h2 style="margin:8px 0 4px">📊 Mein Lernstand</h2><p style="margin:0">Schätze deinen Lernstand ein und halte fest, wobei du noch Unterstützung brauchst.</p></div><div class="pp11-stand-number"><b>${answered}/${items.length}</b><small>Kompetenzen eingeschätzt</small></div></div>
  ...
  <div class="pp11-compact-list">
    ${PP11_COMPETENCIES.map(a=>pp11CompactArea(a,assessments,notes)).join("")}
  </div>

  ${await renderPP11TeacherOverview()}
  <div class="notice" style="margin-top:12px"><b>Deine Notizen:</b> Nur wenn du das Kästchen „Lehrkraft darf meine Notiz sehen“ aktivierst, wird deine persönliche Lernnotiz für die Lehrkraft sichtbar.</div>
  ${footer()}`;
}
async function saveJournalEntry(){
  ...
  const topic = (get("journalTopic")?.value||"").trim();
  const activity = Array.from(document.querySelectorAll('input[name="journalActivity"]:checked')).map(x=>x.value);
  const learned = (get("journalLearned")?.value||"").trim();
  const canNow = (get("journalCanNow")?.value||"").trim();
  const unclear = (get("journalUnclear")?.value||"").trim();
  const nextStep = (get("journalNextStep")?.value||"").trim();
  const helpful = Array.from(document.querySelectorAll('input[name="journalHelpful"]:checked')).map(x=>x.value);
  const helpfulNext = (get("journalHelpfulNext")?.value||"").trim();
  const confidence = Number(get("journalConfidence")?.value||0);
  const practice = (get("journalPractice")?.value||"").trim();
  const shareWithTeacher = !!get("journalShareTeacher")?.checked;

  if(!learned && !canNow && !unclear && !nextStep && !practice){
    toast("Schreibe mindestens einen kurzen Reflexionspunkt.");
    return;
  }
  ...
  try{
    ...
    toast("Lerneintrag gespeichert.");
    await render();
  }catch(e){
    console.error("Lernjournal speichern:",e);
    toast("Der Eintrag konnte nicht gespeichert werden. Bitte prüfe die Firestore-Regeln.");
  }
}
window.saveJournalEntry=saveJournalEntry;

async function renderLernjournal(){
  let entries=[];
  try{
    const q=query(collection(db,"learningJournal"),where("uid","==",currentUser.uid));
    const snap=await getDocs(q);
  ...
  const activityOptions=[
    "Unterricht","Lernwerkstatt","Arbeitsauftrag","Partner-/Gruppenarbeit",
    "Praktikum","eigenes Lernen","Lernvideo / digitales Material","KI genutzt"
  ];
  const helpfulOptions=[
    "Erklärung der Lehrkraft","Mitschüler/in","eigenes Ausprobieren","Beispiel",
    "Lernvideo","Arbeitsblatt","KI","Lernwerkstatt","Praktische Erfahrung","Wiederholung"
  ];

  return `${pageHead("REFLEXION","Lernjournal","Halte kurze Lernmomente fest. Was hast du gelernt, was kannst du jetzt und woran möchtest du weiterarbeiten.",`<button class="primary" onclick="document.getElementById('journalNew')?.scrollIntoView({behavior:'smooth'})">✏️ Lernmoment festhalten</button>`)}
    <div class="grid grid-2">
      <div class="card">
        <span class="badge">MEIN LERNWEG</span>
        <h3 style="margin-top:8px">Was hat sich verändert?</h3>
        <p>Das Lernjournal ergänzt deine Lernwerkstatt: Dort schätzt du deinen Lernstand ein – hier hältst du fest, <b>wie du dich weiterentwickelst</b>.</p>
      </div>
      <div class="card">
        <span class="badge">2–3 MINUTEN</span>
        <h3 style="margin-top:8px">Kurz statt Pflichtprotokoll</h3>
        <p>Du musst nicht jedes Feld ausfüllen. Ein kurzer, ehrlicher Eintrag reicht.</p>
      </div>
    </div>
    ...
    <div id="journalNew" class="card" style="margin-top:12px">
      ...
      <label>Woran habe ich gearbeitet?</label>
      <input id="journalTopic" placeholder="z. B. Attributionstheorie nach Weiner">

      <label style="margin-top:12px">Was habe ich gemacht?</label>
      <div class="check-grid">
        ${activityOptions.map(x=>`<label><input type="checkbox" name="journalActivity" value="${esc(x)}"> ${esc(x)}</label>`).join("")}
      </div>
      ...
      <div class="check-grid">
        ${helpfulOptions.map(x=>`<label><input type="checkbox" name="journalHelpful" value="${esc(x)}"> ${esc(x)}</label>`).join("")}
      </div>
      ...
      <label>Wie sicher fühle ich mich jetzt?</label>
      <select id="journalConfidence">
        <option value="0">noch nicht einschätzen</option>
        <option value="1">🔴 noch nicht</option>
        <option value="2">🟠 teilweise</option>
        <option value="3">🟡 ziemlich sicher</option>
        <option value="4">🟢 sicher</option>
      </select>

      <label style="margin-top:12px">🏫 Schule ↔ Praxis (optional)</label>
      <textarea id="journalPractice" rows="2" placeholder="Was habe ich in der Praxis beobachtet oder besser verstanden?"></textarea>

      <div class="notice" style="margin-top:12px">
        <b>👩‍🏫 Persönliche Lernnotiz</b><br>
        Du entscheidest selbst, ob deine Notiz für die Lehrkraft sichtbar sein soll.
      ...
      <div style="display:flex;justify-content:flex-end;margin-top:12px">
        <button class="primary" onclick="saveJournalEntry()">Eintrag speichern</button>
      </div>
    </div>
      ...
      ${entries.length ? `<div class="list">${entries.map(e=>`
        <div class="card">
          <div style="display:flex;justify-content:space-between;gap:12px">
            <div>
              <strong>${esc(e.topic||"Lernmoment")}</strong>
              <small style="display:block;color:var(--muted);margin-top:3px">${dateText(e)}</small>
            </div>
        ...
        </div>`).join("")}</div>` : `<div class="empty">Noch keine Lernmomente festgehalten. Dein erster Eintrag kann ganz kurz sein.</div>`}
    </div>
    ${footer()}`;
}
