...
function wordcloudCloudHTML(items){
  ...
  return `<div class="wortwolke-cloud">${items.map((it,i)=>{
    ...
    const size=Math.round(16+ratio*34);
    return `<span class="wortwolke-word" style="font-size:${size}px;color:${palette[i%palette.length]}" title="${it.count}×">${esc(it.word)}</span>`;
  }).join("")}</div>`;
}
...
async function renderWortwolkeBoard(){
  ...
  <style>
    .wortwolke-cloud{display:flex;flex-wrap:wrap;gap:10px 18px;align-items:center;justify-content:center;padding:24px 10px;min-height:120px}
    .wortwolke-word{font-weight:800;line-height:1.1}
  </style>
  <div class="card">
...
function boardNoteHTML(p){
  ...
  return `<div class="pin-note" style="background:${noteColorBg(p.color)}">
    ...
    ${canDelete?`<button class="pin-note-delete" title="Notiz entfernen" onclick="deleteBoardPost('${p.id}')">✕</button>`:""}</div>
  </div>`;
}
...
async function render(){
  ...
  document.querySelectorAll(".nav-link").forEach(a=>a.classList.toggle("active",
    ...
    || (a.dataset.page==="pinnwand" && p.startsWith("pinnwand-"))));
