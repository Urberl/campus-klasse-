...
.journal-create-card{margin-top:16px}
.journal-library-card{margin-top:18px}
.journal-library-toolbar{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin-bottom:16px}
.journal-library-toolbar h2{margin:6px 0 3px}
.journal-library-toolbar p{margin:0;color:#68736f}
.journal-list{border:1px solid #dfe5e2;border-radius:14px;overflow:hidden;background:#fff}
...
.journal-list-header{padding:11px 15px;background:#f3f6f4;color:#68736f;font-size:11px;font-weight:800;letter-spacing:.08em}
.journal-list-item{min-height:58px;padding:8px 10px 8px 15px;border-top:1px solid #e7ebe9}
.journal-list-open{width:100%;display:grid;grid-template-columns:145px minmax(0,1fr);gap:14px;align-items:center;border:0;background:transparent;padding:7px 0;text-align:left;cursor:pointer;font:inherit}
.journal-list-open:hover .journal-list-title{text-decoration:underline}
.journal-list-date{font-size:13px;color:#68736f;white-space:nowrap}
.journal-list-title{font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.journal-list-pdf{justify-self:end}
.journal-empty{padding:28px 18px;text-align:center;color:#68736f}
.journal-empty strong{display:block;color:inherit;margin-bottom:4px}
@media(max-width:700px){
 .journal-library-toolbar{align-items:flex-start;flex-direction:column}
 .journal-list-header,.journal-list-item{grid-template-columns:88px minmax(0,1fr) 64px;gap:8px}
 .journal-list-open{grid-template-columns:1fr;gap:2px}
 .journal-list-pdf{padding:7px 8px;font-size:12px}
}
</style>`;
...
async function renderJournal(){
  let entries=[];
  try{
    entries=await getMyJournalEntries();
  }catch(e){
  ...
  return `${JOURNAL_LIST_STYLE}
    ...
    <div class="card journal-create-card">
      <div class="kicker">✍️ NEUER EINTRAG</div>
      <h2>Lernjournal führen</h2>
      <p>Halte deine Gedanken, Lernerfahrungen und nächsten Schritte fest.</p>
      <button class="primary" onclick="openJournalForm()">＋ Neuen Lernjournal-Eintrag erstellen</button>
    </div>
    ...
    <div class="card journal-library-card" id="journalLibraryPanel">
      <div class="journal-library-toolbar">
        <div>
          ...
          <p>${entries.length} gespeicherte${entries.length===1?"r":""} Lernjournal-Eintrag${entries.length===1?"":"̈e"}</p>
        </div>
        ${entries.length
          ? `<button class="secondary" onclick="printMyJournalPDF()">📄 Alle als PDF</button>`
          : ""}
      </div>
      ...
      ${entries.length
        ...
        : `<div class="journal-empty">
             <strong>Noch keine Lernjournal-Einträge</strong>
             <span>Deine Einträge werden hier als übersichtliche Liste gespeichert.</span>
           </div>`
      }
    </div>
...
async function openJournalEntry(id){
  try{
    ...
    if(!entry){
      toast("Der Eintrag wurde nicht gefunden.");
      return;
    }
    ...
    modal(`
      ...
      <div class="card" style="margin-top:14px">
        <div style="line-height:1.7">${esc(entry.text||"—").replace(/\n/g,"<br>")}</div>
      </div>
    ...
    console.error("Lernjournal öffnen:",e);
    toast("Der Eintrag konnte nicht geöffnet werden.");
  }
}
