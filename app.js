...
function showApp(){
  ...
  try{
    ...
    if(result&&typeof result.catch==="function") result.catch(err=>{
      console.error("Startseite konnte nicht gerendert werden:",err);
      const content=$("content");
      if(content) content.innerHTML=`<div class="card"><h3>Willkommen auf dem Campus.</h3><p>Die Anmeldung war erfolgreich. Einige Inhalte konnten noch nicht geladen werden.</p><button class="primary" onclick="window.location.hash='start';location.reload()">Neu laden</button></div>`;
    });
  }catch(err){
    console.error("Renderfehler nach Anmeldung:",err);
  }
}
...
async function render(){
 ...
 const pages={
   ...
   methoden:renderMethoden,impulse:renderImpulse,messung:renderMessung,
   lernpfad:renderLernpfad,lerncoaching:renderLerncoaching,ressourcen:renderRessourcen,
   forum:renderForum,projekte:renderProjekte,kompetenz:renderKompetenz,journal:renderJournal,
   praktikum:renderPraktikum,ki:renderKI,kalender:renderKalender,team:renderTeam
 };
 ...
 try{
   ...
   const content=$("content");
   if(content) content.innerHTML=`<div class="card"><h3>Die Seite konnte nicht geladen werden.</h3><p>Die Anmeldung ist weiterhin aktiv. Ein Bereich konnte nicht geladen werden.</p><div class="form-actions"><button class="primary" onclick="window.go('start')">← Zur Startseite</button><button class="secondary" onclick="location.reload()">Neu laden</button></div></div>`;
 }
...
window.addEventListener("unhandledrejection",e=>{
  ...
  const content=$("content");
  if(currentUser && content && !content.innerHTML.trim()){
    content.innerHTML=`<div class="card"><h3>Die App ist angemeldet, aber der Inhalt konnte nicht geladen werden.</h3><p>Bitte einmal neu laden.</p><button class="primary" onclick="location.reload()">Neu laden</button></div>`;
  }
});
  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      try{
        ...
        console.error("Benutzerprofil konnte nicht geladen werden:",e);
        // Fallback: authentication is valid even if the optional profile read is blocked.
        profile={
          uid:user.uid,
          email:user.email||"",
