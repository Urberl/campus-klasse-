...
function authError(err){
  const map={
     ...
     "auth/operation-not-allowed":"Die Anmeldung mit E-Mail und Passwort ist in Firebase nicht aktiviert.",
     "auth/network-request-failed":"Firebase ist momentan nicht erreichbar. Bitte Internetverbindung prüfen.",
     "auth/user-disabled":"Dieses Firebase-Konto wurde deaktiviert.",
     "auth/user-not-found":"Für diese E-Mail-Adresse wurde kein Firebase-Konto gefunden.",
     "auth/wrong-password":"Das Passwort ist nicht korrekt."
  };
  const code=err?.code||"unbekannt";
  $("authError").textContent=(map[code]||"Anmeldung konnte nicht durchgeführt werden.")+` [${code}]`;
}
function modal(html){$("modal").innerHTML=html;$("modalBackdrop").hidden=false}
function closeModal(){$("modalBackdrop").hidden=true}
/* =========================================================
   CAMPUS-NACHRICHTEN – Benachrichtigungsanzeige
   ========================================================= */
(function installMessageBellStyles(){
  if(document.getElementById("campus-message-bell-styles"))return;
  const style=document.createElement("style");
  style.id="campus-message-bell-styles";
  style.textContent=`
    .campus-message-bell{
      position:relative;display:inline-flex;align-items:center;justify-content:center;
      width:44px;height:44px;padding:0;border:1px solid var(--border);
      border-radius:12px;background:#fff;cursor:pointer;font-size:19px;
      box-shadow:0 1px 2px rgba(0,0,0,.04);transition:transform .15s ease,box-shadow .15s ease;
    }
    .campus-message-bell:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,0,0,.08)}
    .campus-message-bell.has-unread{border-color:rgba(0,92,160,.28)}
    .campus-message-badge{
      position:absolute;top:-5px;right:-5px;min-width:20px;height:20px;padding:0 5px;
      display:flex;align-items:center;justify-content:center;border-radius:999px;
      background:#c62828;color:#fff;font:700 11px/1 system-ui,sans-serif;
      border:2px solid #fff;
    }
    .campus-message-badge[hidden]{display:none}
  `;
  document.head.appendChild(style);
})();
function pageHead(k,h,p,actions=""){
  const messageBell=isApproved()?`<button type="button" class="campus-message-bell" onclick="openForumArea('messages')" aria-label="Nachrichten" title="Meine Nachrichten"><span aria-hidden="true">🔔</span><span class="campus-message-badge" hidden>0</span></button>`:"";
  return `<div class="page-head"><div><div class="kicker">${k}</div><h1>${h}</h1><p>${p}</p>
</div><div class="actions">${messageBell}${actions}</div></div>`
}
...
function repairKompetenzNavigation(){
  const links=[...document.querySelectorAll('a[href="#kompetenz"],a[data-page="kompetenz"]')];
  links.forEach(link=>{
    link.href="#kompetenz";
    link.onclick=(ev)=>{
      ev.preventDefault();
...
function showApp(){
  ...
  repairKompetenzNavigation();
  window.__messageUnreadIds=new Set();
  window.__messageNotificationReady=false;
  startMessageNotificationListener();
  render();
}
function clearListeners(){unsubscribers.forEach(u=>u&&u());unsubscribers=[]}

async function ensureProfile(user, displayName="", names={}){
  const ref=doc(db,"users",user.uid);
  const snap=await getDoc(ref);
  if(!snap.exists()){
    const firstName=String(names.firstName||"").trim();
    const lastName=String(names.lastName||"").trim();
    const fullName=String(displayName||user.displayName||"").trim();
    // Beim normalen Login kein unvollständiges Profil anlegen.
  ...
  try{
    ...
    return `${pageHead("LERNWERKSTATT","Lernressourcen-Bibliothek","Finde passende Lernmaterialien, digitale Angebote und externe Lernwege – zentral für Schüler und Lehrkräfte.",addButton)}
      <div class="card" style="background:var(--soft-green)">
        ...
        <p>TaskCards, KI-Lernangebote, Videos, ByCS/mebis und weitere Webseiten an einem Ort.</p>
...
// Registration requires first name and last name. The existing name field
// remains in the HTML for backward compatibility but is hidden and mirrored.
function setupRegistrationNameFields(){
  const form=$("registerForm");
  if(!form || $("registerFirstName")) return;
  const oldName=$("registerName");
  const email=$("registerEmail");
  const wrap=document.createElement("div");
  wrap.id="registrationNameFields";
  wrap.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
    <label>Vorname<input id="registerFirstName" type="text" maxlength="100" autocomplete="given-name" required placeholder="Vorname"></label>
    <label>Nachname<input id="registerLastName" type="text" maxlength="100" autocomplete="family-name" required placeholder="Nachname"></label>
  </div>`;
  if(email) form.insertBefore(wrap,email); else form.prepend(wrap);
  if(oldName){
    oldName.value="";
    const oldLabel=oldName.closest("label");
    if(oldLabel) oldLabel.hidden=true; else oldName.hidden=true;
  }
  const sync=()=>{
    if(oldName) oldName.value=[$("registerFirstName")?.value.trim(),$("registerLastName")?.value.trim()].filter(Boolean).join(" ");
  };
  $("registerFirstName").addEventListener("input",sync);
  $("registerLastName").addEventListener("input",sync);
}
setupRegistrationNameFields();
...
$("registerForm").addEventListener("submit",async e=>{
 ...
 if(!firstName||!lastName){$("authError").textContent="Bitte Vorname und Nachname eingeben.";return}
 ...
 try{
    ...
    await ensureProfile(cred.user,displayName,{firstName,lastName});
 }catch(err){console.error(err);authError(err)}
});
...
async function renderForum(){
 const active=(window.__forumArea||"forum");
 if(active==="messages") return await renderForumMessages();
 const posts=await getCollection("posts");
 return `${pageHead("GEMEINSCHAFT","Campus-Forum","Austauschen, fragen, helfen und gemeinsam lernen.",`<button class="primary" onclick="openPostForm()">＋ Beitrag schreiben</button>`)}
   <div class="card" style="margin-bottom:14px;padding:18px;background:linear-gradient(135deg,var(--soft-blue),var(--soft-green));border:1px solid rgba(0,92,160,.10)">
     <div style="display:flex;gap:14px;align-items:flex-start;flex-wrap:wrap">
       <div style="flex:1;min-width:240px"><div class="kicker">CAMPUS-FORUM</div><h2 style="margin:4px 0 6px">Gemeinsam im Austausch</h2><p style="margin:0;color:var(--muted)">Das Forum ist der offene Bereich für Fragen, Ideen, Hilfe und gemeinsame Diskussionen. Mit „Gefällt mir“ und Antworten könnt ihr euch gegenseitig unterstützen.</p></div>
       <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:stretch">
         <button class="chip" style="cursor:pointer;padding:12px 14px;text-align:left;background:#fff" onclick="openForumArea('forum')"><strong>💬 Forum</strong><br><small>Öffentlich austauschen</small></button>
         <button class="chip" style="cursor:pointer;padding:12px 14px;text-align:left;background:#fff" onclick="openForumArea('messages')"><strong>✉️ Nachrichten</strong><br><small>Persönlich schreiben</small></button>
       </div>
     </div>
   </div>
   <div class="card" style="margin-bottom:12px"><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px">
     <div style="padding:14px;border:1px solid var(--border);border-radius:14px;background:#fff"><div style="font-size:22px">💬</div><h3 style="margin:6px 0 4px">Öffentlicher Austausch</h3><p style="margin:0;color:var(--muted)">Beiträge der Campusgemeinschaft lesen, Fragen stellen, Ideen teilen und auf Beiträge antworten.</p></div>
     <div style="padding:14px;border:1px solid var(--border);border-radius:14px;background:#fff"><div style="font-size:22px">✉️</div><h3 style="margin:6px 0 4px">Persönliche Nachrichten</h3><p style="margin:0;color:var(--muted)">Direkte 1:1-Kommunikation mit einer Lehrkraft oder einem anderen freigeschalteten Campus-Mitglied.</p></div>
   </div></div>
   <div class="toolbar"><div class="chips"><span class="chip">Alle</span><span class="chip">Fragen</span><span class="chip">Infos</span><span class="chip">Ideen</span><span class="chip">Projekte</span><span class="chip">Praxis</span></div>
<input class="search" id="forumSearch" placeholder="Beiträge durchsuchen …"></div>
 <div class="list" id="forumList">${posts.map(postHTML).join("")||`<div class="empty"><strong>Noch keine Beiträge</strong><p>Schreibe den ersten Beitrag und starte den Austausch.</p><button class="secondary" onclick="openPostForm()">＋ Beitrag schreiben</button></div>`}</div>
  <div class="card" style="margin-top:12px;background:var(--soft-green)"><h3>🤝 Campus hilft</h3><p>Du kannst anderen bei einem Thema helfen? Teile dein Wissen und unterstütze die Campusgemeinschaft.</p><button class="secondary" style="margin-top:10px" onclick="openHelpForm()">Hilfe anbieten</button></div>${footer()}`;
}

function openForumArea(area="forum"){
  window.__forumArea=area==="messages"?"messages":"forum";
  if(area==="messages") window.__messagesSelected=null;
  window.go("forum");
}
...
async function getMessageUsers(){
  ...
  try{
    // Students may select approved teachers/admins.
    // Teachers/admins may select all approved users.
    // The queries deliberately match the Firestore rules so that students
    // do not have to read the complete users collection.
    let q;
    if(isTeacher()){
    ...
    console.error("Nachrichten-Empfänger:",e);
    return [];
  }
}

function messageUnread(m){
  if(!m || !currentUser)return false;
  if(m.recipientUid!==currentUser.uid)return false;
  const readBy=Array.isArray(m.readBy)?m.readBy:[];
  return !readBy.includes(currentUser.uid);
}

function updateMessageBadge(count){
  document.querySelectorAll(".campus-message-badge").forEach(badge=>{
    const n=Number(count||0);
    badge.textContent=n>99?"99+":String(n);
    badge.hidden=n<=0;
  });
  document.querySelectorAll(".campus-message-bell").forEach(btn=>{
    const n=Number(count||0);
    btn.classList.toggle("has-unread",n>0);
    btn.setAttribute("aria-label",n>0?`Nachrichten: ${n} ungelesen`:"Nachrichten");
    btn.title=n>0?`${n} ungelesene Nachricht${n===1?"":"en"}`:"Meine Nachrichten";
  });
}

async function refreshMessageBadge(messages=null){
  if(!isApproved()||!currentUser){updateMessageBadge(0);return 0;}
  try{
    const rows=messages||await getMyMessages();
    const unread=rows.filter(messageUnread).length;
    updateMessageBadge(unread);
    return unread;
  }catch(e){
    console.warn("Nachrichten-Badge:",e);
...
async function markMessagesRead(rows=[]){
  ...
  const unread=rows.filter(messageUnread);
  ...
  await Promise.all(unread.map(async m=>{
    try{
      ...
      if(!m.readBy.includes(currentUser.uid))m.readBy.push(currentUser.uid);
    }catch(e){
      console.warn("Nachricht konnte nicht als gelesen markiert werden:",m.id,e);
    }
  }));
}

function startMessageNotificationListener(){
  if(!db||!currentUser||!isApproved())return;
  try{
    const q=query(collection(db,"messages"),where("participantUids","array-contains",currentUser.uid),limit(100));
    const unsub=onSnapshot(q,snap=>{
      const rows=snap.docs.map(d=>({id:d.id,...d.data()}));
      const unread=rows.filter(messageUnread);
      const ids=new Set(unread.map(m=>m.id));
      const previous=window.__messageUnreadIds||new Set();
      if(window.__messageNotificationReady){
        const newlyUnread=unread.filter(m=>!previous.has(m.id));
        if(newlyUnread.length){
          const latest=newlyUnread.sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0))[0];
          const sender=latest?.senderName||"Campus-Mitglied";
          toast(`Neue Nachricht von ${sender}.`);
        }
      }
      window.__messageUnreadIds=ids;
      window.__messageNotificationReady=true;
      updateMessageBadge(unread.length);
    },err=>{
      console.warn("Nachrichten-Benachrichtigungen:",err);
      updateMessageBadge(0);
    });
    unsubscribers.push(unsub);
  }catch(e){
    console.warn("Nachrichten-Benachrichtigungen konnten nicht gestartet werden:",e);
  }
}
...
async function getMyMessages(){
  ...
  try{
    ...
    return rows;
  }catch(e){console.error("Nachrichten laden:",e);return []}
}
...
function personFullName(person,fallback="Campus-Mitglied"){
  if(!person)return fallback;
  const first=String(person.firstName||person.firstname||person.vorname||"").trim();
  const last=String(person.lastName||person.lastname||person.nachname||"").trim();
  if(first&&last)return `${first} ${last}`;
  if(first)return first;
  if(last)return last;
  const display=String(person.displayName||person.name||"").trim();
  if(display&&display!=="Campus-Mitglied")return display;
  return fallback;
}
...
async function renderForumMessages(){
  ...
  const selected=window.__messagesSelected;
  const conversations={};
  messages.forEach(m=>{
    const other=(m.participantUids||[]).find(uid=>uid!==currentUser.uid)||m.recipientUid||m.senderUid;
    if(!other)return;
    if(!conversations[other])conversations[other]=[];
    conversations[other].push(m);
  });
  const contacts=Object.entries(conversations).map(([uid,rows])=>({uid,rows:rows.sort((a,b)=>(a.createdAt?.seconds||0)-(b.createdAt?.seconds||0)),last:rows[rows.length-1]}));
  const selectedUid=selected||contacts[0]?.uid||"";
  const selectedRows=contacts.find(c=>c.uid===selectedUid)?.rows||[];
  if(selectedRows.length){
    await markMessagesRead(selectedRows);
  }
  await refreshMessageBadge(messages);
  const allPeople=[...users];
  if(profile) allPeople.push({id:currentUser.uid,...profile});
  const names=new Map(allPeople.map(u=>[u.id,personFullName(u,u.email||"Campus-Mitglied")]));
  messages.forEach(m=>{
    if(m.senderUid&&!names.has(m.senderUid))names.set(m.senderUid,m.senderName||"Campus-Mitglied");
    if(m.recipientUid&&!names.has(m.recipientUid))names.set(m.recipientUid,m.recipientName||"Campus-Mitglied");
  });
  const displayName=uid=>esc(names.get(uid)||"Campus-Mitglied");
  const messageSenderName=m=>esc(names.get(m.senderUid)||m.senderName||"Campus-Mitglied");

  return `${pageHead("GEMEINSCHAFT","Campus-Forum","Öffentlicher Austausch und persönliche Nachrichten an einem Ort.",`<button class="primary" onclick="openMessageForm()">＋ Neue Nachricht</button>`)}
  <div class="card" style="margin-bottom:14px;padding:18px;background:linear-gradient(135deg,var(--soft-blue),var(--soft-green));border:1px solid rgba(0,92,160,.10)">
    <div style="display:flex;gap:14px;align-items:flex-start;flex-wrap:wrap">
      <div style="flex:1;min-width:250px"><div class="kicker">PERSÖNLICHE KOMMUNIKATION</div><h2 style="margin:4px 0 6px">Meine Nachrichten</h2><p style="margin:0;color:var(--muted)">Hier kannst du direkt und vertraulich mit einzelnen freigeschalteten Campus-Mitgliedern kommunizieren. Nachrichten sind nur für die beteiligten Personen sichtbar.</p></div>
      <button class="chip" style="cursor:pointer;padding:12px 14px;background:#fff" onclick="openForumArea('forum')"><strong>💬 Zurück zum Forum</strong><br><small>Öffentlichen Austausch öffnen</small></button>
    </div>
  </div>
  <div class="grid grid-3">
    <div class="card" style="grid-column:span 1"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div><div class="kicker">DEINE KONTAKTE</div><h3 style="margin:3px 0">Unterhaltungen</h3></div><span class="pill">${contacts.length}</span></div>
      ${contacts.map(c=>`<button type="button" class="list-item" style="width:100%;text-align:left;border:1px solid var(--border);background:#fff;cursor:pointer;margin-top:8px;border-radius:12px" onclick="selectMessageConversation('${c.uid}')"><div><strong>${displayName(c.uid)}</strong><small>${esc(c.last?.subject||c.last?.text||"")}</small></div><span class="pill">${c.rows.length}</span></button>`).join("")||`<div class="empty" style="margin-top:10px"><strong>Noch keine Unterhaltungen</strong><p>Starte eine persönliche Nachricht.</p><button class="secondary" onclick="openMessageForm()">＋ Neue Nachricht</button></div>`}
    </div>
    <div class="card" style="grid-column:span 2">
      ${selectedUid?`<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap"><div><div class="kicker">VERTRAULICHER AUSTAUSCH</div><h3 style="margin:3px 0">Nachrichten mit ${displayName(selectedUid)}</h3></div><button class="secondary" onclick="openMessageForm('${selectedUid}')">↩ Antworten</button></div><div class="list" style="max-height:420px;overflow:auto;margin-top:12px">${selectedRows.map(m=>`<div class="list-item"><div><strong>${messageSenderName(m)}</strong><small>${esc(m.subject||"")} · ${messageTime(m.createdAt)}</small><div style="margin-top:6px;white-space:pre-wrap">${esc(m.text||"")}</div></div></div>`).join("")||`<div class="empty">Noch keine Nachrichten.</div>`}</div>`:`<div class="empty"><strong>Noch keine Unterhaltung ausgewählt.</strong><p>Wähle links eine Unterhaltung oder starte eine neue Nachricht.</p><button class="primary" onclick="openMessageForm()">＋ Neue Nachricht</button></div>`}
    </div>
  </div>${footer()}`;
}

function selectMessageConversation(uid){window.__messagesSelected=uid;window.__forumArea="messages";window.go("forum")}

async function openMessageForm(preselectedUid=""){
  if(!isApproved()){toast("Nur freigeschaltete Nutzer können Nachrichten senden.");return}
  const users=await getMessageUsers();
  if(!users.length){toast("Es wurden keine freigeschalteten Nutzer gefunden.");return}
  const options=users.map(u=>`<option value="${esc(u.id)}" ${u.id===preselectedUid?"selected":""}>${esc(u.displayName||u.name||u.email||u.id)}${u.role==="teacher"?" · Lehrkraft":u.role==="admin"?" · Admin":" · Schüler/in"}</option>`).join("");
  modal(`<button class="modal-close" type="button" onclick="closeModal()">×</button><div class="kicker">CAMPUS-NACHRICHT</div><h2>Neue Nachricht</h2><p>Die Nachricht ist nur für dich und den ausgewählten Empfänger sichtbar.</p><div class="form">
<label>Empfänger<select id="msgRecipient">${options}</select></label>
<label>Betreff<input id="msgSubject" maxlength="180" placeholder="Worum geht es?"></label>
<label>Nachricht<textarea id="msgText" rows="7" maxlength="5000" placeholder="Deine Nachricht …"></textarea></label>
<div class="form-actions"><button class="secondary" type="button" onclick="closeModal()">Abbrechen</button><button class="primary" type="button" onclick="sendCampusMessage()">Nachricht senden</button></div></div>`);
}

async function sendCampusMessage(){
  const recipientUid=$("msgRecipient")?.value||"";
  const subject=$("msgSubject")?.value.trim()||"";
  const textValue=$("msgText")?.value.trim()||"";
  if(!recipientUid||recipientUid===currentUser.uid){toast("Bitte einen anderen Empfänger auswählen.");return}
  if(!textValue){toast("Bitte eine Nachricht eingeben.");return}
  try{
    const users=await getMessageUsers();
    const recipient=users.find(u=>u.id===recipientUid);
    if(!recipient){toast("Der ausgewählte Empfänger ist nicht mehr verfügbar.");return}
    const senderName=personFullName({...profile},currentUser.displayName||currentUser.email||"Campus-Mitglied");
    const recipientName=personFullName(recipient,recipient.email||"Campus-Mitglied");
    await addDoc(collection(db,"messages"),{senderUid:currentUser.uid,senderName,recipientUid,recipientName,subject,text:textValue,participantUids:[currentUser.uid,recipientUid],readBy:[currentUser.uid],createdAt:serverTimestamp(),emailNotificationRequested:true});
    closeModal();window.__forumArea="messages";window.__messagesSelected=recipientUid;await render();toast("Nachricht gesendet.");
  }catch(e){console.error("Campus-Nachricht:",e);toast(e?.code==="permission-denied"?"Nachrichten sind in den Firebase-Regeln noch nicht freigegeben.":"Nachricht konnte nicht gesendet werden.")}
}
...
${isTeacher()?`<div class="form-actions" style="margin-top:10px"><button class="secondary" onclick="deleteCampusEntry('projects','${p.id}','Projekt')">Löschen</button></div>`:""}
...
window.openForumArea=openForumArea;
window.openMessageForm=openMessageForm;
window.sendCampusMessage=sendCampusMessage;
...
async function addNews(){
  ...
  if(!isTeacher()){
    toast("Nur freigeschaltete Lehrkräfte können News veröffentlichen.");
    return;
  }
  ...
  if(!title||!textValue){
    toast("Bitte Überschrift und News eingeben.");
    return;
  }

  const payload={
    authorUid:currentUser.uid,
    authorName:profile.displayName||currentUser.email||"Lehrkraft",
    title:title,
    text:textValue,
    createdAt:serverTimestamp(),
    updatedAt:serverTimestamp()
  ...
  try{
    // Eine eigene Dokument-ID vermeidet Probleme mit alten Formular-/DOM-Zuständen
    // und stellt sicher, dass die Firestore-create-Regel eindeutig greift.
    ...
    console.error("News veröffentlichen:",e);
    if(e?.code==="permission-denied"){
      toast("Veröffentlichen nicht erlaubt. Bitte die aktuelle firestore.rules veröffentlichen.");
    }else{
      toast("News konnte nicht veröffentlicht werden.");
    }
  }
}
...
if(profile.status === "blocked"){
  ...
  showApp();
         ...
         console.error("Campus-Profil konnte nach erfolgreicher Firebase-Anmeldung nicht geladen werden:",e);
         showAuth();
         const code=e?.code||"unbekannt";
         $("authError").textContent=`Anmeldung war erfolgreich, aber das Campus-Profil konnte nicht gelesen werden. [${code}]`;
       }
    });
  }catch(e){
