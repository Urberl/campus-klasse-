...
/* =========================================================
   CAMPUSKLASSE – STABILE MODULARCHITEKTUR
   Module werden erst beim Öffnen geladen.
   ========================================================= */
const CAMPUS_MODULES={
  ...
  resilienz:{label:"🌱 Resilienz & Respressi",route:"resilienz",ready:true},
...
async function renderRessourcenRoute(){
  try{
    const m=await import("./modules/lernressourcen.js");
    return await m.renderRessourcen();
  }catch(e){
    console.error("Lernressourcen-Modul:",e);
    return moduleErrorPage("📚 Lernressourcen","modules/lernressourcen.js",e);
  }
}
async function renderLernpfadRoute(){
  try{
    const m=await import("./modules/lernpfad.js");
    return await m.renderLernpfad();
  }catch(e){
    console.error("Lernpfad-Modul:",e);
    return moduleErrorPage("🧭 Persönlicher Lernpfad","modules/lernpfad.js",e);
  }
}
function moduleErrorPage(title,file,error){
  return `${pageHead("CAMPUS-MODUL",title,"Das Modul ist momentan nicht erreichbar.",`<button class="secondary" onclick="go('start')">← Startseite</button>`)}
  <div class="card"><h3>Die Campus-App selbst läuft.</h3>
  <p>Es gibt nur ein Problem mit dem einzelnen Modul.</p>
  <div class="notice"><b>Datei:</b> ${esc(file)}<br><small>${esc(error?.message||"Unbekannter Fehler")}</small></div></div>${footer()}`;
}
...
async function render(){
  ...
  const pages={
    ...
    methoden:()=>modulePlaceholder("🧰 Lernmethoden"),
    impulse:()=>modulePlaceholder("💡 Lernimpulse"),
    lernstand:()=>modulePlaceholder("📊 Lernstandsmessung"),
    lerncoaching:()=>modulePlaceholder("💬 Lerncoaching"),
  ...
  try{
    ...
    console.error("Campus render error:",e);
    $("content").innerHTML=`<div class="card"><h3>Die Seite konnte nicht geladen werden.</h3><p>${esc(e?.message||"Unbekannter Fehler")}</p><button class="secondary" onclick="go('start')">← Zur Startseite</button></div>`;
  }
  $("sidebar").classList.remove("open");
}
function modulePlaceholder(title){
  return `${pageHead("CAMPUS-MODUL",title,"Dieser Bereich ist vorbereitet und wird anschließend als eigenes Modul entwickelt.",`<button class="secondary" onclick="go('start')">← Startseite</button>`)}
  <div class="card"><span class="badge">🧩 VORBEREITET</span><h2>${title}</h2><p>Die Master-App bleibt stabil; dieser Bereich wird später unabhängig ergänzt.</p></div>${footer()}`;
}
