...
async function loadFirebase(){
  ...
  firebaseReadyPromise = Promise.all([
    ...
    window.CampusFirebase={
      db,currentUser,collection,doc,addDoc,setDoc,updateDoc,deleteDoc,getDoc,getDocs,
...
function showApp(){
  ...
  if(window.CampusFirebase) window.CampusFirebase.currentUser=currentUser;
  render();
}
...
async function renderStart(){
  let tasks=[],posts=[],calendar=[];
  try{[tasks,posts,calendar]=await Promise.all([getCollection("tasks","deadline",false),getCollection("posts"),getCollection("calendar","date",false)])}catch(e){console.warn("Startdaten:",e)}
  const on=tasks.filter(x=>x.status==="green").length;
  return `${pageHead("DEIN DIGITALER CAMPUS","Gemeinsam lernen. Zukunft gestalten.","Dein Campus verbindet Lernen, Projekte, Praxis, Persönlichkeit und Gemeinschaft.",`<button class="primary" onclick="go('kompass')">🧭 Mein Campus-Kompass</button>`)}
  <section class="hero"><div><span class="badge">🏫 CAMPUSKLASSE 26/27</span><h1>Dein Campus.</h1><p>Ein Ort, an dem du deinen Lernweg selbstständig gestalten, mit anderen zusammenarbeiten und aus Wissen Handlung machen kannst.</p></div><div class="actions"><button class="primary" onclick="go('lernwerkstatt')">🛠️ Lernen starten</button><button class="secondary" onclick="go('journal')">📓 Lernjournal</button></div></section>
  <div class="section-head" style="margin-top:22px"><div><div class="kicker">DEIN LERNWEG</div><h2>Persönlich lernen</h2><p>Lernen wird sichtbar, steuerbar und zunehmend selbstständig.</p></div></div>
  <div class="grid grid-4">${tile("🧭","Persönlicher Lernpfad","Ziele setzen, Lernschritte planen, Fortschritt erkennen.","lernpfad")}${tile("🛠️","Lernwerkstatt","Fachwissen, Methoden, Ressourcen und KI.","lernwerkstatt")}${tile("🧩","Kompetenzwerkstatt","Verstehen → Anwenden → Reflektieren → Gestalten.","kompetenz")}${tile("📓","Lernjournal","Lernprozess dokumentieren und nächsten Schritt festlegen.","journal")}</div>
  <div class="section-head" style="margin-top:22px"><div><div class="kicker">MENSCH & ENTWICKLUNG</div><h2>Stark bleiben</h2><p>Persönlichkeit, Resilienz und Beziehungen gehören zum Lernen.</p></div></div>
  <div class="grid grid-4">${tile("🌱","Resilienz & Respressi","Atem, Regulation, Ressourcen und kurze Impulse.","resilienz")}${tile("💬","Lerncoaching","Individuelle Begleitung und Kontakt zu einer Lehrkraft.","lerncoaching")}${tile("💬","Campus-Forum","Fragen stellen, Erfahrungen teilen und helfen.","forum")}${tile("🧰","Lernmethoden","Methoden für Planung, Lernen, Zusammenarbeit und Reflexion.","lernwerkstatt")}</div>
  <div class="section-head" style="margin-top:22px"><div><div class="kicker">WISSEN WIRD HANDLUNG</div><h2>Projekte & Praxis</h2><p>Deeper Learning verbindet schulisches Wissen mit echten Fragen.</p></div></div>
  <div class="grid grid-3">${tile("🚀","Projekte","Projektteams, Arbeitspakete, Status und Ergebnisse.","projekte")}${tile("🏢","Praxis & Partnerschaften","Praktikum, Praxisaufträge, Mentoring und Praxisprojekte.","praktikum")}${tile("🤖","KI-Innovationslabor","KI-Kompetenz, reale Probleme und Innovationspartnerschaften.","ki")}</div>
  <div class="section-head" style="margin-top:22px"><div><div class="kicker">ORIENTIERUNG</div><h2>Mein Campus im Blick</h2></div></div>
  <div class="grid grid-4">${tile("🗓️","Campus-Kalender","Termine, Meilensteine und wichtige Ereignisse.","kalender")}${tile("🎯","Kompetenzprofil","Kompetenzen sichtbar entwickeln und reflektieren.","kompetenz")}${tile("👥","Team & SQ","Gemeinsam Qualität und Campusentwicklung gestalten.","team")}<div class="card stat"><b>${tasks.length}</b><span>Arbeitspakete</span><small>${on} aktuell auf Kurs</small></div></div>
  <div class="grid grid-2" style="margin-top:12px"><div class="card"><h3>📢 Campus-News</h3><div class="list">${posts.filter(p=>p.type==="info").slice(0,3).map(p=>`<div class="list-item"><div><strong>${esc(p.text)}</strong><small>${fmtDate(p.createdAt)}</small></div><span class="pill">Info</span></div>`).join("")||`<div class="empty">Noch keine News.</div>`}</div></div><div class="card"><h3>🗓️ Nächste Termine</h3><div class="list">${calendar.slice(0,3).map(c=>`<div class="list-item"><div><strong>${esc(c.title)}</strong><small>${esc(c.date)}</small></div><span class="pill green">Termin</span></div>`).join("")||`<div class="empty">Noch keine Termine.</div>`}</div></div>
  ${footer()}`;
}
...
async function renderResilienz(){
 return `${pageHead("MENSCH & ENTWICKLUNG","Resilienz & Respressi","Ein freiwilliger Raum für kurze Impulse, Regulation und den bewussten Umgang mit Herausforderungen.",`<button class="secondary" onclick="go('start')">← Campus</button>`)}
 <div class="grid grid-3">
  <div class="card tile"><span class="emoji">🌬️</span><strong>Atem & Regulation</strong><small>Kurze Atemimpulse zum Ankommen und Regulieren.</small></div>
  <div class="card tile"><span class="emoji">🧘</span><strong>Ruhe & Distanzierung</strong><small>Mini-Übungen für Abstand, Fokus und Ruhe.</small></div>
  <div class="card tile"><span class="emoji">😄</span><strong>Leichtigkeit & Humor</strong><small>Impulse für Perspektivwechsel und Leichtigkeit.</small></div>
  <div class="card tile"><span class="emoji">💪</span><strong>Eigene Ressourcen</strong><small>Menschen, Orte, Fähigkeiten und Erfahrungen, die Kraft geben.</small></div>
  <div class="card tile"><span class="emoji">🤝</span><strong>Beziehungen & Unterstützung</strong><small>Unterstützung erkennen, annehmen und weitergeben.</small></div>
  <div class="card tile"><span class="emoji">🔄</span><strong>Mit Herausforderungen umgehen</strong><small>Situation betrachten, Möglichkeiten finden, nächsten Schritt wählen.</small></div>
 </div>
 <div class="card" style="margin-top:12px;background:var(--soft-green)"><h3>🎧 Respressi</h3><p>Hier können die bereits entwickelten kurzen Respressi-Impulse und Audioangebote direkt eingebunden werden.</p><span class="pill green">freiwillig · kurz · alltagstauglich</span></div>${footer()}`;
}
...
/* =========================================================
   CAMPUSKLASSE MASTER – MODULREGISTRY
   Alle vorgesehenen Bereiche sind hier bereits festgelegt.
   Die einzelnen Module werden anschließend Schritt für
   Schritt entwickelt.
   ========================================================= */

/* =========================================================
   MASTER – sichere Modul-Ladung
   Optionale Module werden erst beim Öffnen geladen.
   Dadurch kann ein fehlendes Modul niemals die komplette
   Campus-App beim Start blockieren.
   ========================================================= */
async function renderRessourcenRoute(){
  try{
    const mod=await import("./modules/lernressourcen.js");
    return await mod.renderRessourcen();
  }catch(e){
    console.error("Lernressourcen-Modul:",e);
    return renderModuleLoadError("📚 Lernressourcen","modules/lernressourcen.js",e);
  }
}
async function renderLernpfadRoute(){
  try{
    const mod=await import("./modules/lernpfad.js");
    return await mod.renderLernpfad();
  }catch(e){
    console.error("Lernpfad-Modul:",e);
    return renderModuleLoadError("🧭 Persönlicher Lernpfad","modules/lernpfad.js",e);
  }
}
function renderModuleLoadError(title,file,error){
  return `${pageHead("CAMPUS-MODUL",title,"Das Modul konnte nicht geladen werden.",`<button class="secondary" onclick="go('start')">← Campus</button>`)}
  <div class="card">
    <h3>Das Campus-Grundsystem funktioniert.</h3>
...
const CAMPUS_MODULES={
  lernpfad:{label:"🧭 Persönlicher Lernpfad",status:"ready"},
  lernressourcen:{label:"📚 Lernressourcen",status:"ready"},
  lernjournal:{label:"📓 Lernjournal",status:"existing"},
  lernmethoden:{label:"🧰 Lernmethoden",status:"planned"},
  lernimpulse:{label:"💡 Lernimpulse",status:"planned"},
  lernstand:{label:"📊 Lernstandsmessung",status:"planned"},
  lerncoaching:{label:"💬 Lerncoaching",status:"planned"},
  resilience:{label:"🌱 Resilienz & Respressi",status:"existing"},
  kompetenz:{label:"🧩 Kompetenzwerkstatt",status:"existing"},
  forum:{label:"💬 Campus-Forum",status:"existing"},
  projekte:{label:"🚀 Projekte",status:"existing"},
  praxis:{label:"🏢 Praxis & Partnerschaften",status:"existing"},
  ki:{label:"🤖 KI-Innovationslabor",status:"existing"},
  kalender:{label:"🗓️ Campus-Kalender",status:"existing"},
  kompetenzprofil:{label:"🎯 Kompetenzprofil",status:"planned"},
  team:{label:"👥 Team & SQ",status:"existing"}
};

const CAMPUS_MODULE_ROUTES={
  lernpfad:"lernpfad",lernressourcen:"ressourcen",lernjournal:"journal",
  lernmethoden:"methoden",lernimpulse:"impulse",lernstand:"lernstand",
  lerncoaching:"lerncoaching",resilience:"resilienz",kompetenz:"kompetenz",
  forum:"forum",projekte:"projekte",praxis:"praktikum",ki:"ki",
  kalender:"kalender",kompetenzprofil:"kompetenzprofil",team:"team"
};


function renderModulePlaceholder(key){
  const m=CAMPUS_MODULES[key]||{label:"Campus-Bereich"};
  return `${pageHead("CAMPUS",m.label,"Dieser Bereich ist vorbereitet und wird als eigenes Modul Schritt für Schritt ausgebaut.",`<button class="secondary" onclick="go('start')">← Campus</button>`)}
  <div class="card" style="background:var(--soft-green)">
    <span class="badge">🧩 MODUL VORBEREITET</span>
    <h2>${esc(m.label)}</h2>
    <p>Die Schnittstelle ist bereits Teil der Master-App. Die Inhalte werden entwickelt, ohne andere Module zu verändern.</p>
  </div>${footer()}`;
}

const pages={start:renderStart,kompass:renderKompass,lernwerkstatt:renderLernwerkstatt,ressourcen:renderRessourcenRoute,forum:renderForum,projekte:renderProjekte,kompetenz:renderKompetenz,journal:renderJournal,praktikum:renderPraktikum,ki:renderKI,kalender:renderKalender,team:renderTeam,resilienz:renderResilienz,lernpfad:renderLernpfadRoute,methoden:()=>renderModulePlaceholder('lernmethoden'),impulse:()=>renderModulePlaceholder('lernimpulse'),lernstand:()=>renderModulePlaceholder('lernstand'),lerncoaching:()=>renderModulePlaceholder('lerncoaching'),kompetenzprofil:()=>renderModulePlaceholder('kompetenzprofil')};
