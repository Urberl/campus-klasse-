...
function updateTeacherTeamNav(){
  ...
  links.forEach(link=>{
    const label=link.querySelector("strong,span,.nav-label")||link;
    if(label && !label.querySelector("svg")) {
      const nodes=[...label.childNodes].filter(n=>n.nodeType===3);
      if(nodes.length) nodes[nodes.length-1].textContent="Lehrkräfte Klassenteam";
      else if(label===link) link.setAttribute("aria-label","Lehrkräfte Klassenteam");
    }
...
async function renderLernwerkstatt(){
  const resources=[
         ...
         ["❓","Fragen & Hilfe","Wenn du nicht weiterkommst: fragen und teilen.","fragenhilfe"]
  ];
...
async function renderFragenHilfe(){
 const posts=(await getCollection("posts")).filter(p=>p.type==="question");
 return `${pageHead("SELBSTSTÄNDIG LERNEN","Fragen & Hilfe","Wenn du nicht weiterkommst: stelle eine konkrete Frage oder unterstütze andere beim Lernen.",`<button class="primary" onclick="openPostForm('question')">＋ Frage stellen</button>`)}
 <div class="grid grid-2" style="margin-bottom:12px">
  <div class="card"><div class="kicker">❓ FRAGEN</div><h3>Du kommst nicht weiter?</h3><p>Formuliere dein Problem möglichst konkret. So können Mitschülerinnen, Mitschüler und Lehrkräfte gezielt helfen.</p><button class="primary" onclick="openPostForm('question')">＋ Frage stellen</button></div>
  <div class="card" style="background:var(--soft-green)"><div class="kicker">🤝 CAMPUS HILFT</div><h3>Du kannst helfen?</h3><p>Schau dir offene Fragen an und teile dein Wissen, eine Idee oder einen Lösungsweg.</p><button class="secondary" onclick="openHelpForm()">Hilfe anbieten</button></div>
 </div>
 <div class="card"><h3>Offene Fragen</h3><div class="list">${posts.map(postHTML).join("")||`<div class="empty"><strong>Noch keine Fragen</strong>Stelle die erste Frage.</div>`}</div></div>${footer()}`;
}
