/* CAMPUS APP VERSION: 5.0 – LERNRESSOURCEN-BIBLIOTHEK */
...
async function loadFirebase(){
  ...
  firebaseReadyPromise = Promise.all([
    ...
    ({getStorage,ref:storageRef,uploadBytes,getDownloadURL}=storageMod);
    if(!app) app=initializeApp(firebaseConfig);
    if(!auth) auth=getAuth(app);
    if(!db) db=getFirestore(app);
    if(!window._campusStorage) window._campusStorage=getStorage(app);
    return true;
  });
...
.resource-link-tile{text-decoration:none;color:inherit}
...
async function renderLernwerkstatt(){
 ...
 <div class="grid grid-4">${resources.map(x=>x[3]?`<a class="card tile resource-link-tile" href="#${x[3]}"><span class="emoji">${x[0]}</span><strong>${x[1]}</strong><small>${x[2]}</small></a>`:`<div class="card tile"><span class="emoji">${x[0]}</span><strong>${x[1]}</strong><small>${x[2]}</small></div>`).join("")}</div>
...
/* ============================================================
   ...
   Firestore: learningResources
   Storage: lernressourcen/{uid}/{timestamp}_{filename}
   ============================================================ */
...
const RESOURCE_TYPES={
 ...
 link:{icon:"🔗",label:"Externer Link"}
};
const RESOURCE_EXT=/\.(pdf|doc|docx|mp3|mp4|jpg|jpeg|png)$/i;
let activeResourceFilter="all";

function resourceTypeInfo(type){return RESOURCE_TYPES[type]||RESOURCE_TYPES.link}
function resourceUrlValid(url){
 try{const u=new URL(url);return u.protocol==="https:"||u.protocol==="http:"}
 catch(e){return false}
}
function resourceAllowedFile(file){
 if(!file)return false;
 return RESOURCE_EXT.test(file.name);
}
function resourceSizeOk(file){
...
function resourceCard(r){
 ...
 const target=r.fileUrl||r.url;
 const action=target
   ? `<a class="primary button" href="${esc(target)}" target="_blank" rel="noopener noreferrer">${r.type==="file"?"Öffnen":"Öffnen →"}</a>`
   : `<span class="pill">Kein Link hinterlegt</span>`;
 const tags=(r.tags||[]).slice(0,5).map(t=>`<span class="pill">${esc(t)}</span>`).join("");
 return `<article class="card resource-card" data-resource-type="${esc(r.type)}">
   ...
   <div class="resource-meta">
    ...
    ${r.fileName?`<span>📎 ${esc(r.fileName)}</span>`:""}
   </div>
...
function updateResourceForm(){
 ...
 const fileWrap=$("rFileWrap"), urlWrap=$("rUrlWrap"), provider=$("rProvider");
 if(fileWrap) fileWrap.hidden=type!=="file";
 if(urlWrap) urlWrap.hidden=type==="file";
 if(provider) provider.textContent=type==="taskcard"?"TaskCard":type==="ai"?"z. B. fobizz":"Externe Lernressource";
}
...
function openResourceForm(){
 ...
 <div class="form">
  <label>Art
   <select id="rType" onchange="updateResourceForm()">
  ...
  <label>Titel<input id="rTitle" placeholder="z. B. Soziale Identität – Lernstation" required></label>
  <label>Beschreibung<textarea id="rDescription" rows="3" placeholder="Was bietet diese Lernressource?"></textarea></label>
  <label>Fach / Lernbereich<input id="rSubject" placeholder="z. B. Pädagogik & Psychologie 11 · Soziale Identität"></label>
  <label>Passende Kompetenz (optional)<input id="rCompetency" placeholder="z. B. Ich kann …"></label>
  <label>Geschätzte Lernzeit<input id="rDuration" placeholder="z. B. 20 Minuten"></label>
  <div id="rUrlWrap">
   <label>Link <span id="rProvider"></span><input id="rUrl" type="url" placeholder="https://…"></label>
  </div>
  <div id="rFileWrap" hidden>
   <label>Datei (PDF, Word, MP3, MP4, JPG, PNG)<input id="rFile" type="file" accept=".pdf,.doc,.docx,.mp3,.mp4,.jpg,.jpeg,.png"></label>
   <small>Maximal 50 MB pro Datei.</small>
  </div>
  <label>Schlagworte<input id="rTags" placeholder="z. B. PP11, Üben, Fallbeispiel"></label>
  <div class="notice">💡 TaskCard- und fobizz-Ressourcen werden nur über ihren Link eingebunden und benötigen keinen Speicherplatz in deiner Materialbibliothek.</div>
  <div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" id="saveResourceBtn" onclick="saveResource()">Speichern</button></div>
 </div>`);
 updateResourceForm();
}
...
async function saveResource(){
 ...
 if(type==="file"){
   ...
   if(!resourceAllowedFile(file)){toast("Dateityp nicht erlaubt. PDF, Word, MP3, MP4, JPG oder PNG.");return}
   if(!resourceSizeOk(file)){toast("Die Datei ist größer als 50 MB.");return}
 }else if(!url||!resourceUrlValid(url)){
   toast("Bitte einen gültigen http/https-Link eingeben.");return
 }

 const btn=$("saveResourceBtn"); if(btn) btn.disabled=true;
 try{
   await loadFirebase();
   let fileUrl="",fileName="",fileSize=0,mimeType="";
   if(type==="file"){
     const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");
     const storagePath=`lernressourcen/${currentUser.uid}/${Date.now()}_${safe}`;
     const ref=storageRef(window._campusStorage,storagePath);
   ...
   await addDoc(collection(db,"learningResources"),{
     title,type,description,subject,competency,duration,tags,
     url:type==="file"?"":url,
   ...
   console.error("Lernressource:",e);
   toast(`Speichern nicht möglich (${e?.code||"Fehler"}).`);
...
function setResourceFilter(type){
 ...
 document.querySelectorAll("[data-resource-filter]").forEach(b=>b.classList.toggle("active",b.dataset.resourceFilter===type));
 filterResources();
}
...
async function renderRessourcen(){
 let resources=[],error="";
 try{resources=await getLearningResources()}catch(e){
   console.error(e);error=e?.code?`${e.code}: ${e.message||""}`:(e?.message||String(e));
 }
 const teacher=typeof isTeacher==="function"?isTeacher():false;
 return `${pageHead("LERNWERKSTATT","Lernressourcen","Alles, was dich beim Lernen unterstützt – Dateien, TaskCards, fobizz und weitere Lernangebote.",teacher?`<button class="primary" onclick="openResourceForm()">＋ Lernressource</button>`:`<a class="secondary button" href="#lernwerkstatt">← Lernwerkstatt</a>`)}
 <div class="card resource-library-intro"><span class="badge">📚 LERNRESSOURCEN-BIBLIOTHEK</span><h2 style="margin:8px 0 4px">Ein Ort für dein Weiterlernen</h2><p>Finde Lernangebote unabhängig davon, ob sie als Datei, TaskCard, KI-Angebot oder externer Link vorliegen.</p></div>
 <div class="resource-toolbar"><input id="resourceSearch" class="search" placeholder="🔎 Lernressource suchen …" oninput="filterResources()"><div class="chips">
  <button class="chip active" data-resource-filter="all" onclick="setResourceFilter('all')">Alle</button>
  ${Object.entries(RESOURCE_TYPES).map(([k,v])=>`<button class="chip" data-resource-filter="${k}" onclick="setResourceFilter('${k}')">${v.icon} ${v.label}</button>`).join("")}
 </div></div>
 ${error?`<div class="card"><b>Lernressourcen konnten nicht geladen werden.</b><p>${esc(error)}</p></div>`:""}
 <div id="resourceGrid" class="resource-grid">${resources.length?resources.map(resourceCard).join(""):`<div class="empty" style="grid-column:1/-1"><strong>Noch keine Lernressourcen</strong><br>${teacher?"Lege die erste TaskCard, fobizz-Ressource oder Datei an.":"Hier werden bald Lernangebote für dich bereitstehen."}</div>`}</div>
 ${footer()}`;
}
...
Object.assign(window,{openResourceForm,updateResourceForm,saveResource,setResourceFilter,filterResources});
