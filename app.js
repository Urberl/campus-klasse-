...
async function renderForumMessages(){
  ...
  const names=new Map(allPeople.map(u=>[u.id,u.displayName||u.name||u.email||u.id]));
  messages.forEach(m=>{
    const uid=(m.participantUids||[]).find(x=>x!==currentUser.uid);
    if(uid&&!names.has(uid))names.set(uid,m.recipientName||m.senderName||uid);
  });
  ...
  <div class="grid grid-3">
    ...
    <div class="card" style="grid-column:span 2">
      ${selectedUid?`<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap"><div><div class="kicker">VERTRAULICHER AUSTAUSCH</div><h3 style="margin:3px 0">Nachrichten mit ${displayName(selectedUid)}</h3></div><button class="secondary" onclick="openMessageForm('${selectedUid}')">↩ Antworten</button></div><div class="list" style="max-height:420px;overflow:auto;margin-top:12px">${selectedRows.map(m=>`<div class="list-item"><div><strong>${esc(m.senderName||m.authorName||"Campus-Mitglied")}</strong><small>${esc(m.subject||"")} · ${messageTime(m.createdAt)}</small><div style="margin-top:6px;white-space:pre-wrap">${esc(m.text||"")}</div></div></div>`).join("")||`<div class="empty">Noch keine Nachrichten.</div>`}</div>`:`<div class="empty"><strong>Noch keine Unterhaltung ausgewählt.</strong><p>Wähle links eine Unterhaltung oder starte eine neue Nachricht.</p><button class="primary" onclick="openMessageForm()">＋ Neue Nachricht</button></div>`}
    </div>
  </div>${footer()}`;
}
...
async function sendCampusMessage(){
  ...
  try{
    ...
    const senderName=profile?.displayName||currentUser.email||"Campus-Mitglied";
    const recipientName=recipient.displayName||recipient.name||recipient.email||"Campus-Mitglied";
