...
async function renderLernwerkstatt(){
 ...
 {title:"Konkret lernen & üben",items:[
...
async function renderResilienz(){
 ...
 <div class="card" style="background:var(--soft-orange);margin-bottom:16px;text-align:center;padding:22px">
 ...
 .res-card{text-align:left;min-height:150px;padding-top:20px;cursor:pointer;transition:.25s cubic-bezier(.2,.8,.2,1);color:var(--ink);font:inherit;border-radius:18px;border:1px solid var(--line,#e2eaf0);box-shadow:0 2px 8px rgba(23,56,79,.05);position:relative;overflow:hidden}
 .res-card::before{content:"";position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,var(--blue-dark),var(--green));transform:scaleX(0);transform-origin:left;transition:transform .3s ease}
 .res-card:hover{transform:translateY(-4px);box-shadow:0 14px 28px rgba(23,56,79,.14);border-color:transparent}
 .res-card:hover::before{transform:scaleX(1)}
 .res-card h3{font-size:15px;color:var(--blue-dark);margin:4px 0 6px;font-weight:800}
 .res-card p{font-size:12px;color:var(--muted);line-height:1.5;margin:0}
 .res-icon{font-size:31px;margin-bottom:8px}.res-tag{display:inline-block;margin-top:8px;border-radius:999px}
 ...
 .stress-sign{border:1px solid var(--line,#ddd);border-radius:14px;padding:12px;background:#fff;transition:.2s}
 ...
 .treasure{min-height:260px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;color:var(--ink);font:inherit;border-radius:18px;transition:.25s}
 .treasure:hover{transform:translateY(-4px);box-shadow:0 14px 28px rgba(23,56,79,.14)}
 .treasure h2{font-size:18px;color:var(--blue-dark);margin:0 0 6px;font-weight:800}
 ...
 .treasure-count{margin-top:6px;border-radius:999px}
 ...
 <h3>${x.title}</h3><p>${x.desc}</p><span class="pill res-tag">${x.tag}</span>
 </button>`).join("")}</div>
 </div>
...
async function openResilienzSchatzkiste(){
 const favorites=await getMyResilienzSchaetze();
 const cards=favorites.map(f=>{const d=resilienzSkillData(f.skillId);return`<button class="card res-card"onclick="startResilienzSkill('${f.skillId}')"><h3>${d[1]}</h3><p>${d[2]}</p></button>`}).join("");
