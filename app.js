...
function openLernpfadCheckinForm(){
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    ...
    <div class="form">
      ...
      <div class="grid grid-2" style="gap:8px">${Object.entries(lernpfadStrategies).map(([key,s])=>`<button type="button" class="secondary lp-strategy-btn" data-strategy="${key}" onclick="selectLernpfadStrategy('${key}')" style="text-align:left">${s.icon} ${esc(s.label)}</button>`).join("")}</div>
