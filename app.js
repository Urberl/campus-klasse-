...
onclick="go('forum')">Campus-Forum</button></div></section>
 ...
 <div class="grid grid-4">
    ...
    ${tile("       ","fpA","Praxisaufträge, Fragen und Projekte.","praktikum")}
...
async function renderPraktikum(){
  let practice=[], impressions=[], cloudEntries=[];
  ...
  const counts={};
  cloudEntries.forEach(e=>String(e.words||"").split(/[,\n;]+/).map(w=>w.trim().toLowerCase()).filter(w=>w.length>=3).forEach(w=>counts[w]=(counts[w]||0)+1));
  const words=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,35), max=Math.max(1,...words.map(x=>x[1]));
  const cards=practice.map(p=>`<div class="card"><span class="pill ${p.state==="offen"?"orange":"green"}">${esc(p.state||"geplant")}</span><h3>${esc(p.title||"Praxisauftrag")}</h3><p>${esc(p.text||"")}</p><small>${esc(p.date||"")}</small></div>`).join("");
  return `${pageHead("SCHULE ↔ PRAXIS","fpA","Praxisaufträge, Reflexion, Fragen, Projekte und KI-Innovationspartnerschaften.",`<button class="primary" onclick="openPracticeForm()">＋ Praxisauftrag</button>`)}
  <div class="grid grid-4">
    <a class="card tile" href="#praktikum"><span class="emoji">🧭</span><strong>Praxisaufträge</strong><small>Aufträge aus dem Praktikum und Reflexion.</small></a>
    <a class="card tile" href="#praxisfragen"><span class="emoji">❓</span><strong>Fragen aus der Praxis</strong><small>Eigenständiges Tool für Praxisfragen.</small></a>
    <a class="card tile" href="#praxisprojekte"><span class="emoji">🚀</span><strong>Projekte in der Praxis</strong><small>Eigenständiges Tool für Praxisprojekte.</small></a>
    <a class="card tile" href="#ki"><span class="emoji">🤖</span><strong>KI-Innovationspartnerschaften</strong><small>Gemeinsam mit Praxispartnern entwickeln.</small></a>
  </div>
  <div class="card" style="margin-top:12px"><h3>📚 Meine Praxis-Eindrücke</h3><div class="list">${mine.map(i=>`<div class="list-item"><div><strong>${esc(i.practiceTitle||"Praxisauftrag")}</strong><small>${esc(i.date||fmtDate(i.createdAt))} · ${esc(i.title||"Mein Eindruck")}</small></div></div>`).join("")||`<div class="empty">Noch keine Praxiseindrücke gespeichert.</div>`}</div></div>
  <div class="card" style="margin-top:12px"><h3>☁️ Wortwolke aus der Praxis</h3><div style="min-height:160px;display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:10px 16px;padding:20px;background:var(--soft,#f7f7f5);border-radius:12px">${words.length?words.map(([w,c])=>`<span style="font-weight:700;font-size:${16+Math.round(c/max*24)}px">${esc(w)}</span>`).join(""):`<div class="empty">Noch keine Begriffe gesammelt.</div>`}</div></div>
  <div class="card" style="margin-top:12px"><h3>📥 Formulare & Downloads</h3><div class="grid grid-2"><div class="card"><strong>Bewertungsbogen des Praktikumsbetriebes</strong><p>Vorgegebenes Formular.</p></div><div class="card"><strong>Wochenbericht</strong><p>Vorgegebenes Formular.</p></div></div></div>
  ${footer()}`;
}
async function renderPraxisFragen(){
  let posts=[];
  try{posts=await getCollection("posts","createdAt",true)}catch(e){console.error("Praxisfragen:",e)}
  const questions=posts.filter(p=>p.type==="question");
  return `${pageHead("fpA · AUSTAUSCH","Fragen aus der Praxis","Ein eigener Bereich für Fragen, die aus Erfahrungen im Praktikum entstehen.",`<button class="primary" onclick="openPostForm('question')">＋ Frage aus der Praxis</button>`)}
  <div class="card"><div class="notice"><strong>💬 Fragen aus der Praxis</strong><p style="margin-bottom:0">Hier gehören Beobachtungen, Unsicherheiten und fachliche Fragen aus der Praxis hinein.</p></div>
  <div class="list" style="margin-top:12px">${questions.map(postHTML).join("")||`<div class="empty"><strong>Noch keine Praxisfragen.</strong><p>Stelle die erste Frage aus deinem Praktikum.</p></div>`}</div></div>${footer()}`;
}

async function renderPraxisProjekte(){
  let projects=[];
  try{projects=await getCollection("projects","createdAt",true)}catch(e){console.error("Praxisprojekte:",e)}
  return `${pageHead("fpA · PROJEKTE","Projekte in der Praxis","Ein eigener Bereich für Projekte aus dem Praktikum und der Zusammenarbeit mit Praxispartnern.",`<button class="primary" onclick="openProjectForm()">＋ Praxisprojekt</button>`)}
  <div class="card"><div class="notice"><strong>🚀 Projekte in der Praxis</strong><p style="margin-bottom:0">Dieser Bereich ist eigenständig und nicht Teil des Kompetenznetzwerks.</p></div>
  <div class="grid grid-3" style="margin-top:12px">${projects.map(p=>`<div class="card"><div class="status-card">${statusDot(p.status||"green")}<div><h3>${esc(p.title||"Praxisprojekt")}</h3><p>${esc(p.goal||"")}</p></div></div><small>${esc(p.team||"")} · Praxispartner: ${esc(p.partner||"—")}</small><div style="margin-top:8px"><span class="pill">${Number(p.progress||0)}%</span></div></div>`).join("")||`<div class="empty"><strong>Noch keine Praxisprojekte.</strong><p>Lege das erste Projekt aus der Praxis an.</p></div>`}</div></div>${footer()}`;
}
