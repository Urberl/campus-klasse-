...
async function renderPraktikum(){
  ...
  try{practice=await getCollection("practice","createdAt",true)}catch(e){console.warn("fpA Praxisaufträge:",e)}
  try{impressions=await getCollection("practiceImpressions","createdAt",true)}catch(e){console.warn("fpA Eindrücke:",e)}
  try{
    const posts=await getCollection("posts","createdAt",true);
    questions=posts.filter(p=>p.type==="question").slice(0,6);
  }catch(e){console.warn("fpA Fragen:",e)}
  try{projects=await getCollection("projects","createdAt",true)}catch(e){console.warn("fpA Projekte:",e)}
  try{cloudEntries=await getCollection("practiceWordCloud","createdAt",true)}catch(e){console.warn("fpA Wortwolke:",e)}
  ...
  const cloudWords=Object.entries(wordCounts)
    .sort((a,b)=>b[1]-a[1])
    .slice(0,40);
  const maxCount=Math.max(1,...cloudWords.map(x=>x[1]));
  ...
  const questionRows=questions.map(q=>`
    <div class="list-item">
      <div>
        <strong>${esc(q.authorName||"Schüler/in")}</strong>
  ...
  const projectRows=projects.slice(0,6).map(p=>`
    <div class="list-item">
      <div>
        <strong>${esc(p.title||"Projekt")}</strong>
        <small>${esc(p.team||"")} · Praxispartner: ${esc(p.partner||"—")}</small>
      </div>
  ...
  const cloudHtml=cloudWords.length
    ? cloudWords.map(([word,count])=>{
        const size=16+Math.round((count/maxCount)*24);
  ...
  <style>
    ...
    @media(max-width:900px){
      .fpa-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
    }
  ...
  <div class="fpa-grid">
    ...
    <section class="card fpa-tile" onclick="document.getElementById('fpaDownloads').scrollIntoView({behavior:'smooth'})">
      ...
      <p>Bewertungsbogen des Praktikumsbetriebes und Wochenbericht zentral bereitstellen.</p>
    ...
    <section class="card fpa-tile" onclick="document.getElementById('fpaCloud').scrollIntoView({behavior:'smooth'})">
      ...
      <h2>Praxis-Eindrücke</h2>
      <p>Erfahrungen und Eindrücke als gemeinsame Wortwolke sichtbar machen.</p>
    ...
    <section class="card fpa-tile" onclick="location.hash='ki'">
      ...
      <p>KI-Ideen gemeinsam mit Praxispartnern entwickeln und erproben.</p>
      <span class="pill" style="margin-top:14px">Öffnen →</span>
    </section>
  ...
  <section id="fpaCloud" class="card fpa-section">
    <div class="page-head" style="margin-bottom:10px">
      <div>
        ...
        <p>Häufig genannte Begriffe erscheinen größer.</p>
      </div>
      <div class="actions">
    ...
    <div class="fpa-downloads">
      <div class="fpa-download-card">
        ...
        <div style="flex:1">
          ...
          <small>Vorgegebenes Formular</small>
        </div>
  ...
  <section id="fpaFragen" class="card fpa-section">
    ...
    <div class="list">
      ${questionRows||`<div class="empty">Noch keine Fragen vorhanden.</div>`}
    </div>
  </section>
  ...
  <section id="fpaProjekte" class="card fpa-section">
    ...
    <div class="list">
      ${projectRows||`<div class="empty">Noch keine Projekte vorhanden.</div>`}
    </div>
  </section>
  ...
  <section class="card fpa-section" style="border-left:4px solid var(--accent,#7b9b82)">
    <div class="page-head" style="margin-bottom:8px">
      <div>
        ...
        <h2>KI-Innovationspartnerschaften</h2>
        <p>Praxisprobleme erkennen, KI-Ideen entwickeln und gemeinsam mit Praxispartnern erproben.</p>
      </div>
...
function fpADisplayDate(v){
  if(!v)return"—";
  if(v?.seconds)return new Date(v.seconds*1000).toLocaleDateString("de-DE");
  return String(v);
}
  ...
  modal(`
    ...
    <div class="form">
      <label>Meine Begriffe
        <textarea id="pwcWords" rows="4" placeholder="z. B. Teamarbeit, Verantwortung, Kommunikation, Geduld"></textarea>
      </label>
      <div class="form-actions">
...
async function savePracticeWordCloud(){
  ...
  if(!words){toast("Bitte mindestens einen Begriff eingeben.");return}
  try{
    ...
    closeModal();
    await render();
    toast("Deine Eindrücke wurden gespeichert.");
  }catch(e){
...
function downloadFpaDocument(type){
  // Hier werden später die beiden offiziellen PDF-Dateien hinterlegt.
  const documents={
    bewertung:"",
    wochenbericht:""
  };
...
function openPracticeImpressionForm(practiceId){
  getCollection("practice","createdAt",true).then(items=>{
    const p=items.find(x=>x.id===practiceId);
    ...
    modal(`
      ...
      <div class="form">
        <label>Titel meines Eindrucks
          <input id="piTitle" placeholder="z. B. Meine Beobachtung">
        </label>
        <label>Datum
          <input id="piDate" type="date" value="${new Date().toISOString().slice(0,10)}">
        </label>
        <label>Mein Eindruck / meine Beobachtung
          <textarea id="piText" rows="6" placeholder="Was habe ich beobachtet? Was hat mich beschäftigt? Was habe ich daraus gelernt?"></textarea>
        </label>
        <label>Was nehme ich für meinen weiteren Lernweg mit?
          <textarea id="piNext" rows="4" placeholder="Frage, Erkenntnis, nächster Schritt …"></textarea>
        </label>
        <div class="form-actions">
          <button class="secondary" onclick="closeModal()">Abbrechen</button>
          <button class="primary" onclick="addPracticeImpression('${esc(p.id)}','${esc(p.title||"Praxisauftrag")}')">Speichern</button>
        </div>
      </div>`);
  }).catch(e=>{
    console.error("Praxisauftrag:",e);
    toast("Praxisauftrag konnte nicht geladen werden.");
  });
}

async function addPracticeImpression(practiceId,practiceTitle){
  const text=$("piText")?.value.trim()||"";
  if(!text){toast("Bitte deinen Eindruck eintragen.");return}
  ...
  try{
    await addDoc(collection(db,"practiceImpressions"),{
      ...
      practiceId,
      practiceTitle,
      title:$("piTitle")?.value.trim()||"Mein Eindruck",
      date:$("piDate")?.value||new Date().toISOString().slice(0,10),
      text,
      next:$("piNext")?.value.trim()||"",
      createdAt:serverTimestamp()
    });
    closeModal();
    await render();
    toast("Praxiseindruck gespeichert.");
  }catch(e){
    console.error("Praxiseindruck:",e);
    toast("Praxiseindruck konnte nicht gespeichert werden.");
  }
}

async function openPracticeImpression(id){
  try{
    ...
    console.error("Praxiseindruck öffnen:",e);
    toast("Eintrag konnte nicht geöffnet werden.");
  }
}
