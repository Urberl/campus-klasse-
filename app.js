...
async function renderStart(){
  const safe=async(promise,fallback=[])=>{
    ...
    ])}catch(e){console.warn("Startseiten-Daten:",e);return fallback}
  };
  const [tasks,projects,posts,calendar]=await Promise.all([
    safe(getCollection("tasks","deadline",false)),
    safe(getCollection("projects")),
    safe(getCollection("posts")),
    safe(getCollection("calendar","date",false))
  ]);
  ...
  <div class="grid grid-4">
    ...
    ${tile("🗂️","fpA","Praxisaufträge und Reflexion.","praktikum")}
    ${tile("🤖","KI-Innovationslabor","KI-Ideen und Innovationspartnerschaften.","ki")}
  </div>
  <div class="grid grid-3" style="margin-top:12px">
    <div class="card stat"><b>${tasks.length}</b><span>Arbeitspakete</span></div>
    <div class="card stat"><b>${on}</b><span>auf Kurs</span></div>
    <div class="card stat"><b>${currentUser?1:0}</b><span>dein Zugang ist aktiv</span></div>
  </div>
  <div class="grid grid-2" style="margin-top:12px">
    ...
    <div class="card"><h3>📅 Nächste Termine</h3><div class="list">${calendar.slice(0,3).map(c=>`<div class="list-item"><div><strong>${esc(c.title)}</strong><small>${esc(c.date)}</small></div><span class="pill green">Termin</span></div>`).join("")||`<div class="empty">Noch keine Termine.</div>`}</div></div>
  </div>${footer()}`;
}
...
async function renderResilienz(){
  const tools=[
    ['🌬️','Atemtechnik','Ruhige Atmung und bewusste Ausatmung.'],
    ['👣','Boden spüren','Über Körper und Sinne wieder ankommen.'],
    ['🪟','Distanzierung','Eine belastende Situation kurz von außen betrachten.'],
    ['🪶','Leichtigkeit','Den inneren Druck für einen Moment reduzieren.'],
    ['⚡','Bewegungsreset','Kurz bewegen und danach bewusst zurückkehren.'],
    ['🌱','Ressourcen aktivieren','Eine eigene Stärke oder hilfreiche Erfahrung aktivieren.'],
    ['🤝','Verbindung','Über soziale Unterstützung Stabilität gewinnen.'],
    ['🎯','Fokus','Den kleinsten machbaren nächsten Schritt finden.']
  ];
  return `${pageHead('RESILIENZ & RESPRESSI','Resilienz & Respressi','Kleine Übungen, die du direkt im Schulalltag ausprobieren kannst.',`<button class="primary" onclick="resilienzImpuls()">🎴 Impuls für mich</button>`)}
  <style>.res-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.res-card{text-align:left;min-height:170px;cursor:pointer}.res-icon{font-size:30px;margin-bottom:8px}@media(max-width:900px){.res-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:600px){.res-grid{grid-template-columns:1fr}}</style>
  <div class="card"><div class="kicker">DEIN MOMENT</div><h2>Was brauchst du gerade?</h2><p>Wähle eine kurze Übung und probiere sie direkt aus.</p><div class="res-grid">${tools.map((x,i)=>`<button class="card res-card" onclick="startResilienzUebung(${i})"><div class="res-icon">${x[0]}</div><h3>${x[1]}</h3><p>${x[2]}</p><span class="pill">2 Min.</span></button>`).join('')}</div></div>
  <div class="card" style="margin-top:16px"><div class="kicker">MEINE RESILIENZ-WOCHE</div><h2>📈 Was hat mir gutgetan?</h2><p>Markiere eine Strategie, die du diese Woche ausprobiert hast.</p><div class="form-actions">${['Bewegung','Pause','Atemübung','Kontakt','Humor','Natur','Musik'].map(x=>`<button class="secondary" onclick="resilienzCheckin('${x}')">${x}</button>`).join('')}</div></div>${footer()}`;
}
function resilienzImpuls(){startResilienzUebung(Math.floor(Math.random()*8))}
function startResilienzUebung(i){
 const a=[['🌬️','Atemtechnik','Atme ruhig ein und verlängere sanft die Ausatmung. Wiederhole das einige Male.'],['👣','Boden spüren','Stell beide Füße auf den Boden und spüre Druck, Temperatur und Kontakt.'],['🪟','Distanzierung','Was würdest du einem guten Freund in deiner Situation raten?'],['🪶','Leichtigkeit','Was wäre heute ein ausreichend guter nächster Schritt?'],['⚡','Bewegungsreset','Steh auf, strecke dich und bewege Schultern und Arme.'],['🌱','Ressource','Welche Stärke hat dir schon einmal geholfen?'],['🤝','Verbindung','Wer könnte dir gerade guttun oder dich unterstützen?'],['🎯','Fokus','Formuliere den kleinsten nächsten Schritt, den du jetzt tun kannst.']];
 const x=a[i]||a[0]; modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">RESPRESSI · ÜBUNG</div><h2>${x[0]} ${x[1]}</h2><p style="font-size:18px;line-height:1.55">${x[2]}</p><div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button><button class="primary" onclick="closeModal();toast('Übung ausprobiert – nimm kurz wahr, was sich verändert hat.')">✓ Geschafft</button></div>`);
}
function resilienzCheckin(name){localStorage.setItem('campus_resilienz_'+name,new Date().toISOString());toast(name+': für diese Woche eingetragen.');}
...
async function render(){
  ...
  // A previous modal must never block the application after navigation.
  ...
  content.innerHTML=`<div class="card" style="min-height:180px;display:flex;align-items:center;justify-content:center"><strong>Bereich wird geladen …</strong></div>`;

  try{
    const html=await Promise.race([
      Promise.resolve().then(()=>fn()),
      new Promise((_,reject)=>setTimeout(()=>reject(new Error("Das Modul antwortet nicht innerhalb von 8 Sekunden.")),8000))
    ]);
    content.innerHTML=html;
  }catch(e){
    console.error("Campus-Seitenfehler:",e);
    content.innerHTML=`<div class="card"><h2>Der Bereich konnte nicht geladen werden.</h2><p>Die Navigation der Campus-App funktioniert weiterhin.</p><div class="notice"><b>Technischer Hinweis:</b> ${esc(e?.message||"Unbekannter Fehler")}</div><div class="form-actions"><button class="primary" onclick="go('start')">← Zur Startseite</button><button class="secondary" onclick="render()">↻ Erneut versuchen</button></div></div>`;
  }
...
window.go=p=>{
  const target=String(p||"start").replace(/^#/,"");
...
// ES modules do not expose lexical functions to inline HTML handlers.
// This delegated listener makes every sidebar/tile navigation reliable.
document.addEventListener("click",e=>{
  const link=e.target.closest?.("a.nav-link, a.tile");
  if(!link)return;
  const href=link.getAttribute("href")||"";
  if(!href.startsWith("#"))return;
  e.preventDefault();
  e.stopPropagation();
  window.go(href.slice(1));
});
