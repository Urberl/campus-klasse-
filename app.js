...
async function renderJournal(){
  ...
  return `${pageHead("REFLEXION","Lernjournal","Dein Lernweg, Reflexionen und nächste Schritte.",`${teacherButton}<button class="primary" onclick="openJournalForm()">＋ Eintrag</button>`)}
  <div class="card journal-list-card">
    <div class="journal-list-head">
      <div><span class="badge">🌱 DEIN LERNWEG</span><h2>Meine Reflexionen</h2><p>Klicke auf einen Eintrag, um die vollständige Reflexion zu öffnen.</p></div>
      ${data.length?`<button class="secondary" onclick="printMyJournalPDF()">📄 PDF</button>`:""}
    </div>
    ${data.length?`<div class="journal-compact-list">${data.map((j,i)=>`
      <button class="journal-list-row" onclick="openJournalEntry('${j.id}')">
        <span class="journal-row-date">${esc(fmtDate(j.createdAt))}</span>
        <span class="journal-row-title">${esc(j.title||"Reflexion")}</span>
        <span class="journal-row-mood">${esc(j.mood||"💭")}</span>
        <span class="journal-row-arrow">›</span>
      </button>`).join("")}</div>`
      :`<div class="empty"><strong>Noch kein Eintrag</strong><p>Starte mit einer kurzen Reflexion.</p><button class="primary" onclick="openJournalForm()">＋ Erste Reflexion</button></div>`}
  </div>${footer()}`;
}
