...
function ensureRegistrationNameFields(){
  const form=$("registerForm");
  if(!form || $("registerFirstName"))return;
  const email=$("registerEmail");
  const wrap=document.createElement("div");
  wrap.id="registrationNameFields";
  wrap.innerHTML=`<div class="grid grid-2" style="gap:12px">
...
async function sendCampusMessage(){
  ...
  try{
    ...
    await addDoc(collection(db,"messages"),{senderUid:currentUser.uid,senderName,recipientUid,recipientName,subject,text:textValue,participantUids:[currentUser.uid,recipientUid],createdAt:serverTimestamp(),emailNotificationRequested:true});
