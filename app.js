...
async function renderJournal(){
  ...
  }catch(e){console.error("Lernjournal laden:",e)}

  const teacherButton=isTeacher()?`<button class="secondary" onclick="openTeacherJournalOverview()">👩‍🏫 Schüler-Lernjournale</button>`:"";

  const styles=`<style id="journal-two-tiles-style">
.journal-two-tiles{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;margin-top:16px}
.journal-tile{min-height:250px;padding:26px;border:1px solid #dfe5e2;border-radius:18px;background:#fff;display:flex;flex-direction:column;box-shadow:0 6px 20px rgba(0,0,0,.05)}
.journal-tile-icon{font-size:38px;margin-bottom:12px}.journal-tile h2{margin:0 0 8px}.journal-tile p{color:#68736f;line-height:1.55}.journal-tile-grow{flex:1}.journal-tile button{width:100%;margin-top:14px}
.journal-library{margin-top:18px}.journal-library-head{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:12px}.journal-library-head p{margin:4px 0 0;color:#68736f}
.journal-list{border:1px solid #dfe5e2;border-radius:12px;overflow:hidden;background:#fff}.journal-list-head,.journal-list-row{display:grid;grid-template-columns:150px 1fr 90px;align-items:center;gap:12px}
.journal-list-head{padding:10px 14px;background:#f4f7f5;color:#68736f;font-size:11px;font-weight:800;letter-spacing:.08em}
.journal-list-row{min-height:54px;padding:8px 10px 8px 14px;border-top:1px solid #e8ecea}.journal-open{display:grid;grid-template-columns:150px 1fr;gap:12px;align-items:center;border:0;background:transparent;text-align:left;cursor:pointer;font:inherit;padding:7px 0;min-width:0}
.journal-date{font-size:13px;color:#68736f;white-space:nowrap}.journal-title{font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.journal-pdf{justify-self:end;white-space:nowrap}
@media(max-width:760px){.journal-two-tiles{grid-template-columns:1fr}.journal-tile{min-height:220px}.journal-list-head,.journal-list-row{grid-template-columns:88px 1fr 68px;gap:8px}.journal-open{grid-template-columns:1fr;gap:2px}.journal-pdf{padding:7px 8px;font-size:12px}}
</style>`;

  const rows=data.map(j=>`<div class="journal-list-row">
<button class="journal-open" onclick="openJournalEntry('${j.id}')"><span class="journal-date">${esc(fmtDate(j.createdAt))}</span><span class="journal-title">${esc(j.title||"Reflexion")}</span></button>
<button class="secondary journal-pdf" onclick="printSingleJournalPDF('${j.id}')">📄 PDF</button></div>`).join("");

  return `${styles}${pageHead("REFLEXION","Lernjournal","Dein Lernweg, Reflexionen und nächste Schritte.",teacherButton)}
<div class="journal-two-tiles">
<section class="journal-tile"><div class="journal-tile-icon">✍️</div><h2>Lernjournal erstellen</h2><p>Halte fest, was du gelernt hast, was dich beschäftigt und welche nächsten Schritte du gehen möchtest.</p><div class="journal-tile-grow"></div><button class="primary" onclick="openJournalForm()">＋ Neuen Eintrag erstellen</button></section>
<section class="journal-tile"><div class="journal-tile-icon">📚</div><h2>Lernjournal-Bibliothek</h2><p>Alle deine bisherigen Lernjournal-Einträge. Öffne einen Eintrag oder gib ihn als PDF aus.</p><div class="journal-tile-grow"></div><button class="secondary" onclick="openJournalLibrary()">Bibliothek öffnen →</button></section>
</div>
<div class="card journal-library" id="journalLibraryPanel">
<div class="journal-library-head"><div><span class="badge">📚 BIBLIOTHEK</span><h2 style="margin:7px 0 0">Meine Lernjournal-Einträge</h2><p>${data.length} gespeicherte Einträge</p></div>${data.length?`<button class="secondary" onclick="printMyJournalPDF()">📄 Alle als PDF</button>`:""}</div>
${data.length?`<div class="journal-list"><div class="journal-list-head"><span>DATUM</span><span>TITEL</span><span>PDF</span></div>${rows}</div>`:`<div class="empty"><strong>Noch keine Einträge</strong><p>Erstelle oben deinen ersten Lernjournal-Eintrag.</p></div>`}
</div>${footer()}`;
}
...
async function openJournalEntry(id){
  try{
    const snap=await getDocs(query(collection(db,"journal"),where("uid","==",currentUser.uid),limit(100)));
    const j=snap.docs.map(d=>({id:d.id,...d.data()})).find(x=>x.id===id);
    if(!j){toast("Der Eintrag wurde nicht gefunden.");return}
    modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">LERNJOURNAL · ${esc(fmtDate(j.createdAt))}</div><h2>${esc(j.title||"Reflexion")}</h2>${j.mood?`<div style="font-size:26px;margin:8px 0">${esc(j.mood)}</div>`:""}<div class="card"><p>${esc(j.text||"—").replace(/\n/g,"<br>")}</p></div><div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button><button class="secondary" onclick="printSingleJournalPDF('${j.id}')">📄 PDF</button></div>`);
...
async function printSingleJournalPDF(id){
  ...
  }catch(e){console.error(e);toast("PDF-Ausgabe nicht möglich.")}
}

async function printMyJournalPDF(){
  try{
    const snap=await getDocs(query(collection(db,"journal"),where("uid","==",currentUser.uid),limit(100)));
    const entries=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));
    openJournalPrintWindow("Meine Lernjournal-Bibliothek",[
