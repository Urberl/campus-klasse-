...
async function renderForum(){
 ...
 try{
  ...
  posts.sort((a,b)=>{
   const time=v=>v?.seconds ? v.seconds*1000 : (v ? Date.parse(v)||0 : 0);
   return time(b.createdAt)-time(a.createdAt);
  });
 }catch(e){
  console.error("Forum konnte nicht geladen werden:",e);
  return `${pageHead("GEMEINSCHAFT","Campus-Forum","Gemeinsam denken, fragen, austauschen und unterstützen.",`<button class="primary" onclick="openPostForm()">＋ Beitrag schreiben</button>`)}<div class="card"><h3>⚠️ Forum-Daten konnten nicht geladen werden</h3><p>Die App ist erreichbar, aber Firestore liefert die Forum-Daten nicht.</p><pre style="white-space:pre-wrap;font-size:12px">${esc(e?.code||e?.message||String(e))}</pre></div>${footer()}`;
 }
...
async function render(){
 ...
 try{$("content").innerHTML=await fn(); if(p==="forum"){const q=$("forumSearch"); if(q) q.addEventListener("input",filterForumPosts)}}catch(e){console.error(e);$("content").innerHTML=`<div class="card"><h3>⚠️ Die Seite konnte nicht geladen werden.</h3><p>${esc(e?.code||e?.message||String(e))}</p></div>`}
...
function filterForumPosts(){const q=$("forumSearch");const term=(q?.value||"").trim().toLowerCase();document.querySelectorAll("#forumList .forum-post").forEach(el=>{el.style.display=!term||el.textContent.toLowerCase().includes(term)?"":"none"})}
function exposeAppActions(){Object.assign(window,{go,openPostForm,addPost,likePost,commentPost,focusComment,deletePost,openHelpForm,openTaskForm,addTask,openProjectForm,addProject,openJournalForm,addJournal,openCompetenceForm,addCompetence,openPracticeForm,addPractice,openCalendarForm,addCalendar,filterForumPosts})}
async function init(){
  exposeAppActions();
  if(!configReady){
    showAuth();
