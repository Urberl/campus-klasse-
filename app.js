...
async function renderPraktikum(){
  ...
  // All optional fpA collections fail silently. This guarantees that the fpA
  // page itself still renders even if a new Firestore collection is not yet
  // configured.
  try{practice=await getCollection("practice","createdAt",true)}catch(e){console.warn("fpA practice",e)}
  try{impressions=await getCollection("practiceImpressions","createdAt",true)}catch(e){console.warn("fpA impressions",e)}
  try{
    const posts=await getCollection("posts","createdAt",true);
    questions=posts.filter(p=>p.type==="question").slice(0,6);
  }catch(e){console.warn("fpA questions",e)}
  try{projects=await getCollection("projects","createdAt",true)}catch(e){console.warn("fpA projects",e)}
  try{cloudEntries=await getCollection("practiceWordCloud","createdAt",true)}catch(e){console.warn("fpA cloud",e)}

  const mine=impressions.filter(i=>i.uid===currentUser.uid);
  const counts={};

  cloudEntries.forEach(e=>{
    String(e.words||"").split(/[,\n;]+/)
      .map(w=>w.trim().toLowerCase())
      .filter(w=>w.length>=3 && w.length<=28)
      .forEach(w=>counts[w]=(counts[w]||0)+1);
  });

  const words=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,40);
  const max=Math.max(1,...words.map(x=>x[1]));

  // IMPORTANT: fpA-specific handlers are installed only when fpA is opened.
  // Nothing new is evaluated during authentication or on the start page.
  window.CampusFpa={
    addImpression:async function(practiceId,practiceTitle){
      const body=document.getElementById("fpaImpressionText")?.value.trim()||"";
      if(!body){toast("Bitte deinen Eindruck eintragen.");return}
      try{
        await addDoc(collection(db,"practiceImpressions"),{
          uid:currentUser.uid,
          studentName:profile?.displayName||currentUser?.email||"Schüler/in",
          practiceId,practiceTitle,
          title:document.getElementById("fpaImpressionTitle")?.value.trim()||"Mein Eindruck",
          date:document.getElementById("fpaImpressionDate")?.value||new Date().toISOString().slice(0,10),
          text:body,
          next:document.getElementById("fpaImpressionNext")?.value.trim()||"",
          createdAt:serverTimestamp()
        });
        closeModal();await render();toast("Praxiseindruck gespeichert.");
      }catch(e){console.error(e);toast("Praxiseindruck konnte nicht gespeichert werden.")}
    },

    openImpressionForm:async function(practiceId){
      const p=practice.find(x=>x.id===practiceId);
      if(!p){toast("Praxisauftrag nicht gefunden.");return}
      modal(`
        <button class="modal-close" onclick="closeModal()">×</button>
        <div class="kicker">PRAXIS · EINDRUCK</div>
        <h2>${esc(p.title||"Praxisauftrag")}</h2>
        <p>${esc(p.text||"")}</p>
        <div class="form">
          <label>Titel meines Eindrucks
            <input id="fpaImpressionTitle" placeholder="z. B. Meine Beobachtung">
          </label>
          <label>Datum
            <input id="fpaImpressionDate" type="date" value="${new Date().toISOString().slice(0,10)}">
          </label>
          <label>Mein Eindruck / meine Beobachtung
            <textarea id="fpaImpressionText" rows="6" placeholder="Was habe ich beobachtet? Was hat mich beschäftigt? Was habe ich daraus gelernt?"></textarea>
          </label>
          <label>Was nehme ich für meinen weiteren Lernweg mit?
            <textarea id="fpaImpressionNext" rows="4" placeholder="Frage, Erkenntnis, nächster Schritt …"></textarea>
          </label>
          <div class="form-actions">
            <button class="secondary" onclick="closeModal()">Abbrechen</button>
            <button class="primary" onclick="CampusFpa.addImpression('${esc(p.id)}','${esc(p.title||"Praxisauftrag")}')">Speichern</button>
          </div>
        </div>`);
    },

    openImpression:async function(id){
      try{
        const items=await getCollection("practiceImpressions","createdAt",true);
        const i=items.find(x=>x.id===id && x.uid===currentUser.uid);
        if(!i){toast("Eintrag nicht gefunden.");return}
        modal(`
          <button class="modal-close" onclick="closeModal()">×</button>
          <div class="kicker">PRAXIS-EINDRUCK</div>
          <h2>${esc(i.title||"Mein Eindruck")}</h2>
          <p><strong>Praxisauftrag:</strong> ${esc(i.practiceTitle||"")}</p>
          <div class="card"><strong>Mein Eindruck</strong><p style="white-space:pre-wrap">${esc(i.text||"")}</p></div>
          ${i.next?`<div class="card"><strong>Was nehme ich mit?</strong><p style="white-space:pre-wrap">${esc(i.next)}</p></div>`:""}
    ...
    addWordCloud:async function(){
      const value=document.getElementById("fpaWordCloudText")?.value.trim()||"";
      if(!value){toast("Bitte mindestens einen Begriff eingeben.");return}
      try{
        ...
        closeModal();await render();toast("Deine Eindrücke wurden gespeichert.");
      }catch(e){console.error(e);toast("Der Beitrag konnte nicht gespeichert werden.")}
    },

    openWordCloudForm:function(){
      modal(`
        <button class="modal-close" onclick="closeModal()">×</button>
        <div class="kicker">PRAXIS · WORTWOLKE</div>
        <h2>Was nehme ich aus der Praxis mit?</h2>
        <p>Gib kurze Begriffe ein, die deine Erfahrungen aus dem Praktikum beschreiben.</p>
        <div class="form">
          <label>Meine Begriffe
            <textarea id="fpaWordCloudText" rows="4" placeholder="z. B. Teamarbeit, Verantwortung, Kommunikation, Geduld"></textarea>
          </label>
          <div class="form-actions">
            <button class="secondary" onclick="closeModal()">Abbrechen</button>
            <button class="primary" onclick="CampusFpa.addWordCloud()">Speichern</button>
          </div>
        </div>`);
    },

    download:function(type){
      // Die offiziellen PDF-Dateien werden hier später eingetragen.
      const documents={bewertung:"",wochenbericht:""};
      if(!documents[type]){
        toast("Für dieses Formular ist noch keine PDF-Datei hinterlegt.");
        return;
      }
      window.open(documents[type],"_blank","noopener,noreferrer");
    }
  };

  const practiceCards=practice.map(p=>{
    const n=mine.filter(i=>i.practiceId===p.id).length;
    return `<div class="card">
      ...
      <div class="form-actions">
        <button class="primary" onclick="CampusFpa.openImpressionForm('${esc(p.id)}')">＋ Mein Eindruck</button>
        ${n?`<span class="pill green">${n} eigener ${n===1?"Eintrag":"Einträge"}</span>`:""}
      </div>
    </div>`;
  }).join("");
  ...
  const impressionRows=mine.map(i=>`
    <div class="list-item">
      <div>
        ...
        <small>${esc(i.date||fmtDate(i.createdAt))} · ${esc(i.title||"Mein Eindruck")}</small>
      </div>
      <button class="secondary" onclick="CampusFpa.openImpression('${esc(i.id)}')">Öffnen</button>
    </div>`).join("");
  ...
  const cloudHtml=words.length
    ?words.map(([word,count])=>`<span class="fpa-cloud-word" style="font-size:${16+Math.round(count/max*24)}px">${esc(word)}</span>`).join("")
  ...
  <style>
    ...
    .fpa-tile{min-height:190px;cursor:pointer}.fpa-tile .emoji{font-size:30px}
    .fpa-tile h2{margin:8px 0 5px}.fpa-tile p{margin:0;color:var(--muted)}
    .fpa-section{margin-top:16px}
    .fpa-cloud{min-height:230px;display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:10px 16px;padding:22px;border-radius:14px;background:var(--soft,#f7f7f5)}
    .fpa-cloud-word{font-weight:700;line-height:1.1}
    .fpa-downloads{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
    .fpa-download-card{display:flex;align-items:center;gap:12px;padding:15px;border:1px solid var(--line,#ddd);border-radius:12px;background:#fff}
    .fpa-download-card strong{display:block}.fpa-download-card small{display:block;color:var(--muted);margin-top:3px}
    @media(max-width:900px){.fpa-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:600px){.fpa-grid,.fpa-downloads{grid-template-columns:1fr}}
  </style>

  <div class="fpa-grid">
    <section class="card fpa-tile" onclick="document.getElementById('fpaPraxis').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">🧭</span><h2>Praxisaufträge</h2><p>Arbeitsaufträge aus der Praxis bearbeiten und eigene Eindrücke festhalten.</p>
    </section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaFragen').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">❓</span><h2>Fragen</h2><p>Fragen aus dem Praktikum sammeln, teilen und gemeinsam weiterdenken.</p>
    </section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaProjekte').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">🚀</span><h2>Projekte</h2><p>Aus Beobachtungen und Fragestellungen entstehen eigene Praxisprojekte.</p>
    </section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaDownloads').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">📥</span><h2>Formulare & Downloads</h2><p>Bewertungsbogen des Praktikumsbetriebes und Wochenbericht.</p>
    </section>
    <section class="card fpa-tile" onclick="document.getElementById('fpaCloud').scrollIntoView({behavior:'smooth'})">
      <span class="emoji">☁️</span><h2>Praxis-Eindrücke</h2><p>Erfahrungen und Eindrücke als gemeinsame Wortwolke sichtbar machen.</p>
    </section>
    <section class="card fpa-tile" onclick="location.hash='ki'">
      <span class="emoji">🤖</span><h2>KI-Innovationspartnerschaften</h2><p>KI-Ideen gemeinsam mit Praxispartnern entwickeln und erproben.</p>
    </section>
  </div>

  <section id="fpaPraxis" class="card fpa-section">
    <div class="page-head"><div><div class="kicker">PRAXISAUFTRÄGE</div>
      <h2>Praxisaufträge & meine Eindrücke</h2><p>Zu jedem Praxisauftrag kannst du persönliche Beobachtungen speichern.</p>
    </div></div>
    <div class="grid grid-2">${practiceCards||`<div class="empty">Noch keine Praxisaufträge vorhanden.</div>`}</div>
    <div class="card" style="margin-top:12px"><h3>📚 Meine Praxis-Eindrücke</h3>
      <div class="list">${impressionRows||`<div class="empty">Du hast noch keinen Praxiseindruck gespeichert.</div>`}</div>
    </div>
  </section>

  <section id="fpaCloud" class="card fpa-section">
    <div class="page-head"><div><div class="kicker">REFLEXION</div>
      <h2>☁️ Wortwolke: Was nehme ich aus der Praxis mit?</h2><p>Häufig genannte Begriffe erscheinen größer.</p>
    </div><button class="primary" onclick="CampusFpa.openWordCloudForm()">＋ Eindruck hinzufügen</button></div>
    <div class="fpa-cloud">${cloudHtml}</div>
  </section>

  <section id="fpaDownloads" class="card fpa-section">
    <div class="page-head"><div><div class="kicker">UNTERLAGEN</div>
      <h2>📥 Formulare & Downloads</h2><p>Die zentralen Dokumente für die fpA.</p>
    </div></div>
    <div class="fpa-downloads">
      <div class="fpa-download-card"><div style="font-size:26px">📄</div><div style="flex:1">
        <strong>Bewertungsbogen des Praktikumsbetriebes</strong><small>Vorgegebenes Formular</small>
      </div><button class="secondary" onclick="CampusFpa.download('bewertung')">Download</button></div>
      <div class="fpa-download-card"><div style="font-size:26px">📋</div><div style="flex:1">
        <strong>Wochenbericht</strong><small>Vorgegebenes Formular</small>
      </div><button class="secondary" onclick="CampusFpa.download('wochenbericht')">Download</button></div>
    </div>
  </section>

  <section id="fpaFragen" class="card fpa-section">
    <div class="page-head"><div><div class="kicker">FRAGEN</div><h2>Fragen aus der Praxis</h2>
      <p>Fragen aus dem Praktikum sammeln und teilen.</p>
    </div><button class="primary" onclick="openPostForm('question')">＋ Frage stellen</button></div>
  ...
  <section id="fpaProjekte" class="card fpa-section">
    <div class="page-head"><div><div class="kicker">PROJEKTE</div><h2>Praxisprojekte</h2></div>
      <button class="primary" onclick="openProjectForm()">＋ Projekt anlegen</button></div>
  ...
  <section class="card fpa-section" style="border-left:4px solid var(--accent,#7b9b82)">
    <div class="page-head"><div><div class="kicker">INNOVATION</div>
      <h2>KI-Innovationspartnerschaften</h2><p>Praxisprobleme erkennen, KI-Ideen entwickeln und gemeinsam erproben.</p>
    </div><button class="primary" onclick="location.hash='ki'">KI-Innovationslabor öffnen →</button></div>
  </section>
