...
function pp11InjectStyles(){
  ...
  style.textContent=`
    ...
    .pp11-stand-card{margin-bottom:12px}
    .pp11-stand-head{display:flex;justify-content:space-between;gap:18px;align-items:center;flex-wrap:wrap}
    .pp11-stand-number{min-width:150px;text-align:center;border:1px solid var(--line);border-radius:14px;padding:10px 14px;background:var(--bg)}
    .pp11-stand-number b{display:block;font-size:22px}
    .pp11-stand-number small{color:var(--muted)}
    .pp11-stand-stats{display:flex;gap:12px;flex-wrap:wrap;margin-top:10px;font-size:12px;color:var(--muted)}
    .pp11-compact-list{display:grid;gap:8px}
    .pp11-compact-area{background:var(--card);border:1px solid var(--line);border-radius:14px;overflow:hidden}
    .pp11-compact-area summary{list-style:none;cursor:pointer;display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px 14px}
    .pp11-compact-area summary::-webkit-details-marker{display:none}
    .pp11-area-name{display:flex;align-items:center;gap:9px;min-width:0}
    .pp11-area-name>span:last-child{display:flex;flex-direction:column}
    .pp11-area-name small{color:var(--muted);font-size:11px;margin-top:2px}
    .pp11-area-count{display:flex;gap:5px;flex-wrap:wrap;justify-content:flex-end}
    .pp11-compact-body{border-top:1px solid var(--line);padding:12px 14px}
    .pp11-compact-item{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;padding:8px 0;border-bottom:1px solid var(--line);font-size:13px}
    .pp11-rating-mini{display:flex;gap:3px}
    .pp11-rating-mini button{width:28px;height:28px;padding:0;border:1px solid var(--line);background:var(--bg);border-radius:7px;font-weight:700;cursor:pointer}
    .pp11-rating-mini button.active.green{background:#dff4e6;border-color:#74bd8a}
    .pp11-rating-mini button.active.yellow{background:#fff3cc;border-color:#e4bd54}
    .pp11-rating-mini button.active.red{background:#ffe1df;border-color:#dc7b74}
    .pp11-note-box{margin-top:12px;padding:12px;border:1px dashed var(--line);border-radius:12px;background:var(--bg)}
    .pp11-note-box label:first-child{display:block}
    .pp11-note-box label small{display:block;color:var(--muted);font-weight:400;margin-top:3px}
    .pp11-note-box textarea{width:100%;box-sizing:border-box;margin-top:8px;min-height:58px;border:1px solid var(--line);border-radius:9px;padding:9px;font:inherit;background:var(--card);resize:vertical}
    .pp11-note-actions{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-top:7px;flex-wrap:wrap}
    .checkline{font-size:12px;color:var(--muted);display:flex;align-items:center;gap:6px}
    .pp11-teacher-view{background:var(--soft-green)}
    .pp11-category-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}
    .pp11-category-tile{display:flex;flex-direction:column;gap:7px;min-height:120px;text-decoration:none}
    .pp11-category-tile .emoji{font-size:24px}
    .pp11-category-tile strong{font-size:15px}
    .pp11-category-tile small{color:var(--muted);line-height:1.4}
    .pp11-category-tile:hover{transform:translateY(-1px)}
    .pp11-status-mini{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}
    .pp11-stand-link{border:1px solid var(--line);background:var(--soft-green);border-radius:14px;padding:12px 14px;display:flex;justify-content:space-between;align-items:center;gap:12px;text-decoration:none}
    .journal-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
    .journal-checks{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}
    .journal-checks label{border:1px solid var(--line);border-radius:10px;padding:8px;background:var(--bg);font-size:13px}
    .journal-entry{border-left:3px solid var(--ink)}
    .teacher-journal{background:var(--soft-green)}
    @media(max-width:900px){.pp11-category-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:760px){.pp11-category-grid,.journal-grid,.journal-checks{grid-template-columns:1fr}}
...
async function getPP11Notes(){
  const q=query(collection(db,"selfAssessmentNotes"),where("uid","==",currentUser.uid));
  const snap=await getDocs(q);
  const result={};
  snap.docs.forEach(d=>{const x=d.data(); result[x.areaId]={note:x.note||"",shareWithTeacher:x.shareWithTeacher===true,updatedAt:x.updatedAt||null}});
  return result;
}
...
async function savePP11Assessment(competencyId,rating){
  try{
    ...
    await setDoc(ref,{uid:currentUser.uid,competencyId,rating:Number(rating),status:pp11StatusFromRating(rating),updatedAt:serverTimestamp()},{merge:true});
    toast("Selbsteinschätzung gespeichert.");
    await render();
  }catch(e){console.error("PP11 Selbsteinschätzung:",e);toast("Speichern nicht möglich. Prüfe die Firestore-Regeln.")}
}
window.savePP11Assessment=savePP11Assessment;

async function savePP11Note(areaId){
  const noteEl=document.querySelector(`#pp11-note-${areaId}`);
  const shareEl=document.querySelector(`#pp11-share-${areaId}`);
  if(!noteEl)return;
  try{
    const ref=doc(db,"selfAssessmentNotes",`${currentUser.uid}_${areaId}`);
    await setDoc(ref,{uid:currentUser.uid,authorName:profile?.displayName||currentUser.displayName||"Schüler/in",areaId,note:noteEl.value.trim(),shareWithTeacher:!!shareEl?.checked,updatedAt:serverTimestamp()},{merge:true});
    toast(shareEl?.checked?"Notiz gespeichert und für die Lehrkraft freigegeben.":"Notiz gespeichert.");
  }catch(e){console.error("PP11 Lernnotiz:",e);toast("Notiz konnte nicht gespeichert werden.")}
}
window.savePP11Note=savePP11Note;

function pp11AreaSummary(area,assessments){
  ...
  const open=items.length-vals.length;
  return {total:items.length,answered:vals.length,green,yellow,red,open};
}

function pp11CompactArea(area,assessments,notes){
  const sum=pp11AreaSummary(area,assessments);
  const percent=sum.total?Math.round(sum.answered/sum.total*100):0;
  const note=notes[area.id]||{};
  return `<details class="pp11-compact-area">
    <summary>
      <span class="pp11-area-name"><span>${area.id==="lb1"?"🔬":area.id==="lb2"?"🧠":area.id==="lb3"?"🤝":"📚"}</span><span><b>${esc(area.title)}</b><small>${esc(area.short)}</small></span></span>
      <span class="pp11-area-count"><span class="pill green">🟢 ${sum.green}</span><span class="pill">🟡 ${sum.yellow}</span><span class="pill">🔴 ${sum.red}</span><span class="pill">${sum.answered}/${sum.total}</span></span>
    </summary>
    <div class="pp11-compact-body">
      <p class="muted" style="margin:0 0 10px">${esc(area.description)}</p>
      ${area.sections.map(sec=>`<div class="pp11-section"><h4>${esc(sec.title)}</h4>
        ${sec.items.map(i=>{
          const val=assessments[i[0]]||0;
          return `<div class="pp11-compact-item"><span>${esc(i[1])}</span><div class="pp11-rating-mini">${[4,3,2,1].map(v=>`<button class="${val===v?"active ":""}${pp11StatusFromRating(v)}" title="${esc(PP11_RATING[v].hint)}" onclick="savePP11Assessment('${i[0]}',${v})">${v}</button>`).join("")}</div></div>`;
        }).join("")}
      </div>`).join("")}
      <div class="pp11-note-box">
        <label for="pp11-note-${area.id}"><b>📝 Wobei bist du noch unsicher?</b><small>Hier kannst du notieren, was du noch nicht verstanden hast oder wo du Unterstützung brauchst.</small></label>
        <textarea id="pp11-note-${area.id}" rows="2" placeholder="z. B. Ich kann die Attributionen noch nicht sicher auf Fallbeispiele anwenden …">${esc(note.note||"")}</textarea>
        <div class="pp11-note-actions"><label class="checkline"><input id="pp11-share-${area.id}" type="checkbox" ${note.shareWithTeacher?"checked":""}> Lehrkraft darf meine Notiz sehen</label><button class="secondary" onclick="savePP11Note('${area.id}')">Notiz speichern</button></div>
      </div>
    </div>
  </details>`;
}
...
async function renderPP11TeacherOverview(){
  ...
  try{
    const [snap,noteSnap]=await Promise.all([
      getDocs(collection(db,"selfAssessments")),
      getDocs(collection(db,"selfAssessmentNotes"))
    ]);
    const all=snap.docs.map(d=>d.data());
    const notes=noteSnap.docs.map(d=>d.data())
      .filter(x=>x.shareWithTeacher===true && String(x.note||"").trim());

    const byId={};
    all.forEach(x=>{
      byId[x.competencyId]??=[];
      byId[x.competencyId].push(Number(x.rating||0));
    });
    ...
    }).filter(x=>x.n).sort((a,b)=>a.avg-b.avg).slice(0,6);

    return `<div class="card pp11-teacher-view" style="margin-top:12px">
      <h3>👩‍🏫 Für die Lehrkraft</h3>
      <p>Hier erscheinen nur persönliche Lernnotizen, die Schülerinnen und Schüler ausdrücklich freigegeben haben.</p>
      ${notes.length
        ? `<div class="list">${notes.slice(0,12).map(x=>`
          <div class="list-item">
            <div>
              <strong>${esc(x.authorName||"Schüler/in")}</strong>
              <small>${esc(x.note)} · ${esc(x.areaId||"Lernbereich")}</small>
            </div>
            <span class="pill">freigegeben</span>
          </div>`).join("")}</div>`
        : `<div class="empty">Noch keine freigegebenen Lernnotizen.</div>`}
      ${weak.length
        ? `<h4 style="margin-top:14px">Häufiger Klärungsbedarf in der Gruppe</h4>
           <div class="pp11-teacher-grid">${weak.map(x=>`
             <div class="pp11-teacher-card">
               <small>${esc(x.areaTitle)} · ${esc(x.sectionTitle)}</small>
               <strong>${esc(x.text)}</strong>
               <span class="pill ${x.avg>=3.5?"green":x.avg>=2?"":"red"}">Ø ${x.avg.toFixed(1)} · ${x.n}</span>
             </div>`).join("")}</div>`
        : ""}
    </div>`;
  }catch(e){
    console.error("PP11 Lehrerübersicht:",e);
    return `<div class="card"><b>Lehrkraftübersicht momentan nicht verfügbar.</b></div>`;
  }
}

async function renderLernwerkstatt(){
  pp11InjectStyles();
  let summary={answered:0,total:pp11AllItems().length,green:0,yellow:0,red:0};
  try{
    const assessments=await getPP11Assessments();
    const items=pp11AllItems();
    summary.answered=items.filter(i=>assessments[i.id]).length;
    summary.green=items.filter(i=>assessments[i.id]===4).length;
    summary.yellow=items.filter(i=>assessments[i.id]===2||assessments[i.id]===3).length;
    summary.red=items.filter(i=>assessments[i.id]===1).length;
  }catch(e){console.error("Lernstand-Zusammenfassung:",e)}

  const percent=summary.total?Math.round(summary.answered/summary.total*100):0;

  return `${pageHead("SELBSTSTÄNDIG LERNEN","Lernwerkstatt","Dein übersichtlicher Lernbereich: Aufträge, Impulse, Materialien, Methoden, digitale Werkzeuge und KI.",`<a class="secondary button" href="#lernstand">📊 Lernstand einschätzen</a>`)}
    <div class="pp11-category-grid">
      ${tile("📋","Lernaufträge","Arbeitsaufträge und selbstständige Lernphasen.","kompass")}
      ${tile("💡","Lernimpulse","Kurze Impulse zum Verstehen, Denken und Weiterlernen.","kompetenz")}
      ${tile("🧰","Materialien & Methoden","Materialien, Methoden und Ideen für deinen Lernweg.","praktikum")}
      ${tile("💻","Digitale Tools","Werkzeuge für Zusammenarbeit, Recherche und Lernprodukte.","ki")}
      ${tile("🤖","KI zum Lernen","KI sinnvoll, kritisch und kompetenzorientiert einsetzen.","ki")}
      <a class="card pp11-category-tile" href="#lernstand">
        <span class="emoji">📊</span>
        <strong>Mein Lernstand</strong>
        <small>Lehrplan-Kompetenzen einschätzen und festhalten, wo du noch Unterstützung brauchst.</small>
        <span class="pp11-status-mini">
          <span class="pill green">🟢 ${summary.green}</span>
          <span class="pill">🟡 ${summary.yellow}</span>
          <span class="pill">🔴 ${summary.red}</span>
        </span>
      </a>
    </div>
    ...
    <div class="card" style="margin-top:12px">
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap">
        <div>
          ...
          <h3 style="margin:8px 0 4px">📊 Mein Lernstand</h3>
          <p style="margin:0;color:var(--muted)">Nur die Übersicht – die ausführliche Selbsteinschätzung öffnet sich erst, wenn du „Mein Lernstand“ anklickst.</p>
        </div>
        <a class="primary button" href="#lernstand">Öffnen →</a>
      </div>
      <div class="pp11-progress" style="margin-top:12px"><i style="width:${percent}%"></i></div>
      <div class="pp11-stand-stats"><span>${summary.answered}/${summary.total} eingeschätzt</span><span>🟢 ${summary.green} Auf Kurs</span><span>🟡 ${summary.yellow} Klärungsbedarf</span><span>🔴 ${summary.red} Handlungsbedarf</span></div>
    </div>
    ${footer()}`;
}
...
async function renderLernstand(){
  pp11InjectStyles();
  const [assessments,notes]=await Promise.all([getPP11Assessments(),getPP11Notes()]);
  const items=pp11AllItems();
  ...
  return `${pageHead("LERNSTAND","Mein Lernstand","Schätze deine PP11-Kompetenzen ein. Erst beim Öffnen dieses Bereichs werden die einzelnen Aussagen sichtbar.",`<a class="secondary button" href="#lernwerkstatt">← Lernwerkstatt</a>`)}
    <div class="card pp11-stand-card">
      <div class="pp11-stand-head">
        <div>
          <span class="badge">PP 11 · LEHRPLAN 2026/27</span>
          <h2 style="margin:8px 0 4px">Wo stehe ich?</h2>
          <p style="margin:0;max-width:700px">Schätze jede Kompetenz ehrlich ein. Du kannst zusätzlich notieren, was dir noch schwerfällt oder wobei du Unterstützung brauchst.</p>
        </div>
        <div class="pp11-stand-number"><b>${answered}/${items.length}</b><small>Kompetenzen eingeschätzt</small></div>
      </div>
      <div class="pp11-progress" style="margin-top:14px"><i style="width:${percent}%"></i></div>
      <div class="pp11-stand-stats"><span>🟢 ${green} Auf Kurs</span><span>🟡 ${yellow} Klärungsbedarf</span><span>🔴 ${red} Handlungsbedarf</span></div>
    </div>
    <div class="notice"><b>Skala:</b> 4 = 🟢 Auf Kurs · 3/2 = 🟡 Klärungsbedarf · 1 = 🔴 Handlungsbedarf. Die Skala ist eine Lernhilfe und keine Leistungsbewertung.</div>
    <div class="pp11-compact-list" style="margin-top:12px">
      ${PP11_COMPETENCIES.map(a=>pp11CompactArea(a,assessments,notes)).join("")}
    </div>
...
async function getForumPosts(){
  try{
    ...
    console.warn("Forum sortiert laden nicht möglich, Fallback:",e);
...
async function renderForum(){
  let posts=[];
  try{posts=await getForumPosts()}catch(e){
    console.error("Forum laden:",e);
  }
  ...
  return `${pageHead("GEMEINSCHAFT","Campus-Forum","Gemeinsam denken, fragen, austauschen und unterstützen.",`<button class="primary" onclick="openPostForm()">＋ Beitrag schreiben</button>`)}
    <div class="toolbar">
      <div class="chips"><span class="chip">Alle</span><span class="chip">❓ Fragen</span><span class="chip">📢 Infos</span><span class="chip">💡 Ideen</span><span class="chip">🚀 Projekte</span><span class="chip">🏢 Praxis</span></div>
      <input class="search" id="forumSearch" placeholder="Beiträge durchsuchen …">
    </div>
    ...
    <div class="card" style="margin-top:12px;background:var(--soft-green)">
      <h3>🤝 Campus hilft</h3>
      <p>Du kannst anderen bei einem Thema helfen? Teile dein Wissen.</p>
      <button class="secondary" style="margin-top:10px" onclick="openHelpForm()">Hilfe anbieten</button>
    </div>${footer()}`;
}
...
async function getLearningJournalEntries(){
  try{
    const snap=await getDocs(query(collection(db,"learningJournal"),where("uid","==",currentUser.uid),limit(100)));
    return snap.docs.map(d=>({id:d.id,...d.data()}))
      .sort((a,b)=>(b.createdAtMs||0)-(a.createdAtMs||0));
  }catch(e){
    console.error("Lernjournal laden:",e);
    return [];
  }
}

async function saveLearningJournalEntry(){
  if(!currentUser){toast("Bitte zuerst anmelden.");return}

  const value=id=>(document.getElementById(id)?.value||"").trim();
  const activity=Array.from(document.querySelectorAll('input[name="journalActivity"]:checked')).map(x=>x.value);
  const helpful=Array.from(document.querySelectorAll('input[name="journalHelpful"]:checked')).map(x=>x.value);
  const entry={
    uid:currentUser.uid,
    authorName:profile?.displayName||currentUser.displayName||"Schüler/in",
    topic:value("journalTopic"),
    activity,
    learned:value("journalLearned"),
    canNow:value("journalCanNow"),
    unclear:value("journalUnclear"),
    nextStep:value("journalNextStep"),
    helpful,
    helpfulNext:value("journalHelpfulNext"),
    confidence:Number(document.getElementById("journalConfidence")?.value||0),
    practice:value("journalPractice"),
    shareWithTeacher:!!document.getElementById("journalShareTeacher")?.checked,
    createdAt:serverTimestamp(),
    createdAtMs:Date.now()
  };

  if(!entry.topic && !entry.learned && !entry.canNow && !entry.unclear && !entry.nextStep && !entry.practice){
    toast("Schreibe mindestens einen kurzen Lernmoment.");
    return;
  }

  try{
    await addDoc(collection(db,"learningJournal"),entry);
    toast("Lernmoment gespeichert.");
    await render();
  }catch(e){
    console.error("Lernjournal speichern:",e);
    toast(`Lernjournal konnte nicht gespeichert werden (${e?.code||"Fehler"}).`);
  }
}
window.saveLearningJournalEntry=saveLearningJournalEntry;

async function renderLearningJournalTeacherView(){
  if(!isTeacher()) return "";
  try{
    const snap=await getDocs(collection(db,"learningJournal"));
    const shared=snap.docs.map(d=>({id:d.id,...d.data()}))
      .filter(x=>x.shareWithTeacher===true)
      .sort((a,b)=>(b.createdAtMs||0)-(a.createdAtMs||0))
      .slice(0,12);

    return `<div class="card teacher-journal" style="margin-top:12px">
      <h3>👩‍🏫 Für die Lehrkraft</h3>
      <p>Hier erscheinen nur Lernjournal-Einträge, die ausdrücklich freigegeben wurden.</p>
      ${shared.length
        ? `<div class="list">${shared.map(e=>`
          <div class="card journal-entry">
            <strong>${esc(e.authorName||"Schüler/in")}</strong>
            <small style="display:block;color:var(--muted);margin-top:3px">${esc(e.topic||"Lernmoment")}</small>
            ${e.learned?`<p><b>Gelernt:</b> ${esc(e.learned)}</p>`:""}
            ${e.canNow?`<p><b>Jetzt kann ich:</b> ${esc(e.canNow)}</p>`:""}
            ${e.unclear?`<p><b>Noch unklar:</b> ${esc(e.unclear)}</p>`:""}
            ${e.nextStep?`<p><b>Nächster Schritt:</b> ${esc(e.nextStep)}</p>`:""}
          </div>`).join("")}</div>`
        : `<div class="empty">Noch keine freigegebenen Lernjournal-Einträge.</div>`}
    </div>`;
  }catch(e){
    console.error("Lehrkraft-Lernjournal:",e);
    return "";
  }
}

async function renderJournal(){
  const entries=await getLearningJournalEntries();
  const activityOptions=["Unterricht","Lernwerkstatt","Arbeitsauftrag","Partner-/Gruppenarbeit","Praktikum","eigenes Lernen","Lernvideo / digitales Material","KI genutzt"];
  const helpfulOptions=["Erklärung der Lehrkraft","Mitschüler/in","eigenes Ausprobieren","Beispiel","Lernvideo","Arbeitsblatt","KI","Lernwerkstatt","Praktische Erfahrung","Wiederholung"];

  const dateText=e=>{
    const d=e.createdAt?.toDate ? e.createdAt.toDate() : (e.createdAtMs?new Date(e.createdAtMs):null);
    return d?d.toLocaleString("de-DE",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"gerade eben";
  };

  return `${pageHead("REFLEXION","Lernjournal","Dein Lernweg: kurz festhalten, was du gelernt hast, was du jetzt kannst und woran du weiterarbeiten möchtest.",`<button class="primary" onclick="document.getElementById('journalNew')?.scrollIntoView({behavior:'smooth'})">＋ Lernmoment</button>`)}
    <div class="grid grid-2">
      <div class="card"><span class="badge">MEIN LERNWEG</span><h3 style="margin-top:8px">Was hat sich verändert?</h3><p>Die Lernwerkstatt zeigt dir, <b>wo du stehst</b>. Das Lernjournal hält fest, <b>wie du dich weiterentwickelst</b>.</p></div>
      <div class="card"><span class="badge">2–3 MINUTEN</span><h3 style="margin-top:8px">Kurz statt Pflichtprotokoll</h3><p>Du musst nicht jedes Feld ausfüllen. Ein kurzer, ehrlicher Eintrag reicht.</p></div>
    </div>

    <div id="journalNew" class="card" style="margin-top:12px">
      <h3>✏️ Lernmoment festhalten</h3>
      <div class="journal-grid">
        <label>Woran habe ich gearbeitet?<input id="journalTopic" placeholder="z. B. Attributionstheorie nach Weiner"></label>
        <label>Wie sicher fühle ich mich jetzt?
          <select id="journalConfidence">
            <option value="0">noch nicht einschätzen</option><option value="1">🔴 noch nicht</option><option value="2">🟠 teilweise</option><option value="3">🟡 ziemlich sicher</option><option value="4">🟢 sicher</option>
          </select>
        </label>
      </div>

      <label style="margin-top:12px">Was habe ich gemacht?</label>
      <div class="journal-checks">${activityOptions.map(x=>`<label><input type="checkbox" name="journalActivity" value="${esc(x)}"> ${esc(x)}</label>`).join("")}</div>

      <label style="margin-top:12px">Was habe ich dabei gelernt?</label>
      <textarea id="journalLearned" rows="3" placeholder="Ich habe verstanden, dass …"></textarea>

      <label>🧠 Jetzt kann ich …</label>
      <textarea id="journalCanNow" rows="2" placeholder="Ich kann inzwischen …"></textarea>

      <label>❓ Das ist mir noch unklar</label>
      <textarea id="journalUnclear" rows="2" placeholder="Ich verstehe noch nicht … / Ich kann noch nicht …"></textarea>

      <label>➡️ Mein nächster Schritt</label>
      <textarea id="journalNextStep" rows="2" placeholder="Als Nächstes möchte ich …"></textarea>

      <label>💡 Was hat mir beim Lernen geholfen?</label>
      <div class="journal-checks">${helpfulOptions.map(x=>`<label><input type="checkbox" name="journalHelpful" value="${esc(x)}"> ${esc(x)}</label>`).join("")}</div>

      <label style="margin-top:12px">Das möchte ich beim nächsten Mal wieder nutzen</label>
      <input id="journalHelpfulNext" placeholder="z. B. mit Fallbeispielen arbeiten">

      <label style="margin-top:12px">🏫 Schule ↔ Praxis (optional)</label>
      <textarea id="journalPractice" rows="2" placeholder="Was habe ich in der Praxis beobachtet oder dadurch besser verstanden?"></textarea>

      <div class="notice" style="margin-top:12px">
        <b>👩‍🏫 Persönliche Lernnotiz</b><br>
        Du entscheidest selbst, ob dieser Eintrag für die Lehrkraft sichtbar sein soll.
        <label style="display:block;margin-top:8px"><input id="journalShareTeacher" type="checkbox"> Ich möchte diesen Eintrag für meine Lehrkraft freigeben.</label>
      </div>

      <div style="display:flex;justify-content:flex-end;margin-top:12px">
        <button class="primary" onclick="saveLearningJournalEntry()">Eintrag speichern</button>
      </div>
    </div>

    <div style="margin-top:12px">
      <h3>📓 Meine bisherigen Lernmomente</h3>
      ${entries.length
        ? `<div class="list">${entries.map(e=>`
          <article class="card journal-entry">
            <div style="display:flex;justify-content:space-between;gap:12px">
              <div><strong>${esc(e.topic||"Lernmoment")}</strong><small style="display:block;color:var(--muted);margin-top:3px">${dateText(e)}</small></div>
              ${e.confidence?`<span class="pill">${["","🔴","🟠","🟡","🟢"][e.confidence]}</span>`:""}
            </div>
            ${e.learned?`<p><b>Gelernt:</b><br>${esc(e.learned)}</p>`:""}
            ${e.canNow?`<p><b>Jetzt kann ich:</b><br>${esc(e.canNow)}</p>`:""}
            ${e.unclear?`<p><b>Noch unklar:</b><br>${esc(e.unclear)}</p>`:""}
            ${e.nextStep?`<p><b>Nächster Schritt:</b><br>${esc(e.nextStep)}</p>`:""}
            ${e.practice?`<p><b>Schule ↔ Praxis:</b><br>${esc(e.practice)}</p>`:""}
            <small>${e.shareWithTeacher?"👩‍🏫 Für Lehrkraft freigegeben":"🔒 Nur für mich"}</small>
          </article>`).join("")}</div>`
        : `<div class="empty"><strong>Noch kein Lernmoment</strong>Starte mit einem kurzen Eintrag.</div>`}
    </div>

    ${await renderLearningJournalTeacherView()}
    ${footer()}`;
}
...
async function render(){
 ...
 const pages={start:renderStart,kompass:renderKompass,lernwerkstatt:renderLernwerkstatt,lernstand:renderLernstand,forum:renderForum,projekte:renderProjekte,kompetenz:renderKompetenz,journal:renderJournal,praktikum:renderPraktikum,ki:renderKI,kalender:renderKalender,team:renderTeam};
...
async function addPost(){
  ...
  const type=$("pType")?.value||"question";
  if(!text){toast("Bitte zuerst einen Beitrag eingeben.");return}
  if(!currentUser){toast("Bitte zuerst anmelden.");return}

  const button=document.querySelector('#modal .primary');
  if(button) button.disabled=true;

  try{
    await addDoc(collection(db,"posts"),{
      authorUid:currentUser.uid,
      authorName:profile?.displayName||currentUser.displayName||currentUser.email?.split("@")[0]||"Campus-Mitglied",
      type,
      text,
      likes:0,
      comments:[],
    ...
    console.error("Forum Beitrag speichern:",e);
    const code=e?.code||"unbekannt";
    toast(`Beitrag konnte nicht gespeichert werden (${code}).`);
...
function openJournalForm(){go("journal");setTimeout(()=>document.getElementById("journalNew")?.scrollIntoView({behavior:"smooth"}),100)}
async function addJournal(){return saveLearningJournalEntry()}
