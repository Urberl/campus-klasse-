...
function updateTeacherTeamNavigation(){
  const links=document.querySelectorAll('.nav-link[data-page="team"]');
  links.forEach(link=>{
    link.querySelector(".nav-label")?.replaceChildren(document.createTextNode("Lehrkräfte Klassenteam"));
    if(!link.querySelector(".nav-label")) {
      const textNodes=[...link.childNodes].filter(n=>n.nodeType===3);
      if(textNodes.length) textNodes[textNodes.length-1].textContent="Lehrkräfte Klassenteam";
    }
...
async function renderKalender(){
  ...
  if(!events.length){try{events=await getCollection("calendar","date",false)}catch(e){console.error("Kalender calendar:",e)}}
  ...
  const typeMeta={
    schulaufgabe:{label:"Schulaufgabe",className:"type-schulaufgabe"},
    kurzarbeit:{label:"Kurzarbeit",className:"type-kurzarbeit"},
    projektvorstellung:{label:"Projektvorstellung",className:"type-projektvorstellung"},
    referat:{label:"Referat",className:"type-referat"},
    praesentation:{label:"Präsentation",className:"type-praesentation"},
    sonstiges:{label:"Sonstiger Termin",className:"type-sonstiges"}
  };
  const normalizeType=e=>{
    const raw=String(e.type||e.eventType||e.category||"sonstiges").toLowerCase().trim();
    if(raw==="präsentation")return "praesentation";
    return typeMeta[raw]?raw:"sonstiges";
  };
  const dateVal=e=>{
  ...
  const monthHTML=(y,m,name)=>{
    ...
    for(let d=1;d<=days;d++){
      ...
      const hasMultiple=ds.length>1;
      cells.push(`<button type="button" class="cal-day ${ds.length?"has-event "+typeMeta[firstType].className:""}"
        onclick="openCalendarDay(${y},${m},${d})">
        <span class="cal-num">${d}</span>
        ${ds.length?`<span class="cal-event-label">${esc(typeMeta[firstType].label)}</span>${hasMultiple?`<span class="cal-count">+${ds.length-1}</span>`:""}`:""}
      </button>`);
    }
  ...
  const addButton=canEdit?'<button id="calendarAddBtn" class="primary" type="button">＋ Termin eintragen</button>':"";

  const html=`${pageHead("ORGANISATION","Campus-Kalender",
    canEdit
      ?"Termine im Schuljahr 26/27. Farbige Markierungen zeigen die Terminart auf einen Blick."
      :"Termine im Schuljahr 26/27. Farbige Markierungen zeigen die Terminart auf einen Blick.",
    addButton)}
  <style>
    .cal-months{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}
    .cal-month{padding:18px}
    .cal-month-head{margin-bottom:10px}
    ...
    .cal-day{
      min-height:62px;border:1px solid var(--line);border-radius:9px;background:#fff;
      padding:7px;text-align:left;cursor:pointer;position:relative;overflow:hidden
    }
    .cal-day.empty{border:0;background:transparent;cursor:default}
    .cal-day.has-event{border:2px solid rgba(0,0,0,.12);font-weight:700}
    .cal-num{display:block;font-size:14px}
    .cal-event-label{
      display:block;margin-top:5px;font-size:10px;line-height:1.15;
      white-space:nowrap;overflow:hidden;text-overflow:ellipsis
    }
    .cal-count{
      position:absolute;right:5px;bottom:5px;font-size:10px;
      background:rgba(255,255,255,.75);border-radius:10px;padding:1px 5px
    }
    .type-schulaufgabe{background:#dbeafe!important}
    .type-kurzarbeit{background:#fee2e2!important}
    .type-projektvorstellung{background:#dcfce7!important}
    .type-referat{background:#fef3c7!important}
    .type-praesentation{background:#ede9fe!important}
    .type-sonstiges{background:#e5e7eb!important}
    .cal-legend{display:flex;flex-wrap:wrap;gap:8px;margin:0 0 16px}
    .cal-legend-item{display:inline-flex;align-items:center;gap:7px;border:1px solid var(--line);
      border-radius:999px;padding:6px 10px;background:#fff;font-size:12px}
    .cal-legend-dot{width:12px;height:12px;border-radius:3px;border:1px solid rgba(0,0,0,.12)}
    @media(max-width:800px){.cal-months{grid-template-columns:1fr}.cal-day{min-height:58px}}
  </style>
  <div class="card" style="margin-bottom:16px">
    <strong>Campus-Kalender</strong>
    <p style="margin-bottom:10px">Termine werden im gemeinsamen Kalender gespeichert. Klicke auf einen Tag, um die Termine zu sehen.</p>
    <div class="cal-legend">
      ${Object.entries(typeMeta).map(([key,v])=>`<span class="cal-legend-item"><span class="cal-legend-dot ${v.className}"></span>${v.label}</span>`).join("")}
    </div>
  </div>
  <div class="cal-months">${months.map(x=>monthHTML(x.y,x.m,x.name)).join("")}</div>${footer()}`;

  setTimeout(()=>{
    const b=$("calendarAddBtn");
    if(b)b.addEventListener("click",openCalendarForm);
  },0);
  return html;
}
...
function openCalendarDay(y,m,d){
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    ...
    <div class="list">${day.map(e=>{
      const typ=normalizeCalendarTypeForDisplay(e);
      return `<div class="card ${typ.className}" style="margin-bottom:10px">
        <span class="pill">${esc(typ.label)}</span>
        <h3>${esc(e.title||e.name||"Termin")}</h3>
        <p>${esc(e.description||e.text||e.notes||"")}</p>
        ${e.time?`<small>🕒 ${esc(e.time)}</small>`:""}
        ${e.location?`<small style="display:block;margin-top:4px">📍 ${esc(e.location)}</small>`:""}
      </div>`;
...
function normalizeCalendarTypeForDisplay(e){
  const raw=String(e.type||e.eventType||e.category||"sonstiges").toLowerCase().trim();
...
function openCalendarForm(){
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    ...
    <div class="form">
      ...
      <label>Titel / Bezeichnung<input id="calTitle" placeholder="z. B. Schulaufgabe Pädagogik"></label>
      <label>Uhrzeit (optional)<input id="calTime" type="time"></label>
      <label>Ort / Raum (optional)<input id="calLocation" placeholder="z. B. F203"></label>
      <label>Weitere Informationen<textarea id="calDescription" rows="4" placeholder="Alles, was zum Termin wichtig ist …"></textarea></label>
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Abbrechen</button>
        <button class="primary" onclick="addCalendar()">Termin speichern</button>
      </div>
    </div>`);
}
...
async function addCalendar(){
  ...
  const type=$("calType")?.value||"sonstiges";
  const title=$("calTitle")?.value.trim()||"";
  if(!date||!title){toast("Bitte Datum und Titel eintragen.");return}
  try{
    await addDoc(collection(db,"events"),{
      ...
      type,
      time:$("calTime")?.value||"",
      location:$("calLocation")?.value.trim()||"",
      description:$("calDescription")?.value.trim()||"",
      createdBy:currentUser.uid,
      createdAt:serverTimestamp()
    });
    closeModal();await render();toast("Termin gespeichert.");
  }catch(e){console.error("Kalender speichern:",e);toast("Termin konnte nicht gespeichert werden.")}
}
...
async function renderTeam(){
  if(!isTeacher()){
    return `${pageHead("LEHRKRÄFTE","Lehrkräfte Klassenteam","Dieser Bereich ist ausschließlich für das Klassenteam der Lehrkräfte zugänglich.",
      `<button class="secondary" onclick="go('start')">← Startseite</button>`)}
      <div class="card">
        <h2>Kein Zugang</h2>
        <p>Der Bereich „Lehrkräfte Klassenteam“ ist nur für Lehrkräfte freigeschaltet.</p>
      </div>${footer()}`;
  }

  let updates=[];
  try{updates=await getCollection("classTeamUpdates","createdAt",true)}catch(e){
    console.error("Klassenteam-Bibliothek:",e);
  }
  ...
  return `${pageHead(
    ...
    "Interne Informationen für das Klassenteam – aktuell informieren, dokumentieren und später nachvollziehen.",
    `<button class="primary" onclick="openClassTeamUpdateForm()">＋ Information posten</button>`
  )}
  <style>
    .team-info-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.8fr);gap:16px}
    .team-info-card{border:1px solid var(--line);border-radius:12px;padding:16px;background:#fff}
  ...
  <div class="team-info-grid">
    <section class="team-info-card">
      ...
      <div class="notice">
        <strong>Nur für Lehrkräfte</strong>
        <p style="margin-bottom:0">Schülerinnen und Schüler haben keinen Zugang zu diesem Bereich.</p>
      </div>
      <div style="margin-top:14px">
        <button class="primary" onclick="openClassTeamUpdateForm()">＋ Neue Information</button>
      </div>
    </section>
    ...
    <section class="team-info-card">
      ...
      <h2>Historie</h2>
      <p>Alle bisherigen Informationen des Klassenteams werden chronologisch gesammelt.</p>
      <div class="list">
        ${updates.map(u=>{
          const type=typeMeta[u.type]||"Sonstiges";
          const date=u.date||u.createdAt?.toDate?.()?.toLocaleDateString("de-DE")||"";
          return `<article class="team-history-item">
            <div class="team-history-head">
              <div>
                <span class="pill">${esc(type)}</span>
                <h3 style="margin:8px 0 0">${esc(u.title||"Information")}</h3>
              </div>
...
function openClassTeamUpdateForm(){
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">LEHRKRÄFTE KLASSENTEAM</div>
    <h2>Information posten</h2>
    <p>Die Information wird in der internen Bibliothek des Klassenteams gespeichert.</p>
    <div class="form">
      <label>Datum<input id="ctDate" type="date" value="${today}" required></label>
      <label>Art
        <select id="ctType">
      ...
      <label>Nächster Schritt / Vereinbarung (optional)<textarea id="ctFollowUp" rows="3" placeholder="Was wurde vereinbart oder ist noch zu tun?"></textarea></label>
      <div class="form-actions">
        <button class="secondary" onclick="closeModal()">Abbrechen</button>
        <button class="primary" onclick="saveClassTeamUpdate()">Veröffentlichen</button>
      </div>
    </div>`);
}
...
async function saveClassTeamUpdate(){
  ...
  const textValue=$("ctText")?.value.trim()||"";
  if(!title||!textValue){toast("Bitte Titel und Information ausfüllen.");return}
  try{
    await addDoc(collection(db,"classTeamUpdates"),{
      date:$("ctDate")?.value||new Date().toISOString().slice(0,10),
      type:$("ctType")?.value||"info",
      title,
      text:textValue,
    ...
    closeModal();
    await render();
    toast("Information für das Klassenteam gespeichert.");
  }catch(e){
    console.error("Klassenteam speichern:",e);
    toast("Information konnte nicht gespeichert werden.");
  }
}
...
async function render(){
  ...
  if(p==="team" && !isTeacher()){
    document.querySelectorAll(".nav-link[data-page='team']").forEach(a=>{a.hidden=true});
  }
...
window.updateTeacherTeamNavigation=updateTeacherTeamNavigation;
