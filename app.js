...
/* =========================================================
   CAMPUS-FORUM – TABS (Forum / Nachrichten)
   ========================================================= */
let forumActiveTab="forum";
let activeConversationUid=null, activeConversationName="";
let messageReplyTo=null, editingMessageId=null, currentThreadMessages=[];

function forumTabsHTML(unread){
  const tab=(id,label)=>{
    const active=forumActiveTab===id;
    return `<button class="secondary" style="${active?"background:var(--soft-green);font-weight:800;":""}"
onclick="switchForumTab('${id}')">${label}</button>`;
  };
  return `<div class="toolbar" style="gap:8px">${tab("forum","💬 Forum")}${tab("messages","✉️ Nachrichten"+(unread?` <span class="badge">${unread}</span>`:""))}</div>`;
}

function switchForumTab(tab){
  forumActiveTab=tab;
  if(tab==="forum"){activeConversationUid=null;activeConversationName="";messageReplyTo=null;editingMessageId=null;}
  render();
}

async function renderForum(){
  const unread=await getUnreadMessageCount();
  const tabsHTML=forumTabsHTML(unread);
  return forumActiveTab==="messages" ? await renderMessagesTab(tabsHTML) : await renderForumTab(tabsHTML);
}

async function renderForumTab(tabsHTML){
...
/* ---------------------------------------------------------
   NACHRICHTEN – persönliche Konversationen
   --------------------------------------------------------- */
...
async function renderMessagesTab(tabsHTML){
  if(activeConversationUid) return await renderConversationView(tabsHTML);
  const conversations=await getMyConversations();
  return `${pageHead("GEMEINSCHAFT","Nachrichten","Schreibe Schüler/innen oder Lehrkräften direkt eine persönliche Nachricht.",`<button
class="primary" onclick="openNewMessagePicker()">＋ Neue Nachricht</button>`)}
${tabsHTML}
  <div class="list" id="conversationList">${conversations.map(c=>`
    <article class="card" style="cursor:pointer" onclick="openConversation('${c.otherUid}')">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px">
        ...
        <div style="text-align:right;flex-shrink:0"><small>${fmtDate(c.lastAt)}</small>${c.unread?`<div class="badge" style="margin-top:4px">${c.unread}</div>`:""}</div>
      </div>
...
async function markConversationRead(otherUid){
  try{
    ...
    const unread=snap.docs.filter(d=>{const v=d.data();return v.toUid===currentUser.uid && !v.read;});
...
async function renderConversationView(tabsHTML){
  const convId=conversationIdFor(currentUser.uid,activeConversationUid);
  let msgs=[];
  try{
    const snap=await getDocs(query(collection(db,"messages"),where("conversationId","==",convId)));
    msgs=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(a.createdAt?.seconds||0)-(b.createdAt?.seconds||0));
  }catch(e){console.error("Konversation laden:",e)}
  currentThreadMessages=msgs;
  return `${pageHead("GEMEINSCHAFT",esc(activeConversationName||"Nachricht"),"Persönliche Unterhaltung.",`<button
class="secondary" onclick="closeConversation()">← Alle Nachrichten</button>`)}
${tabsHTML}
...
class="primary" onclick="saveEditMessage('${m.id}')">Speichern</button></div>
  ...
  return `<div class="comment" style="${mine?"background:var(--soft-green)":""}">
    ...
    <div class="post-actions">
      ...
      ${mine?`<button onclick="editMessage('${m.id}')">Bearbeiten</button><button onclick="deleteMessage('${m.id}')">Löschen</button>`:""}
    </div>
  </div>`;
}
...
async function getBirthdayEvents(){
  try{
    const snap=await getDocs(collection(db,"users"));
    const yearFor=mm=>mm>=8?2026:2027; // Sept–Dez 2026, Jan–Jul 2027
    return snap.docs
