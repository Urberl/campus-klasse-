...
async function renderPraktikum(){
  let assignments=[];
  try{
    const all=await getCollection("practice","createdAt",true);
    assignments=all.filter(p=>p.module==="fpa" && p.type==="teacherAssignment");
  }catch(e){console.error("fpA:",e)}

  const cards=assignments.map(p=>`
    <div class="card">
  ...
  return `${pageHead(
    "SCHULE ↔ PRAXIS","fpA",
    "Praxisaufträge der Lehrkraft für die fachpraktische Ausbildung.",
    `<button class="primary" onclick="openPracticeForm()">＋ Praxisauftrag</button>`
  )}
  <style>
    .fpa-tools{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin-top:18px}
    .fpa-tool{min-height:150px;cursor:pointer}
    @media(max-width:800px){.fpa-tools{grid-template-columns:1fr}}
  </style>
  <div class="notice">
    <b>🧭 Praxisaufträge</b>
    <p style="margin:6px 0 0">Hier erscheinen ausschließlich von der Lehrkraft angelegte fpA-Praxisaufträge. Einträge aus anderen Modulen werden nicht übernommen.</p>
  </div>
  <div class="grid grid-2" style="margin-top:14px">
    ${cards||`<div class="empty"><strong>Noch keine fpA-Praxisaufträge.</strong><p>Die Lehrkraft kann hier neue Aufträge anlegen.</p></div>`}
  </div>
  <div class="fpa-tools">
    <a class="card fpa-tool tile" href="#praxisfragen"><span class="emoji">❓</span><strong>Fragen aus der Praxis</strong><small>Eigenständiges Tool – getrennt von den Praxisaufträgen.</small></a>
    <a class="card fpa-tool tile" href="#praxisprojekte"><span class="emoji">🚀</span><strong>Projekte in der Praxis</strong><small>Eigenständiges Tool – getrennt von den Praxisaufträgen.</small></a>
    <a class="card fpa-tool tile" href="#ki"><span class="emoji">🤖</span><strong>KI-Innovationspartnerschaften</strong><small>Eigenständiger Bereich – getrennt von fpA-Aufträgen.</small></a>
  </div>
  ${footer()}`;
}

async function renderPraxisFragen(){
  let posts=[];
  try{posts=await getCollection("posts","createdAt",true)}catch(e){console.error(e)}
  const items=posts.filter(p=>p.module==="praxisfragen" && p.type==="question");
  return `${pageHead("fpA · EIGENES TOOL","Fragen aus der Praxis","Fragen aus dem Praktikum – vollständig getrennt von den Praxisaufträgen.",`<button class="primary" onclick="openPostForm('question')">＋ Frage</button>`)}
  <div class="card"><h2>❓ Fragen aus der Praxis</h2><p>Nur Einträge dieses Tools werden hier angezeigt.</p>
  <div class="list">${items.map(postHTML).join("")||`<div class="empty">Noch keine Fragen aus der Praxis.</div>`}</div></div>${footer()}`;
}

async function renderPraxisProjekte(){
  let projects=[];
  try{projects=await getCollection("projects","createdAt",true)}catch(e){console.error(e)}
  const items=projects.filter(p=>p.module==="praxisprojekte");
  return `${pageHead("fpA · EIGENES TOOL","Projekte in der Praxis","Praxisprojekte – vollständig getrennt von Praxisaufträgen und Kompetenznetzwerk.",`<button class="primary" onclick="openProjectForm()">＋ Praxisprojekt</button>`)}
  <div class="card"><h2>🚀 Projekte in der Praxis</h2><p>Nur Einträge dieses Tools werden hier angezeigt.</p>
  <div class="grid grid-3">${items.map(p=>`<div class="card"><h3>${esc(p.title||"Praxisprojekt")}</h3><p>${esc(p.goal||p.description||"")}</p><small>${esc(p.team||"")} ${p.partner?`· ${esc(p.partner)}`:""}</small></div>`).join("")||`<div class="empty">Noch keine Projekte in der Praxis.</div>`}</div></div>${footer()}`;
}

async function renderKI(){
  let challenges=[],solutions=[],results=[];
  try{challenges=await getCollection("kiChallenges","createdAt",true)}catch(e){console.warn("KI challenges",e)}
  try{solutions=await getCollection("kiSolutions","createdAt",true)}catch(e){console.warn("KI solutions",e)}
  try{results=await getCollection("kiResults","createdAt",true)}catch(e){console.warn("KI results",e)}

  return `${pageHead(
    "INNOVATIONSPARTNERSCHAFT",
    "KI-Innovationspartnerschaften",
    "Von der Praxis-Herausforderung über die Schülerentwicklung bis zum Ergebnis.",
    `<button class="primary" onclick="openKIChallengeForm()">＋ Herausforderung eintragen</button>`
  )}
  <style>
    .ki-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}
    .ki-card{min-height:245px;cursor:pointer;transition:transform .15s,box-shadow .15s}
    .ki-card:hover{transform:translateY(-2px)}
    .ki-step{font-size:27px;font-weight:800;margin-bottom:10px}
    .ki-action{margin-top:18px}
    .ki-process{margin-top:16px}
    .ki-process-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}
    .ki-process-item{min-height:110px}
    @media(max-width:850px){.ki-grid,.ki-process-grid{grid-template-columns:1fr}}
  </style>

  <div class="ki-grid">
    <section class="card ki-card" role="button" tabindex="0"
      onclick="openKIChallengesLibrary()" onkeydown="if(event.key==='Enter'||event.key===' ')openKIChallengesLibrary()">
      <div class="ki-step">1</div>
      <h2>Praxisproblem<br>Herausforderungen im Praktikumsbetrieb</h2>
      <p>Betriebe tragen konkrete Herausforderungen aus ihrem Arbeitsalltag ein. Diese werden in einer Bibliothek gesammelt.</p>
      <div class="ki-action"><button class="primary" onclick="event.stopPropagation();openKIChallengeForm()">＋ Eintragen</button>
      <span class="pill">${challenges.length} Einträge</span></div>
    </section>

    <section class="card ki-card" role="button" tabindex="0"
      onclick="openKISolutionsLibrary()" onkeydown="if(event.key==='Enter'||event.key===' ')openKISolutionsLibrary()">
      <div class="ki-step">2</div>
      <h2>Schülerteam / Schüler<br>löst Herausforderung</h2>
      <p>Einzelne Schüler oder Teams übernehmen eine Herausforderung und dokumentieren Aufgaben, Verantwortlichkeiten und KI-Einsatz.</p>
      <div class="ki-action"><button class="primary" onclick="event.stopPropagation();openKIChallengesLibrary()">＋ Herausforderung übernehmen</button>
      <span class="pill">${solutions.length} Bearbeitungen</span></div>
    </section>

    <section class="card ki-card" role="button" tabindex="0"
      onclick="openKIResultsLibrary()" onkeydown="if(event.key==='Enter'||event.key===' ')openKIResultsLibrary()">
      <div class="ki-step">3</div>
      <h2>Ergebnisse<br>Ideen & Produkte</h2>
      <p>Ideen, Konzepte, Prototypen, Produkte und weitere entstandene Ergebnisse werden gesammelt.</p>
      <div class="ki-action"><button class="primary" onclick="event.stopPropagation();openKIResultForm()">＋ Ergebnis eintragen</button>
      <span class="pill">${results.length} Ergebnisse</span></div>
    </section>
  </div>

  <div class="card ki-process">
    <h3>Der Ablauf</h3>
    <div class="ki-process-grid">
      <div class="card ki-process-item"><strong>1. Praxisproblem</strong><br><small>Ein realer Bedarf aus einem Betrieb wird beschrieben.</small></div>
      <div class="card ki-process-item"><strong>2. Entwicklung</strong><br><small>Schüler bearbeiten die Herausforderung mit geeigneten KI-Werkzeugen.</small></div>
      <div class="card ki-process-item"><strong>3. Ergebnis</strong><br><small>Die entstandene Lösung wird dokumentiert und für den Praxispartner nutzbar gemacht.</small></div>
    </div>
  </div>
  ${footer()}`;
}
...
function openKIChallengeForm(){
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
  <div class="kicker">KI-INNOVATIONSPARTNERSCHAFT</div><h2>Herausforderung aus dem Praktikumsbetrieb</h2>
  <div class="form">
    <label>Betrieb / Einrichtung<input id="kiCompany"></label>
    <label>Ansprechperson<input id="kiContact"></label>
    <label>Titel der Herausforderung<input id="kiTitle"></label>
    <label>Herausforderung<textarea id="kiDescription" rows="5" placeholder="Was ist das konkrete Problem in der Praxis?"></textarea></label>
    ...
    <div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="saveKIChallenge()">Speichern</button></div>
  </div>`);
}
async function saveKIChallenge(){
  const title=$("kiTitle")?.value.trim()||"", description=$("kiDescription")?.value.trim()||"";
  if(!title||!description){toast("Bitte Titel und Herausforderung ausfüllen.");return}
  try{
    await addDoc(collection(db,"kiChallenges"),{module:"kiInnovationspartnerschaften",title,description,company:$("kiCompany")?.value.trim()||"",contact:$("kiContact")?.value.trim()||"",target:$("kiTarget")?.value.trim()||"",goal:$("kiGoal")?.value.trim()||"",privacy:$("kiPrivacy")?.value.trim()||"",status:"offen",createdBy:currentUser.uid,createdAt:serverTimestamp()});
    closeModal();await render();toast("Herausforderung gespeichert.");
  }catch(e){console.error("KI Herausforderung:",e);toast("Speichern fehlgeschlagen: "+(e.code||"unbekannter Fehler"))}
}
async function openKIChallengesLibrary(){
  let a=[];try{a=await getCollection("kiChallenges","createdAt",true)}catch(e){console.error(e)}
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">1 · PRAXISPROBLEM</div><h2>Bibliothek der Herausforderungen</h2>
  <div class="list">${a.map(c=>`<div class="card" style="margin-bottom:10px"><strong>${esc(c.title||"Herausforderung")}</strong><small>${esc(c.company||"")}</small><p>${esc(c.description||"")}</p><button class="primary" onclick="openKITakeChallenge('${esc(c.id)}')">Herausforderung übernehmen</button></div>`).join("")||`<div class="empty">Noch keine Herausforderungen eingetragen.</div>`}</div>
  <div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button><button class="primary" onclick="closeModal();setTimeout(openKIChallengeForm,50)">＋ Neue Herausforderung</button></div>`);
}
async function openKITakeChallenge(id){
  let a=[];try{a=await getCollection("kiChallenges","createdAt",true)}catch(e){console.error(e)}
  const c=a.find(x=>x.id===id);if(!c){toast("Herausforderung nicht gefunden.");return}
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">2 · ENTWICKLUNG</div><h2>${esc(c.title||"Herausforderung")}</h2><p>${esc(c.description||"")}</p>
  <div class="form"><label>Einzelperson oder Team<select id="kiMode"><option value="team">Schülerteam</option><option value="single">Einzelschüler/in</option></select></label>
  <label>Name / Team<input id="kiTeam"></label><label>Mitglieder<textarea id="kiMembers" rows="3"></textarea></label>
  <label>Wer macht was?<textarea id="kiRoles" rows="4"></textarea></label><label>Geplanter KI-Einsatz<textarea id="kiAI" rows="4"></textarea></label>
  <div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="saveKISolution('${esc(c.id)}')">Bearbeitung speichern</button></div></div>`);
}
async function saveKISolution(challengeId){
  ...
  try{
    await addDoc(collection(db,"kiSolutions"),{module:"kiInnovationspartnerschaften",challengeId,mode:$("kiMode")?.value||"team",team,members:$("kiMembers")?.value.trim()||"",roles:$("kiRoles")?.value.trim()||"",aiUse:$("kiAI")?.value.trim()||"",status:"in Bearbeitung",createdBy:currentUser.uid,createdAt:serverTimestamp()});
    closeModal();await render();toast("Bearbeitung gespeichert.");
  }catch(e){console.error("KI Lösung:",e);toast("Speichern fehlgeschlagen: "+(e.code||"unbekannter Fehler"))}
}
async function openKISolutionsLibrary(){
  let a=[];try{a=await getCollection("kiSolutions","createdAt",true)}catch(e){console.error(e)}
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">2 · ENTWICKLUNG</div><h2>Schülerteams & Lösungsentwicklung</h2>
  <div class="list">${a.map(s=>`<div class="card" style="margin-bottom:10px"><strong>${esc(s.team||"Schüler/in")}</strong><small>${s.mode==="team"?"Schülerteam":"Einzelschüler/in"} · ${esc(s.status||"")}</small><p><b>Wer macht was:</b> ${esc(s.roles||"—")}</p><p><b>KI-Einsatz:</b> ${esc(s.aiUse||"—")}</p></div>`).join("")||`<div class="empty">Noch keine Bearbeitungen.</div>`}</div>
  <div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button><button class="primary" onclick="closeModal();setTimeout(openKIChallengesLibrary,50)">＋ Herausforderung auswählen</button></div>`);
}
function openKIResultForm(){
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">3 · ERGEBNIS</div><h2>Ergebnis dokumentieren</h2>
  <div class="form"><label>Titel<input id="kiResultTitle"></label>
  ...
  <label>Beschreibung<textarea id="kiResultDescription" rows="5"></textarea></label><label>Schülerteam / Schüler<input id="kiResultTeam"></label>
  <label>Praxispartner<input id="kiResultPartner"></label><label>Link zum Ergebnis<input id="kiResultLink"></label>
  <div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="saveKIResult()">Ergebnis speichern</button></div></div>`);
}
async function saveKIResult(){
  const title=$("kiResultTitle")?.value.trim()||"";if(!title){toast("Bitte einen Titel eingeben.");return}
  try{
    await addDoc(collection(db,"kiResults"),{module:"kiInnovationspartnerschaften",title,type:$("kiResultType")?.value||"Idee",description:$("kiResultDescription")?.value.trim()||"",team:$("kiResultTeam")?.value.trim()||"",partner:$("kiResultPartner")?.value.trim()||"",link:$("kiResultLink")?.value.trim()||"",createdBy:currentUser.uid,createdAt:serverTimestamp()});
    closeModal();await render();toast("Ergebnis gespeichert.");
  }catch(e){console.error("KI Ergebnis:",e);toast("Speichern fehlgeschlagen: "+(e.code||"unbekannter Fehler"))}
}
async function openKIResultsLibrary(){
  let a=[];try{a=await getCollection("kiResults","createdAt",true)}catch(e){console.error(e)}
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">3 · ERGEBNIS</div><h2>Ergebnisse, Ideen & Produkte</h2>
  <div class="list">${a.map(r=>`<div class="card" style="margin-bottom:10px"><strong>${esc(r.title||"Ergebnis")}</strong><small>${esc(r.type||"")}</small><p>${esc(r.description||"")}</p>${r.team?`<p><b>Team:</b> ${esc(r.team)}</p>`:""}${r.partner?`<p><b>Praxispartner:</b> ${esc(r.partner)}</p>`:""}${r.link?`<p><a href="${esc(r.link)}" target="_blank" rel="noopener">Ergebnis öffnen</a></p>`:""}</div>`).join("")||`<div class="empty">Noch keine Ergebnisse.</div>`}</div>
  <div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button><button class="primary" onclick="closeModal();setTimeout(openKIResultForm,50)">＋ Ergebnis</button></div>`);
}


async function renderResilienz(){
  const exercises=[
    {id:"atem",icon:"🌬️",title:"Atemtechnik",desc:"Kurze Atemübung zum bewussten Regulieren und Ankommen.",mins:2},
    {id:"distanz",icon:"🪟",title:"Distanzierung",desc:"Eine Situation kurz aus einer anderen Perspektive betrachten.",mins:2},
    {id:"leichtigkeit",icon:"🪶",title:"Leichtigkeit",desc:"Den inneren Druck für einen Moment reduzieren.",mins:2},
    {id:"lachen",icon:"😄",title:"Lachen",desc:"Humor und positive Aktivierung bewusst einsetzen.",mins:2},
    {id:"boden",icon:"👣",title:"Boden spüren",desc:"Über Körper und Sinne wieder ins Hier und Jetzt kommen.",mins:1},
    {id:"bewegung",icon:"⚡",title:"Bewegungsreset",desc:"Kurze Aktivierung zwischen Lern- und Arbeitsphasen.",mins:2},
    {id:"ressource",icon:"🌱",title:"Ressourcen aktivieren",desc:"Eine persönliche Stärke oder hilfreiche Ressource bewusst machen.",mins:2},
    {id:"kontakt",icon:"🤝",title:"Verbindung",desc:"Über Menschen und soziale Unterstützung Stabilität gewinnen.",mins:2},
    {id:"fokus",icon:"🎯",title:"Fokus",desc:"Auf den nächsten machbaren Schritt konzentrieren.",mins:2},
    {id:"perspektive",icon:"🧠",title:"Perspektivwechsel",desc:"Gedanken prüfen und eine hilfreichere Sichtweise ausprobieren.",mins:3},
    {id:"pause",icon:"☕",title:"Pause",desc:"Eine bewusste kurze Unterbrechung als aktive Ressource nutzen.",mins:3}
  ];

  const stateKey="campus_resilienz_favoriten";
  let favorites=[];
  try{favorites=JSON.parse(localStorage.getItem(stateKey)||"[]")}catch(e){favorites=[]}

  const cards=exercises.map(x=>`
    <button class="card res-card" onclick="startResilienzUebung('${x.id}')">
      <div class="res-icon">${x.icon}</div>
      <h3>${x.title}</h3>
      <p>${x.desc}</p>
      <span class="pill">ca. ${x.mins} Min.</span>
    </button>`).join("");

  return `${pageHead(
    "RESILIENZ & RESPRESSI",
    "Resilienz & Respressi",
    "Kleine Übungen, die du direkt im Schulalltag ausprobieren kannst.",
    `<button class="primary" onclick="resilienzImpuls()">🎴 Impuls für mich</button>`
  )}
  <style>
    .res-hero{display:grid;grid-template-columns:1.4fr .8fr;gap:16px}
    .res-main{padding:24px;border-radius:16px;background:var(--soft,#f4f7f3)}
    .res-slider{width:100%;margin:14px 0}
    ...
    .res-card{text-align:left;min-height:190px;cursor:pointer;border:1px solid var(--line,#ddd);background:#fff}
    .res-card:hover{transform:translateY(-2px)}
    .res-icon{font-size:30px;margin-bottom:8px}
    .res-card h3{margin:0 0 7px}
    .res-card p{color:var(--muted);margin:0 0 12px}
    .res-tool{min-height:150px}
    .res-fav{margin-top:16px}
    @media(max-width:1050px){.res-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.res-hero{grid-template-columns:1fr}}
  ...
  <div class="res-hero">
    <div class="card res-main">
      ...
      <p>Wähle kurz aus, wie du dich gerade fühlst. Die App schlägt dir passende Übungen vor.</p>
      <input class="res-slider" id="resStress" type="range" min="0" max="10" value="5" oninput="document.getElementById('resStressValue').textContent=this.value">
      <div style="display:flex;justify-content:space-between"><small>ruhig</small><strong id="resStressValue">5</strong><small>sehr angespannt</small></div>
      <div class="res-actions" style="margin-top:14px">
        <button class="primary" onclick="resilienzEmpfehlung()">✨ Passende Übung finden</button>
        <button class="secondary" onclick="resilienzImpuls()">🎴 Zufallsimpuls</button>
      </div>
    </div>
    <div class="card res-tool">
      <div class="kicker">MEIN WERKZEUGKASTEN</div>
      <h2>🧰 Meine Resilienz-Tools</h2>
      <p>Speichere Übungen, die dir im Schulalltag tatsächlich helfen.</p>
      <div class="res-actions">
        <button class="secondary" onclick="showResilienzFavoriten()">Meine Favoriten</button>
      </div>
    </div>
  </div>
  ...
  <div class="card" style="margin-top:16px">
    <div class="page-head">
      <div>
        ...
        <h2>⚡ 2-Minuten-Resets</h2>
        <p>Wähle eine Übung aus und probiere sie direkt aus.</p>
      </div>
    </div>
  ...
  <div class="card" style="margin-top:16px">
    <div class="kicker">MEINE RESILIENZ-WOCHE</div>
    <h2>📈 Was hat mir gutgetan?</h2>
    <p>Markiere Strategien, die du diese Woche ausprobiert hast.</p>
    <div class="res-actions">
      ${["Bewegung","Pause","Atemübung","Kontakt","Humor","Natur","Musik","Hilfe annehmen"].map(x=>`<button class="secondary" onclick="resilienzCheckin('${x}')">${x}</button>`).join("")}
    </div>
  </div>
  ${footer()}`;
}

function resilienzUebungById(id){
  const map={
    atem:["🌬️","Atemtechnik","Atme ruhig ein. Verlängere anschließend sanft die Ausatmung. Wiederhole das einige Male.","120"],
    distanz:["🪟","Distanzierung","Stell dir vor, du schaust kurz von außen auf deine Situation. Was würdest du einem guten Freund raten?","120"],
    leichtigkeit:["🪶","Leichtigkeit","Lass für einen Moment den Anspruch los, alles sofort lösen zu müssen. Was wäre heute ein ausreichend guter nächster Schritt?","120"],
    lachen:["😄","Lachen","Erinnere dich an etwas, das dich zum Lachen gebracht hat. Lass ein kleines Lächeln bewusst zu.","120"],
    boden:["👣","Boden spüren","Stell beide Füße auf den Boden. Spüre Druck, Temperatur und Kontakt. Schau dich anschließend langsam im Raum um.","60"],
    bewegung:["⚡","Bewegungsreset","Steh auf, strecke dich, bewege Schultern und Arme und gehe einige Schritte. Kehre danach bewusst zurück.","120"],
    ressource:["🌱","Ressourcen aktivieren","Denke an eine Situation, in der du etwas Schwieriges bereits bewältigt hast. Was davon kannst du heute nutzen?","120"],
    kontakt:["🤝","Verbindung","Überlege: Wer könnte mir gerade guttun oder mich unterstützen? Entscheide, ob du diese Person kontaktieren möchtest.","120"],
    fokus:["🎯","Fokus","Formuliere den kleinsten nächsten Schritt, den du jetzt tatsächlich tun kannst. Nur diesen einen Schritt.","120"],
    perspektive:["🧠","Perspektivwechsel","Welcher Gedanke belastet dich? Formuliere ihn anschließend so um, dass er realistisch und hilfreich ist.","180"],
    pause:["☕","Bewusste Pause","Lege für einige Minuten das aktuelle Thema beiseite. Atme, trinke etwas oder schaue aus dem Fenster – ohne nebenbei etwas zu erledigen.","180"]
  };
  return map[id]||map.atem;
}

function startResilienzUebung(id){
  const x=resilienzUebungById(id);
  const sec=Number(x[3]);
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    <div class="kicker">RESPRESSI · ÜBUNG</div>
    <h2>${x[0]} ${x[1]}</h2>
    <p style="font-size:18px;line-height:1.55">${x[2]}</p>
    <div style="text-align:center;margin:24px 0">
      <div id="resTimer" style="font-size:46px;font-weight:800">02:00</div>
      <button class="primary" id="resTimerBtn" onclick="toggleResTimer(${sec})">▶ Start</button>
    </div>
    <div class="form-actions">
      <button class="secondary" onclick="toggleResilienzFavorit('${id}')">☆ Zu meinen Tools</button>
      <button class="secondary" onclick="closeModal()">Schließen</button>
    </div>`);
}

let resTimerHandle=null;
let resTimerEnd=0;
function toggleResTimer(seconds){
  const btn=$("resTimerBtn");
  if(resTimerHandle){clearInterval(resTimerHandle);resTimerHandle=null;btn.textContent="▶ Weiter";return}
  if(!resTimerEnd||resTimerEnd<Date.now())resTimerEnd=Date.now()+seconds*1000;
  btn.textContent="⏸ Pause";
  resTimerHandle=setInterval(()=>{
    const left=Math.max(0,resTimerEnd-Date.now());
    const total=Math.ceil(left/1000),m=String(Math.floor(total/60)).padStart(2,"0"),s=String(total%60).padStart(2,"0");
    const el=$("resTimer");if(el)el.textContent=`${m}:${s}`;
    if(left<=0){clearInterval(resTimerHandle);resTimerHandle=null;btn.textContent="✓ Geschafft";toast("Übung beendet – nimm kurz wahr, was sich verändert hat.");}
  },250);
}

function toggleResilienzFavorit(id){
  const key="campus_resilienz_favoriten";let a=[];
  try{a=JSON.parse(localStorage.getItem(key)||"[]")}catch(e){}
  a=a.includes(id)?a.filter(x=>x!==id):[...a,id];
  localStorage.setItem(key,JSON.stringify(a));
  toast(a.includes(id)?"Zu deinen Resilienz-Tools hinzugefügt.":"Aus deinen Resilienz-Tools entfernt.");
}
function showResilienzFavoriten(){
  let a=[];try{a=JSON.parse(localStorage.getItem("campus_resilienz_favoriten")||"[]")}catch(e){}
  if(!a.length){toast("Du hast noch keine Übungen gespeichert.");return}
  const names=a.map(id=>resilienzUebungById(id)[1]).join(", ");
  toast("Deine Tools: "+names);
}
function resilienzImpuls(){
  const ids=["atem","distanz","leichtigkeit","lachen","boden","bewegung","ressource","kontakt","fokus","perspektive","pause"];
  startResilienzUebung(ids[Math.floor(Math.random()*ids.length)]);
}
function resilienzEmpfehlung(){
  const v=Number($("resStress")?.value||5);
  const ids=v<=3?["ressource","leichtigkeit","fokus"]:v<=6?["atem","boden","perspektive","bewegung"]:["atem","boden","distanz","pause","kontakt"];
  startResilienzUebung(ids[Math.floor(Math.random()*ids.length)]);
}
function resilienzCheckin(name){
  const key="campus_resilienz_woche";let a={};
  try{a=JSON.parse(localStorage.getItem(key)||"{}")}catch(e){}
  a[name]=new Date().toISOString();localStorage.setItem(key,JSON.stringify(a));
  toast(`${name}: für diese Woche eingetragen.`);
}
