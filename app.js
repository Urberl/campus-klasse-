/* CAMPUS APP VERSION: 4.0 – FORUM ROBUST FIX */
...
async function getCollection(name,sortField="createdAt",desc=true){
 if(name==="posts"){
  ...
  return data.sort((a,b)=>{
   ...
   const at=av?.seconds ? av.seconds*1000 : (av ? new Date(av).getTime() : 0);
   const bt=bv?.seconds ? bv.seconds*1000 : (bv ? new Date(bv).getTime() : 0);
...
async function render(){
 ...
 try{$("content").innerHTML=await fn()}catch(e){
 console.error("Campus-Seite Fehler:",e);
 const msg=e?.code ? `${e.code}: ${e.message||"Unbekannter Firebase-Fehler"}` : (e?.message||String(e));
 $("content").innerHTML=`<div class="card"><h3>Das Forum konnte nicht geladen werden.</h3><p><strong>Technischer Fehler:</strong><br>${esc(msg)}</p><p style="font-size:.9em;color:var(--muted)">Bitte schicke mir diesen Text, dann kann ich die Ursache gezielt beheben.</p></div>`;
}
...
Object.assign(window,{
 openPostForm,addPost,likePost,commentPost,focusComment,deletePost,openHelpForm,
 openProjectForm,addProject,openJournalForm,addJournal,openCompetenceForm,
 addCompetence,openPracticeForm,addPractice,openCalendarForm,addCalendar,
 openTaskForm,addTask
});
