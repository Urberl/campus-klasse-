...
async function renderPraktikum(){
  ...
  try{
    ...
    // Only genuine fpA teacher assignments belong here.
    data=all.filter(p=>p.module==="fpa" && p.type==="teacherAssignment");
  }catch(e){
    console.error("fpA:",e);
    toast("Die fpA-Praxisaufträge konnten nicht geladen werden.");
  }
  ...
  <div class="notice">
    <b>🧭 Praxisaufträge</b>
    <p style="margin:6px 0 0">Hier erscheinen ausschließlich Aufträge, die als fpA-Praxisauftrag angelegt wurden.</p>
  </div>
  <div class="grid grid-2" style="margin-top:12px">
    ${data.map(p=>`
      <div class="card">
      ...
      <div class="empty">
        <strong>Noch keine fpA-Praxisaufträge.</strong>
        <p>Die Lehrkraft kann hier neue Praxisaufträge anlegen.</p>
      </div>`}
  </div>
  ${footer()}`;
}
...
async function renderKI(){
  ...
  return `${pageHead(
    ...
    "Von einer echten Herausforderung im Praktikumsbetrieb über die Schülerentwicklung bis zum Ergebnis.",
    `<button class="primary" onclick="openKIChallengeForm()">＋ Herausforderung eintragen</button>`
  )}
  <style>
    .ki3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}
    .ki3 .card{min-height:220px;cursor:pointer}
    .ki-num{font-size:28px;font-weight:800;margin-bottom:10px}
    .ki-list{margin-top:16px}
    .ki-row{padding:14px;border:1px solid var(--line,#ddd);border-radius:12px;margin-bottom:10px;background:#fff}
    @media(max-width:850px){.ki3{grid-template-columns:1fr}}
  </style>

  <div class="ki3">
    <div class="card" onclick="openKIChallengesLibrary()">
      <div class="ki-num">1️⃣</div>
      <h2>Herausforderungen in den Praktikumsbetrieben</h2>
      <p>Betriebe tragen konkrete Herausforderungen aus ihrem Arbeitsalltag ein.</p>
      <span class="pill">${challenges.length} Einträge</span>
    </div>

    <div class="card" onclick="openKISolutionsLibrary()">
      <div class="ki-num">2️⃣</div>
      <h2>Schülerteam / Schüler löst Herausforderung</h2>
      <p>Schüler übernehmen eine Herausforderung und dokumentieren Team, Aufgaben, KI-Einsatz und Fortschritt.</p>
      <span class="pill">${solutions.length} Bearbeitungen</span>
    </div>

    <div class="card" onclick="openKIResultsLibrary()">
      <div class="ki-num">3️⃣</div>
      <h2>Ergebnisse, Ideen & Produkte</h2>
      <p>Entstandene Ideen, Konzepte, Prototypen und Produkte werden gesammelt.</p>
      <span class="pill">${results.length} Ergebnisse</span>
    </div>
  </div>

  <div class="card ki-list">
    <h3>Der Ablauf</h3>
    <div class="grid grid-3">
      <div class="card"><b>1 · Praxis</b><small>Ein realer Bedarf wird sichtbar.</small></div>
      <div class="card"><b>2 · Entwicklung</b><small>Schüler bearbeiten ihn mit geeigneten KI-Werkzeugen.</small></div>
      <div class="card"><b>3 · Ergebnis</b><small>Die Lösung wird dokumentiert und für den Praxispartner nutzbar gemacht.</small></div>
    </div>
  </div>
  ${footer()}`;
}
...
function openKIChallengeForm(){
  ...
  <div class="form">
    ...
    <label>Gewünschter Nutzen<textarea id="kiGoal" rows="3" placeholder="Was wäre eine gute Lösung?"></textarea></label>
...
async function saveKIChallenge(){
  ...
  try{
    await addDoc(collection(db,"kiChallenges"),{
      ...
      contact:$("kiContact")?.value.trim()||"",title,description,
  ...
  }catch(e){console.error(e);toast("Herausforderung konnte nicht gespeichert werden.")}
}
...
async function openKIChallengesLibrary(){
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">BIBLIOTHEK</div><h2>Herausforderungen aus Praktikumsbetrieben</h2>
  <div class="list">${a.map(c=>`<div class="ki-row"><strong>${esc(c.title||"Herausforderung")}</strong><small>${esc(c.company||"")}</small><p>${esc(c.description||"")}</p><button class="primary" onclick="openKITakeChallenge('${esc(c.id)}')">Herausforderung übernehmen</button></div>`).join("")||`<div class="empty">Noch keine Herausforderungen.</div>`}</div>
...
async function openKITakeChallenge(id){
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">BEARBEITUNG STARTEN</div><h2>${esc(c.title)}</h2><p>${esc(c.description)}</p>
  <div class="form">
    ...
    <label>Name / Team<input id="kiTeam"></label>
    <label>Mitglieder<textarea id="kiMembers" rows="3"></textarea></label>
    <label>Aufgabenverteilung / Wer macht was?<textarea id="kiRoles" rows="4"></textarea></label>
    <label>Geplanter KI-Einsatz<textarea id="kiAI" rows="4" placeholder="Welche KI-Werkzeuge sollen wofür eingesetzt werden?"></textarea></label>
    <div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="saveKISolution('${esc(c.id)}')">Bearbeitung speichern</button></div>
  </div>`);
}
...
async function saveKISolution(challengeId){
  ...
  try{
    await addDoc(collection(db,"kiSolutions"),{
      module:"kiInnovationspartnerschaften",challengeId,
      mode:$("kiMode")?.value||"team",team,members:$("kiMembers")?.value.trim()||"",
      roles:$("kiRoles")?.value.trim()||"",aiUse:$("kiAI")?.value.trim()||"",
      status:"in Bearbeitung",createdBy:currentUser.uid,createdAt:serverTimestamp()
    });
    closeModal();await render();toast("Bearbeitung gespeichert.");
  }catch(e){console.error(e);toast("Bearbeitung konnte nicht gespeichert werden.")}
}

async function openKISolutionsLibrary(){
  let a=[];try{a=await getCollection("kiSolutions","createdAt",true)}catch(e){console.error(e)}
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">BEARBEITUNGEN</div><h2>Schülerteams & Lösungsentwicklung</h2>
  <div class="list">${a.map(s=>`<div class="ki-row"><strong>${esc(s.team||"Schüler/in")}</strong><small>${s.mode==="team"?"Schülerteam":"Einzelschüler/in"} · ${esc(s.status||"")}</small><p><b>Verantwortlichkeiten:</b> ${esc(s.roles||"—")}</p><p><b>KI-Einsatz:</b> ${esc(s.aiUse||"—")}</p></div>`).join("")||`<div class="empty">Noch keine Bearbeitungen.</div>`}</div>
  <div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button></div>`);
}

function openKIResultForm(){
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">ERGEBNIS</div><h2>Ergebnis dokumentieren</h2>
  <div class="form">
    <label>Titel<input id="kiResultTitle"></label>
    ...
    <label>Schülerteam / Schüler<input id="kiResultTeam"></label>
    <label>Praxispartner<input id="kiResultPartner"></label>
    <label>Link zum Ergebnis<input id="kiResultLink"></label>
    <div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="saveKIResult()">Ergebnis speichern</button></div>
  </div>`);
}
...
async function saveKIResult(){
  ...
  }catch(e){console.error(e);toast("Ergebnis konnte nicht gespeichert werden.")}
}

async function openKIResultsLibrary(){
  let a=[];try{a=await getCollection("kiResults","createdAt",true)}catch(e){console.error(e)}
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">ERGEBNISSE</div><h2>Ergebnisse, Ideen & Produkte</h2>
  <div class="list">${a.map(r=>`<div class="ki-row"><strong>${esc(r.title||"Ergebnis")}</strong><small>${esc(r.type||"")}</small><p>${esc(r.description||"")}</p><div class="chips">${r.team?`<span class="chip">Team: ${esc(r.team)}</span>`:""}${r.partner?`<span class="chip">Partner: ${esc(r.partner)}</span>`:""}${r.link?`<a href="${esc(r.link)}" target="_blank" rel="noopener">Ergebnis öffnen</a>`:""}</div></div>`).join("")||`<div class="empty">Noch keine Ergebnisse.</div>`}</div>
...
async function renderKalender(){
  ...
  const schoolMonths=[];
  for(let m=7;m<=11;m++) schoolMonths.push(new Date(2026,m,1));
  for(let m=0;m<=6;m++) schoolMonths.push(new Date(2027,m,1));

  const byDate={};
  data.forEach(c=>{
    const d=String(c.date||"").slice(0,10);
    if(d){(byDate[d] ||= []).push(c)}
  });
  ...
  const monthHtml=schoolMonths.map(month=>{
    const y=month.getFullYear(), mo=month.getMonth();
    const first=new Date(y,mo,1);
    const days=new Date(y,mo+1,0).getDate();
    const offset=(first.getDay()+6)%7;
    let cells="";
    for(let i=0;i<offset;i++) cells+=`<div class="campus-cal-day empty-day"></div>`;
    for(let d=1;d<=days;d++){
      const key=`${y}-${String(mo+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
      const events=byDate[key]||[];
      const has=events.length>0;
      cells+=`<button type="button" class="campus-cal-day ${has?"has-event":""} ${key===todayKey?"today":""}" onclick="showCalendarDay('${key}')" aria-label="${d}. ${monthNames[mo]} ${y}${has?` – ${events.length} Termin${events.length===1?"":"e"}`:""}">
        <span>${d}</span>${has?`<i>${events.length>1?events.length:""}</i>`:""}
      </button>`;
    }
    return `<section class="campus-month ${Object.keys(byDate).some(k=>k.startsWith(`${y}-${String(mo+1).padStart(2,"0")}-`))?"month-has-events":""}">
      <div class="campus-month-head"><strong>${monthNames[mo]}</strong><span>${y}</span></div>
      <div class="campus-weekdays">${weekdays.map(w=>`<span>${w}</span>`).join("")}</div>
      <div class="campus-days">${cells}</div>
    </section>`;
  }).join("");

  window.showCalendarDay=function(key){
    const events=byDate[key]||[];
    const [y,m,d]=key.split("-").map(Number);
    const label=`${d}. ${monthNames[m-1]} ${y}`;
    modal(`<button class="modal-close" onclick="closeModal()">×</button>
      <div class="kicker">CAMPUS-KALENDER</div><h2>${label}</h2>
      ${events.length?`<div class="list">${events.map(c=>`<div class="list-item"><div><strong>${esc(c.title||"Termin")}</strong><small>${esc(c.description||"")}</small></div><span class="pill green">Termin</span></div>`).join("")}</div>`:`<div class="empty">An diesem Tag ist noch kein Termin eingetragen.</div>`}
      <div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button></div>`);
  };

  return `${pageHead("ORGANISATION","Campus-Kalender","Schuljahr 2026/27 – alle Monate auf einen Blick. Ein Punkt zeigt, dass in diesem Monat bereits Termine eingetragen sind.",`<button class="primary" onclick="openCalendarForm()">＋ Termin</button>`)}
  <style>
    .campus-cal-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}
    .campus-month{border:1px solid var(--line,#ddd);border-radius:14px;padding:12px;background:var(--card,#fff)}
    .campus-month.month-has-events{border-color:var(--accent,#7b9b82)}
    .campus-month-head{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:9px}
    .campus-month-head strong{font-size:18px}.campus-month-head span{font-size:12px;color:var(--muted)}
    .campus-weekdays,.campus-days{display:grid;grid-template-columns:repeat(7,1fr);gap:3px}
    .campus-weekdays span{text-align:center;font-size:10px;color:var(--muted);padding:3px 0}
    .campus-cal-day{position:relative;min-width:0;aspect-ratio:1;border:0;border-radius:7px;background:transparent;color:inherit;cursor:pointer;font:inherit;font-size:12px;padding:0}
    .campus-cal-day:hover{background:var(--soft,#f3f5f2)}
    .campus-cal-day.has-event{font-weight:700;background:var(--accent-bg-subtle,#edf3ed)}
    .campus-cal-day.has-event i{position:absolute;right:3px;top:2px;width:6px;height:6px;border-radius:50%;background:var(--accent,#7b9b82);font-size:0}
    .campus-cal-day.has-event i:not(:empty){width:14px;height:14px;line-height:14px;color:var(--accent-text,#fff);font-size:8px;font-style:normal;text-align:center;top:2px;right:2px}
    .campus-cal-day.today{outline:2px solid var(--accent,#7b9b82);outline-offset:-2px}
    .empty-day{visibility:hidden;cursor:default}
    @media(max-width:1050px){.campus-cal-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:650px){.campus-cal-grid{grid-template-columns:1fr}.campus-cal-day{font-size:13px}}
  </style>
  <div class="notice" style="margin-bottom:12px"><b>Schuljahr 2026/27</b> · August 2026 bis Juli 2027 · <span style="display:inline-flex;align-items:center;gap:5px"><i style="display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--accent,#7b9b82)"></i> Termin vorhanden</span> · Klicke auf einen Tag, um die Termine dieses Tages zu sehen.</div>
  <div class="campus-cal-grid">${monthHtml}</div>
  ${footer()}`;
}
