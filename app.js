...
function updateTeacherTeamNav(){
  ...
  const links=all.filter(link=>{
    ...
    return link.matches('a[href="#team"],button[data-page="team"],.nav-link[data-page="team"]')
      || /Team\s*&\s*SQ/i.test(text)
      || /Lehrkräfte\s+Klassenteam/i.test(text);
  });
  const teamLinks=links.filter(link=>link.matches('a[href="#team"],button[data-page="team"],.nav-link[data-page="team"]'));
  const canonical=teamLinks[0]||links[0];

  links.forEach(link=>{
    if(link!==canonical && links.length>1){
      link.hidden=true;
      link.setAttribute("aria-hidden","true");
      return;
    }
    const label=link.querySelector(".nav-label")||link.querySelector("strong,span")||link;
    if(label){
      // Remove only text nodes of the label, preserving icons/SVG elements.
...
async function renderStart(){
 let tasks=[],projects=[],posts=[],nextCalendar=null;
 try{[tasks,projects,posts,nextCalendar]=await Promise.all([getCollection("tasks","deadline",false),getCollection("projects"),getCollection("posts"),getUpcomingCampusCalendarEvent()])}catch(e){}
...
class="list-item"><div><strong>${esc(p.title||p.text)}</strong>${p.title?`<small>${esc(p.text)} · ${fmtDate(p.createdAt)}</small>`:`<small>${fmtDate(p.createdAt)}</small>`}</div><span class="pill">Info</span>
...
async function renderFragenHilfe(){
  const faqs=[
    ["Was ist die Campusklasse?","Die Campusklasse verbindet Lernen, Projekte, Praxis, Kompetenzentwicklung und Gemeinschaft. Du arbeitest zunehmend selbstständig und gestaltest deinen Lernweg aktiv mit."],
    ["Wie funktioniert das Lernen in der Campusklasse?","Du setzt dir Ziele, planst deine nächsten Schritte, bearbeitest Lernaufträge und reflektierst deinen Lernweg. Die Lernwerkstatt unterstützt dich dabei mit Methoden, Lernressourcen, Lernimpulsen, Lernstandsmessungen und KI zum Lernen."],
    ...
    ["Was ist die Lernwerkstatt?","Die Lernwerkstatt ist dein Bereich für selbstständiges Lernen. Dort findest du Lernpfade, Lernressourcen, Lernimpulse, Lernstandsmessungen, KI zum Lernen und weitere Unterstützung."],
    ["Wie nutze ich KI zum Lernen?","KI kann dich beim Verstehen, Üben, Prüfen und Reflektieren unterstützen. Nutze sie als Lernpartner und prüfe die Antworten kritisch, statt fertige Lösungen einfach zu übernehmen."],
    ["Was mache ich, wenn ich nicht weiterkomme?","Formuliere möglichst konkret, was du gerade bearbeitest und an welcher Stelle du nicht weiterkommst. Nutze passende Lernressourcen oder KI und wende dich bei Bedarf an deine Lehrkraft bzw. das Klassenteam."],
    ...
    ["Was ist ein Lernjournal?","Im Lernjournal dokumentierst und reflektierst du deinen Lernweg: Was habe ich gelernt? Was hat funktioniert? Wo gab es Schwierigkeiten? Was ist mein nächster Schritt?"],
    ["Was sind Lernstandsmessungen?","Lernstandsmessungen helfen dir zu erkennen, wo du bei deinen Kompetenzen stehst und woran du als Nächstes arbeiten solltest. So kann deine Kompetenzentwicklung sichtbar werden."],
    ["Wo finde ich die Termine?","Im Campus-Kalender findest du die wichtigen Termine der Campusklasse. Auch die bayerischen Schulferien für das Schuljahr 2026/27 sind dort farblich gekennzeichnet."],
    ["Wer hilft mir bei Fragen zur Campusklasse?","Wenn deine Frage hier nicht beantwortet wird, kannst du dich an deine Lehrkraft bzw. das Klassenteam wenden."]
  ];
  return `${pageHead("ORIENTIERUNG","Fragen & Hilfe","Antworten rund um die Campusklasse und das Lernen.")}
  <div class="card" style="margin-bottom:16px;background:var(--soft-green)">
    <span class="badge">💡 ORIENTIERUNG</span>
    <h2>Fragen zur Campusklasse?</h2>
    <p>Hier findest du Antworten auf wichtige Fragen rund um die Campusklasse, selbstständiges Lernen und deinen Lernweg.</p>
  </div>
...
async function addNews(){
  ...
  try{await addDoc(collection(db,"posts"),{authorUid:currentUser.uid,authorName:profile?.displayName||currentUser?.email||"Lehrkraft",type:"info",title,text,likes:0,comments:[],createdAt:serverTimestamp()});closeModal();await render();toast("News veröffentlicht.")}catch(e){console.error(e);toast("News konnte nicht veröffentlicht werden.")}
}
