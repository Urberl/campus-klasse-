...
async function renderKalender(){
  ...
  const eventTypeClass=e=>{
    const t=String(e.type||"sonstiges").toLowerCase();
    if(t.includes("schulaufgabe"))return "event-schulaufgabe";
    if(t.includes("kurzarbeit"))return "event-kurzarbeit";
    if(t.includes("projekt"))return "event-projekt";
    if(t.includes("referat"))return "event-referat";
    if(t.includes("präsentation")||t.includes("praesentation"))return "event-praesentation";
    return "event-sonstiges";
  };
  const eventTypeLabel=e=>e.type||"Sonstiger Termin";
  const dateVal=e=>{
  ...
  const monthHTML=(y,m,name)=>{
    ...
    for(let d=1;d<=days;d++){
      const ds=eventsForDay(y,m,d),primary=ds[0];
      const eventClasses=ds.map(eventTypeClass).join(" ");
      cells.push(`<button type="button" class="cal-day ${ds.length?`has-event ${eventClasses}`:""}" onclick="openCalendarDay(${y},${m},${d})"><span class="cal-num">${d}</span>${ds.length?`<span class="cal-event-label">${esc(eventTypeLabel(primary))}</span><span class="cal-count">${ds.length>1?`+${ds.length-1}`:""}</span>`:""}</button>`);
    }
  ...
  const html=`${pageHead("ORGANISATION","Campus-Kalender","Termine sind direkt im Kalender nach Terminart farblich erkennbar. Termine können nur von Lehrkräften eingetragen werden.",teacher?'<button id="calendarAddBtn" class="primary" type="button">＋ Termin eintragen</button>':'')}
  <style>
    .cal-months{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}.cal-month{padding:18px}.cal-month-head{margin-bottom:10px}.cal-week,.cal-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:4px}.cal-week{font-size:12px;color:var(--muted);text-align:center}.cal-day{min-height:64px;border:1px solid var(--line);border-radius:9px;background:#fff;padding:7px;text-align:left;cursor:pointer;transition:.15s;position:relative}.cal-day:hover{transform:translateY(-1px)}.cal-day.empty{border:0;background:transparent;cursor:default}.cal-day.has-event{font-weight:700}.cal-event-label{display:block;margin-top:6px;font-size:10px;line-height:1.2;padding:3px 5px;border-radius:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cal-count{display:block;font-size:10px;margin-top:3px}.event-schulaufgabe{background:#dbeafe;border-color:#60a5fa}.event-schulaufgabe .cal-event-label{background:#bfdbfe;color:#1e3a8a}.event-kurzarbeit{background:#fee2e2;border-color:#f87171}.event-kurzarbeit .cal-event-label{background:#fecaca;color:#991b1b}.event-projekt{background:#dcfce7;border-color:#4ade80}.event-projekt .cal-event-label{background:#bbf7d0;color:#166534}.event-referat{background:#fef3c7;border-color:#fbbf24}.event-referat .cal-event-label{background:#fde68a;color:#92400e}.event-praesentation{background:#f3e8ff;border-color:#c084fc}.event-praesentation .cal-event-label{background:#e9d5ff;color:#6b21a8}.event-sonstiges{background:#e5e7eb;border-color:#9ca3af}.event-sonstiges .cal-event-label{background:#d1d5db;color:#374151}.cal-legend{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.cal-legend-item{padding:5px 9px;border-radius:7px;font-size:12px;font-weight:700}.cal-legend-item.schulaufgabe{background:#dbeafe;color:#1e3a8a}.cal-legend-item.kurzarbeit{background:#fee2e2;color:#991b1b}.cal-legend-item.projekt{background:#dcfce7;color:#166534}.cal-legend-item.referat{background:#fef3c7;color:#92400e}.cal-legend-item.praesentation{background:#f3e8ff;color:#6b21a8}.cal-legend-item.sonstiges{background:#e5e7eb;color:#374151}@media(max-width:800px){.cal-months{grid-template-columns:1fr}.cal-day{min-height:58px}.cal-event-label{font-size:9px}}
  </style>
  <div class="card" style="margin-bottom:16px"><strong>Campus-Kalender</strong><p>Termine werden im gemeinsamen Kalender gespeichert. Klicke auf einen Tag, um vorhandene Termine zu sehen.</p><div class="cal-legend"><span class="cal-legend-item schulaufgabe">Schulaufgabe</span><span class="cal-legend-item kurzarbeit">Kurzarbeit</span><span class="cal-legend-item projekt">Projektvorstellung</span><span class="cal-legend-item referat">Referat</span><span class="cal-legend-item praesentation">Präsentation</span><span class="cal-legend-item sonstiges">Sonstiger Termin</span></div></div>
  ...
  modal(`<button class="modal-close" onclick="closeModal()">×</button>
    ...
    <div class="list">${day.map(e=>`
      <div class="card" style="margin-bottom:10px">
        <span class="pill">${esc(e.type||"Sonstiger Termin")}</span>
        <h3>${esc(e.title||e.name||"Termin")}</h3>
        ${e.time?`<p><b>Uhrzeit:</b> ${esc(e.time)} Uhr</p>`:""}
        <p>${esc(e.description||e.text||"")}</p>
        ${e.location?`<small>📍 ${esc(e.location)}</small>`:""}
      </div>`).join("")||`<div class="empty">An diesem Tag ist noch kein Termin eingetragen.</div>`}</div>
    <div class="form-actions"><button class="secondary" onclick="closeModal()">Schließen</button>
      ${isTeacher()?`<button class="primary" onclick="closeModal();setTimeout(openCalendarForm,50)">＋ Termin eintragen</button>`:""}</div>`);
}
...
function openCalendarForm(){
  ...
  modal(`<button class="modal-close" type="button" onclick="closeModal()">×</button><div class="kicker">CAMPUS-KALENDER · LEHRKRAFT</div><h2>Neuen Termin eintragen</h2>
  <div class="form">
  <label>Terminart *<select id="calType">
    ...
    <option value="Sonstiger Termin">Sonstiger Termin / frei wählbar</option>
  </select></label>
  <label>Titel *<input id="calTitle" type="text" placeholder="z. B. Entwicklungspsychologie – Bindung"></label>
  ...
  <label>Weitere Informationen<textarea id="calDescription" rows="4" placeholder="Alles, was Schülerinnen und Schüler zum Termin wissen sollten …"></textarea></label>
  <div class="notice"><strong>Hinweis:</strong> Der Termin wird nach der Terminart automatisch farblich im Kalender gekennzeichnet.</div>
...
async function addCalendar(){
  if(!currentUser){toast("Bitte zuerst anmelden.");return}
  if(!isTeacher()){toast("Nur Lehrkräfte können Termine eintragen.");return}
  const type=$("calType")?.value||"Sonstiger Termin",title=$("calTitle")?.value.trim()||"",date=$("calDate")?.value||"",time=$("calTime")?.value||"",location=$("calLocation")?.value.trim()||"",description=$("calDescription")?.value.trim()||"";
  if(!title){toast("Bitte einen Titel eingeben.");return}
  if(!date){toast("Bitte ein Datum auswählen.");return}
  const payload={title,type,date,start:date,time,location,description,createdBy:currentUser.uid,createdByName:profile?.displayName||currentUser.email||"Lehrkraft",createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
