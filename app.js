...
async function renderJournal(){
  ...
  }catch(e){console.error("Lernjournal laden:",e);loadError=e?.message||"Unbekannter Fehler"}
  ...
  return `${JOURNAL_UI_STYLE}${pageHead("REFLEXION","Lernjournal","Dein Lernweg, Reflexionen und nächste Schritte.")}
<div class="journal-actions-grid">
<section class="journal-action-card"><span class="emoji">✍️</span><h2>Lernjournal erstellen</h2><p>Halte deine Lernerfahrungen ausführlich fest. Schreibe mehrere Absätze und reflektiere deinen persönlichen Lernweg.</p><div class="grow"></div><button class="primary" onclick="window.openJournalForm()">＋ Lernjournal erstellen</button></section>
<section class="journal-action-card"><span class="emoji">📚</span><h2>Lernjournal-Bibliothek</h2><p>Hier werden deine bisherigen Einträge übersichtlich nach Datum und Titel gespeichert.</p><div class="grow"></div><button class="secondary" onclick="window.openJournalLibrary()">Bibliothek öffnen →</button></section>
</div>
<div class="card journal-library-card" id="journalLibraryPanel"><div class="journal-library-head"><div><span class="badge">📚 BIBLIOTHEK</span><h2>Meine Lernjournal-Einträge</h2><p>${entries.length} gespeicherte Einträge</p></div>${entries.length?`<button class="secondary" onclick="window.printMyJournalPDF()">📄 Alle als PDF</button>`:""}</div>
${loadError?`<div class="notice"><b>Lernjournal konnte nicht geladen werden.</b><br>${esc(loadError)}</div>`:entries.length?`<div class="journal-table"><div class="journal-table-head"><span>DATUM</span><span>TITEL</span><span>PDF</span></div>${rows}</div>`:`<div class="journal-empty"><strong>Noch keine Einträge</strong><br>Erstelle deinen ersten ausführlichen Lernjournal-Eintrag.</div>`}
</div>${footer()}`;
}
...
function openJournalForm(){
  ...
  <div class="kicker">LERNJOURNAL</div><h2>Ausführliche Lernreflexion</h2>
