...
async function renderResilienz(){
  const tools=[
    ['🌬️','Resonanzatmung','4 Sekunden einatmen · 6 Sekunden ausatmen','Atem'],
    ['👣','Boden spüren','Mit den Sinnen wieder im Hier und Jetzt ankommen','Körper'],
    ['🪟','Distanzierung','Eine belastende Situation kurz von außen betrachten','Gedanken'],
    ['🪶','Leichtigkeit','Den inneren Druck für einen Moment reduzieren','Gedanken'],
    ['⚡','Bewegungsreset','Kurz bewegen und danach bewusst zurückkehren','Körper'],
    ['🌱','Ressource aktivieren','Eine eigene Stärke oder hilfreiche Erfahrung aktivieren','Ressourcen'],
    ['🤝','Verbindung','Soziale Unterstützung bewusst nutzen','Beziehungen'],
    ['🎯','Fokus','Den kleinsten machbaren nächsten Schritt finden','Handeln']
  ];
  return `${pageHead('RESILIENZ & RESPRESSI','Resilienz & Respressi','Kleine interaktive Übungen für den Schulalltag.',`<button class="primary" onclick="resilienzImpuls()">🎴 Impuls für mich</button>`)}
  <style>
    .res-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.res-card{text-align:left;min-height:185px;cursor:pointer}.res-icon{font-size:30px;margin-bottom:8px}.res-tag{display:inline-block;margin-top:7px}
    .res-intro{margin-bottom:16px}.res-week{margin-top:16px}.res-checks{display:flex;flex-wrap:wrap;gap:8px}.res-check{min-width:120px}
    .breath-wrap{text-align:center;padding:8px 0}.breath-circle{width:150px;height:150px;border-radius:50%;margin:18px auto;display:flex;align-items:center;justify-content:center;border:4px solid currentColor;transition:transform 4s ease-in-out,opacity .4s}.breath-circle.inhale{transform:scale(1.22)}.breath-circle.exhale{transform:scale(.82)}.breath-phase{font-size:24px;font-weight:700}.breath-time{font-size:42px;font-weight:800;margin-top:10px}.breath-hint{font-size:17px;line-height:1.5;min-height:52px}.breath-progress{height:10px;border-radius:99px;background:rgba(0,0,0,.08);overflow:hidden;margin:16px 0}.breath-progress>div{height:100%;width:0%;transition:width .2s linear;background:currentColor}
    .res-task{padding:16px;border:1px solid var(--line,#ddd);border-radius:12px;margin-top:16px}.res-task textarea{width:100%;min-height:90px}.res-scale{width:100%}
    @media(max-width:900px){.res-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:600px){.res-grid{grid-template-columns:1fr}}
  </style>
  <div class="card res-intro"><div class="kicker">DEIN MOMENT</div><h2>Was brauchst du gerade?</h2><p>Wähle eine Übung und probiere sie direkt aus. Es geht nicht darum, etwas „richtig“ zu machen – sondern wahrzunehmen, was dir gerade hilft.</p><div class="res-grid">${tools.map((x,i)=>`<button class="card res-card" onclick="startResilienzUebung(${i})"><div class="res-icon">${x[0]}</div><h3>${x[1]}</h3><p>${x[2]}</p><span class="pill res-tag">${x[3]}</span></button>`).join('')}</div></div>
  <div class="card res-week"><div class="kicker">MEINE RESILIENZ-WOCHE</div><h2>📈 Was hat mir gutgetan?</h2><p>Markiere Strategien, die du diese Woche ausprobiert hast.</p><div class="res-checks">${['Bewegung','Pause','Atemübung','Kontakt','Humor','Natur','Musik','Hilfe annehmen'].map(x=>`<button class="secondary res-check" onclick="resilienzCheckin('${x}')">${x}</button>`).join('')}</div></div>${footer()}`;
}
function resilienzImpuls(){startResilienzUebung(Math.floor(Math.random()*8));}
function startResilienzUebung(i){
  const a=[
    ['🌬️','Resonanzatmung','4 Sekunden einatmen · 6 Sekunden ausatmen. Die Animation gibt den Rhythmus vor.'],
    ['👣','Boden spüren','Stell beide Füße auf den Boden. Spüre nacheinander Druck, Temperatur und Kontakt. Schau anschließend langsam drei Dinge im Raum an.'],
    ['🪟','Distanzierung','Denke kurz an das, was dich gerade beschäftigt. Stell dir vor, du schaust von außen auf die Situation. Was würdest du einem guten Freund raten?'],
    ['🪶','Leichtigkeit','Du musst nicht alles auf einmal lösen. Formuliere: „Für heute reicht als nächster Schritt …“ und wähle nur einen kleinen Schritt.'],
    ['⚡','Bewegungsreset','Steh auf. Kreise Schultern und Arme, strecke dich und gehe einige Schritte. Kehre anschließend bewusst an deinen Platz zurück.'],
    ['🌱','Ressource aktivieren','Denke an eine Situation, die du trotz einer Schwierigkeit bewältigt hast. Welche Stärke oder Strategie von damals kannst du heute gebrauchen?'],
    ['🤝','Verbindung','Überlege: Wer könnte dir gerade guttun oder dich unterstützen? Entscheide, ob du diese Person heute ansprechen möchtest.'],
    ['🎯','Fokus','Formuliere den kleinsten nächsten Schritt, den du jetzt tatsächlich tun kannst. Nur diesen einen Schritt – nicht die ganze Aufgabe.']
  ];
  const x=a[i]||a[0];
  if(i===0){openResonanzatmung();return;}
  const tasks={
    1:{q:'Was hast du gerade wahrgenommen?',placeholder:'z. B. „Meine Füße fühlen sich warm an …“',button:'✓ Wahrgenommen'},
    2:{q:'Was würdest du deinem guten Freund raten?',placeholder:'Ein kurzer hilfreicher Satz …',button:'✓ Perspektive gefunden'},
    3:{q:'Was ist dein ausreichend guter nächster Schritt?',placeholder:'Nur einen kleinen Schritt notieren …',button:'✓ Nächsten Schritt festgelegt'},
    4:{q:'Was hat sich durch die Bewegung verändert?',placeholder:'z. B. „Ich fühle mich wacher …“',button:'✓ Veränderung wahrgenommen'},
    5:{q:'Welche Stärke oder Erfahrung möchtest du mitnehmen?',placeholder:'z. B. Geduld, Humor, Durchhaltevermögen …',button:'✓ Ressource aktiviert'},
    6:{q:'Wer könnte dir guttun oder dich unterstützen?',placeholder:'Name oder Art der Unterstützung …',button:'✓ Unterstützung erkannt'},
    7:{q:'Was ist dein kleinster nächster Schritt?',placeholder:'Ich mache jetzt …',button:'✓ Fokus gesetzt'}
  };
  const t=tasks[i];
  modal(`<button class="modal-close" onclick="closeModal()">×</button><div class="kicker">RESPRESSI · ${esc(x[1].toUpperCase())}</div><h2>${x[0]} ${esc(x[1])}</h2><p style="font-size:18px;line-height:1.55">${esc(x[2])}</p><div class="res-task"><label for="resTaskInput"><strong>${esc(t.q)}</strong></label><textarea id="resTaskInput" placeholder="${esc(t.placeholder)}"></textarea></div><div class="form-actions"><button class="secondary" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="resilienzAbschluss('${esc(t.button)}')">${esc(t.button)}</button></div>`);
}
function resilienzAbschluss(message){toast(message+' – nimm kurz wahr, was sich verändert hat.');closeModal();}
let resonanzTimer=null,resonanzRunning=false,resonanzEnd=0,resonanzElapsed=0;
function openResonanzatmung(){
  modal(`<button class="modal-close" onclick="stopResonanzTimer();closeModal()">×</button><div class="kicker">RESPRESSI · RESONANZATMUNG</div><h2>🌬️ 4 Sekunden ein · 6 Sekunden aus</h2><p style="font-size:17px;line-height:1.5">Atme entspannt durch die Nase ein, während der Kreis größer wird. Danach langsam und ohne Druck ausatmen, während der Kreis kleiner wird.</p><div class="breath-wrap"><div id="breathCircle" class="breath-circle"><span id="breathPhase" class="breath-phase">Bereit</span></div><div id="breathClock" class="breath-time">02:00</div><div id="breathHint" class="breath-hint">Starte die Übung. Der Rhythmus läuft automatisch: 4 s ein · 6 s aus.</div><div class="breath-progress"><div id="breathProgress"></div></div><button class="primary" id="breathStart" onclick="toggleResonanzTimer()">▶ Start</button></div><div class="form-actions"><button class="secondary" onclick="stopResonanzTimer();closeModal()">Schließen</button></div>`);
}
function toggleResonanzTimer(){
  const btn=$('breathStart');
  if(!btn)return;
  if(resonanzRunning){resonanzRunning=false;clearInterval(resonanzTimer);resonanzTimer=null;btn.textContent='▶ Weiter';return;}
  if(!resonanzEnd)resonanzEnd=Date.now()+120000;
  resonanzRunning=true;btn.textContent='⏸ Pause';
  resonanzTimer=setInterval(updateResonanzTimer,50);updateResonanzTimer();
}
function updateResonanzTimer(){
  const left=Math.max(0,resonanzEnd-Date.now());
  ...
  if(progress)progress.style.width=`${Math.min(100,100-(left/120000*100))}%`;
  if(left<=0){stopResonanzTimer();if(phase)phase.textContent='Geschafft';if(hint)hint.textContent='Nimm kurz wahr: Was hat sich verändert?';const b=$('breathStart');if(b)b.textContent='✓ Beendet';return;}
  const elapsed=120000-left;const cycle=elapsed%10000;
  if(cycle<4000){if(circle){circle.classList.add('inhale');circle.classList.remove('exhale')}if(phase)phase.textContent='Einatmen · '+Math.ceil((4000-cycle)/1000)+' s';if(hint)hint.textContent='Langsam einatmen …';}
  else {if(circle){circle.classList.add('exhale');circle.classList.remove('inhale')}if(phase)phase.textContent='Ausatmen · '+Math.ceil((10000-cycle)/1000)+' s';if(hint)hint.textContent='Langsam und entspannt ausatmen …';}
}
function stopResonanzTimer(){if(resonanzTimer){clearInterval(resonanzTimer);resonanzTimer=null}resonanzRunning=false;resonanzEnd=0;}
...
async function renderKI(){
  ...
  <div class="card ki-process">
    ...
    <div class="grid grid-3">
      <div class="card"><strong style="display:block;margin-bottom:6px">1. Praxisproblem</strong><small>Ein realer Bedarf wird beschrieben.</small></div>
      <div class="card"><strong style="display:block;margin-bottom:6px">2. Entwicklung</strong><small>Ein Schülerteam bearbeitet die Herausforderung.</small></div>
      <div class="card"><strong style="display:block;margin-bottom:6px">3. Ergebnis</strong><small>Die Lösung wird dokumentiert.</small></div>
    </div>
  </div>${footer()}`;
}
...
window.startResilienzUebung=startResilienzUebung;
