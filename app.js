...
$("loginForm").addEventListener("submit",async e=>{
 e.preventDefault();
 $("authError").textContent="";
 if(!configReady){$("authError").textContent="Firebase ist nicht konfiguriert.";return}
 const email=$("loginEmail").value.trim();
 const password=$("loginPassword").value;
 if(!email||!password){$("authError").textContent="Bitte E-Mail-Adresse und Passwort eingeben.";return}
 try{
    await loadFirebase();
    await signInWithEmailAndPassword(auth,email,password);
    // Der Seitenwechsel erfolgt zentral über onAuthStateChanged.
 }catch(err){
    console.error("Campus Login:",err);
    authError(err);
 }
});
...
if(profile.status === "blocked"){
  ...
  showApp();
         ...
         console.error("Campus Benutzerprofil:",e);
         showAuth();
         const code=e?.code||"";
         $("authError").textContent=code==="permission-denied"
           ? "Firebase verweigert den Zugriff auf das Benutzerprofil. Bitte die veröffentlichten Firestore-Regeln prüfen."
           : `Benutzerprofil konnte nicht geladen werden${code?` (${code})`:""}.`;
       }
    });
  }catch(e){
