...
async function sendForumPost(){
  const input=document.getElementById("forumMessage");
  const text=(input?.value||"").trim();
  if(!text){
    toast("Bitte zuerst eine Nachricht eingeben.");
    return;
  }
  if(!currentUser){
  ...
  const button=document.getElementById("forumSendButton");
  if(button) button.disabled=true;

  try{
    await addDoc(collection(db,"forumPosts"),{
      uid:currentUser.uid,
      authorName:currentUser.displayName || currentUser.email?.split("@")[0] || "Schüler/in",
      text:text,
      createdAt:serverTimestamp(),
      createdAtMs:Date.now()
    });
    if(input) input.value="";
    toast("Beitrag wurde gesendet.");
    await render();
  }catch(e){
    console.error("Forum senden:",e);
    toast("Der Beitrag konnte nicht gesendet werden. Bitte prüfe die Firestore-Regeln.");
  }finally{
    if(button) button.disabled=false;
  }
}
window.sendForumPost=sendForumPost;

async function renderForum(){
  let posts=[];
  try{
    const q=query(collection(db,"forumPosts"),orderBy("createdAt","desc"),limit(50));
    const snap=await getDocs(q);
    posts=snap.docs.map(d=>({id:d.id,...d.data()}));
  }catch(e){
    console.error("Forum laden:",e);
    // Fallback, falls createdAt bei älteren Dokumenten fehlt.
    try{
      const snap=await getDocs(query(collection(db,"forumPosts"),limit(50)));
      posts=snap.docs.map(d=>({id:d.id,...d.data()}))
        .sort((a,b)=>(b.createdAtMs||0)-(a.createdAtMs||0));
    }catch(e2){
      console.error("Forum-Fallback:",e2);
    }
  }

  const formatDate=(p)=>{
    const ts=p.createdAt?.toDate ? p.createdAt.toDate() : (p.createdAtMs ? new Date(p.createdAtMs) : null);
    return ts ? ts.toLocaleString("de-DE",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}) : "gerade eben";
  };

  return `${pageHead("AUSTAUSCH","Forum","Fragen stellen, Ideen teilen und voneinander lernen.",`<button class="primary" onclick="document.getElementById('forumMessage')?.focus()">✏️ Beitrag schreiben</button>`)}
    <div class="card">
      <h3>Neuen Beitrag schreiben</h3>
      <textarea id="forumMessage" rows="4" placeholder="Was möchtest du mit der Gruppe teilen?"></textarea>
      <div style="display:flex;justify-content:flex-end;margin-top:10px">
        <button id="forumSendButton" class="primary" onclick="sendForumPost()">Senden</button>
      </div>
    </div>
      ...
      ${posts.length ? posts.map(p=>`
        <div class="card">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">
            <div>
              <strong>${esc(p.authorName||"Schüler/in")}</strong>
              <small style="display:block;color:var(--muted);margin-top:3px">${formatDate(p)}</small>
            </div>
          </div>
          <p style="white-space:pre-wrap;margin-bottom:0">${esc(p.text||"")}</p>
        </div>`).join("") : `<div class="empty">Noch keine Beiträge. Starte den Austausch!</div>`}
    </div>
    ${footer()}`;
}
